...
class CleanArchitectureMigrator:
    ...
    def generate_migration_plan(self, analysis: Dict[str, any]) -> List[MigrationItem]:
        ...
        if (
            "routes/agents.py" in analysis["files"]
            or "routes/agents_new.py" in analysis["files"]
        ):
            plan.append(
                MigrationItem(
...
def main():
    ...
    print(
        f"- Total endpoints found: {sum(len(f.get('endpoints', [])) for f in analysis['files'].values())}"
    )
    print(
        f"- Total classes found: {sum(len(f.get('classes', [])) for f in analysis['files'].values())}"
    )
    print()
