...
class MonitoringConfig:
    ...
    def add_health_check(self, name: str, check_function: callable, interval: int = 30, timeout: int = 10, critical: bool = False):
        """Add a health check."""
    ...
    def register_metric(self, name: str, value: float, unit: str, tags: Optional[Dict[str, Any]] = None):
        """Register a metric."""
        ...
        for health_check in self.health_checks:
            try:
                ...
                if (health_check.last_check is None or 
                    ...
                    # Track performance
                    self.track_performance(f"health_check_{health_check.name}", duration)
    ...
    def get_overall_health(self) -> HealthStatus:
        ...
        all_healthy = all(check.last_status == HealthStatus.HEALTHY for check in self.health_checks)
...
def track_request_metrics(request_id: str, method: str, path: str, status_code: int, duration: float):
...
def track_service_metrics(service_name: str, method: str, duration: float, success: bool):
