...
def analyze_agent_complexity():
    ...
    agent_utils_lines = len(open('shared/core/agents/agent_utilities.py', 'r', encoding='utf-8').readlines())
    retrieval_utils_lines = len(open('shared/core/agents/retrieval_utilities.py', 'r', encoding='utf-8').readlines())
    ...
    total_agent_method_lines = factcheck_process_task_lines + synthesis_process_task_lines
    ...
    print(f"✅ Code reuse ratio: {(total_utility_lines / (total_agent_method_lines + total_utility_lines) * 100):.1f}%")
    ...
    if 'AgentTaskProcessor' in synthesis_content:
        synthesis_improvements.append("✅ Uses AgentTaskProcessor for standardized workflow")
