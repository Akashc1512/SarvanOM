#!/usr/bin/env python3
"""
Standalone Windows-Compatible Metrics Collector
...
class StandaloneWindowsMetricsCollector:
    """Standalone Windows-compatible metrics collector."""
    ...
    def _init_system_metrics(self):
        ...
        self._create_metric("memory_usage_bytes", "Memory usage in bytes", MetricType.GAUGE, ["type"])
        self._create_metric("cpu_usage_percent", "CPU usage percentage", MetricType.GAUGE, ["core"])
        self._create_metric("disk_usage_percent", "Disk usage percentage", MetricType.GAUGE, ["device"])
    
    def _create_metric(self, name: str, description: str, metric_type: MetricType, label_names: List[str] = None):
        """Create a new metric."""
        with self.lock:
            self.metrics[name] = {
    ...
    def inc(self, metric_name: str, amount: float = 1.0, labels: Dict[str, str] = None):
        ...
        with self.lock:
            if metric_name not in self.metrics:
                self._create_metric(metric_name, f"Counter for {metric_name}", MetricType.COUNTER)
    ...
    def set(self, metric_name: str, value: float, labels: Dict[str, str] = None):
        ...
        with self.lock:
            if metric_name not in self.metrics:
                self._create_metric(metric_name, f"Gauge for {metric_name}", MetricType.GAUGE)
    ...
    def observe(self, metric_name: str, value: float, labels: Dict[str, str] = None):
        ...
        with self.lock:
            if metric_name not in self.metrics:
                self._create_metric(metric_name, f"Histogram for {metric_name}", MetricType.HISTOGRAM)
    ...
    def _set_value(self, metric_name: str, label_key: str, value: float):
        ...
        if len(self.metrics[metric_name]["values"][label_key]) > 1000:
            self.metrics[metric_name]["values"][label_key] = self.metrics[metric_name]["values"][label_key][-1000:]
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            summary = {
                "collector_type": "standalone_windows_metrics",
            ...
            for metric_name, metric_data in self.metrics.items():
                summary["metrics"][metric_name] = {
                    ...
                    "values_count": sum(len(values) for values in metric_data["values"].values()),
    ...
    def _get_current_values(self, metric_name: str) -> Dict[str, float]:
        ...
        for label_key, values in metric_data["values"].items():
            if values:
                labels = json.loads(label_key) if label_key != "{}" else {}
                label_str = ",".join(f"{k}={v}" for k, v in sorted(labels.items())) if labels else "default"
            ...
            for metric_name, metric_data in self.metrics.items():
                ...
                for label_key, values in metric_data["values"].items():
                    if values:
                        ...
                        if label_str:
                            lines.append(f"{metric_name}{{{label_str}}} {values[-1].value}")
                        else:
...
def get_metrics_collector() -> StandaloneWindowsMetricsCollector:
    """Get the global metrics collector instance."""
    global _metrics_collector
    if _metrics_collector is None:
        _metrics_collector = StandaloneWindowsMetricsCollector()
...
def record_request(method: str, endpoint: str, status: int, duration: float):
    ...
    collector.inc("http_requests_total", labels={"method": method, "endpoint": endpoint, "status": str(status)})
    collector.observe("http_request_duration_seconds", duration, labels={"method": method, "endpoint": endpoint})

def record_error(method: str, endpoint: str, error_type: str):
    """Record error metrics."""
    collector = get_metrics_collector()
    collector.inc("http_errors_total", labels={"method": method, "endpoint": endpoint, "error_type": error_type})
...
def check_monitoring_health() -> Dict[str, Any]:
    ...
    return {
        ...
        "collector_type": "standalone_windows_metrics",
        "platform": platform.system(),
        "message": "Standalone Windows-compatible metrics collector operational"
    }

if __name__ == "__main__":
    # Test the implementation
    print("Testing Standalone Windows Metrics Collector")
    print("=" * 50)
