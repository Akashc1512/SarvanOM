...
class TestRealBackendPipeline:
    ...
    def test_basic_pipeline_flow(self, authenticated_client, test_session_id):
        ...
        print(
            f"✅ Basic pipeline test passed - LLM Provider: {data['llm_provider']}, "
    ...
    def test_cache_hit_behavior(self, authenticated_client, test_session_id):
        ...
        print(
            f"✅ Cache hit test passed - Cache: {data2['cache_status']}, "
    ...
    def test_llm_routing_for_complex_query(self, authenticated_client, test_session_id):
        ...
        assert any(
            ...
            for keyword in ["vector", "database", "knowledge", "graph", "ai"]
        )
        ...
        print(
            f"✅ Complex query routing test passed - LLM Provider: {data['llm_provider']}, "
    ...
    def test_fallback_on_ollama_failure(self, authenticated_client, test_session_id):
        ...
        # Mock Ollama failure
        with patch(
            "shared.core.llm_client_v3.OllamaProvider.generate_text"
        ) as mock_ollama:
        ...
        print(
            f"✅ Fallback test passed - LLM Provider: {data['llm_provider']}, "
            f"Fallback successful"
        )

    def test_factcheck_freshness_validation(
        self, authenticated_client, test_session_id
    ):
        ...
        if data["validation_status"] == "Outdated":
            assert any(
                ...
                for keyword in ["outdated", "old", "2020", "previous", "historical"]
            )

        print(
            f"✅ Freshness validation test passed - Status: {data['validation_status']}"
        )
    ...
    def test_technical_query_routing(self, authenticated_client, test_session_id):
        ...
        print(
            f"✅ Technical query test passed - Confidence: {data['confidence_score']:.2f}"
        )
    ...
    def test_hybrid_retrieval_validation(self, authenticated_client, test_session_id):
        ...
        print(
            f"✅ Hybrid retrieval test passed - Vector: {len(vector_results)}, "
    ...
    def test_error_handling_and_recovery(self, authenticated_client, test_session_id):
        ...
        with patch(
            "services.search_service.retrieval_agent.RetrievalAgent.process_task"
        ) as mock_retrieval:
            mock_retrieval.side_effect = Exception(
        ...
        assert response.status_code in [
            200,
            500,
        ]  # Allow both success and graceful failure
    ...
    def test_performance_under_load(self, authenticated_client, test_session_id):
        ...
        avg_execution_time = (
            total_execution_time / successful_responses
            if successful_responses > 0
            else 0
        )
        print(
            f"✅ Performance test passed - Success: {successful_responses}/{len(queries)}, "
