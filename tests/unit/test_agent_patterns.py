...
class TestSynthesisStrategy:
    ...
    @pytest.mark.asyncio
    async def test_synthesis_strategy_execution(
        self, synthesis_strategy, valid_task, context
    ):
    ...
    @pytest.mark.asyncio
    async def test_synthesis_strategy_llm_failure_fallback(
        self, synthesis_strategy, valid_task, context
    ):
    ...
    def test_synthesis_strategy_calculate_confidence(self, synthesis_strategy):
        ...
        facts = [{"confidence": 0.8}, {"confidence": 0.9}, {"confidence": 0.7}]
    ...
    def test_synthesis_strategy_fallback_synthesis(self, synthesis_strategy):
        ...
        assert (
            "Low confidence fact" not in result.data["answer"]
...
class TestFactCheckStrategy:
    ...
    @pytest.mark.asyncio
    async def test_factcheck_strategy_execution(
        self, factcheck_strategy, valid_task, context
    ):
    ...
    def test_factcheck_strategy_is_factual_statement(self, factcheck_strategy):
        ...
        assert not factcheck_strategy._is_factual_statement(
            "What is the capital?"
        )  # Question
        assert not factcheck_strategy._is_factual_statement(
            "I think it's good"
    ...
    def test_factcheck_strategy_is_relevant_to_claim(self, factcheck_strategy):
        ...
        # Relevant content
        assert factcheck_strategy._is_relevant_to_claim(
            claim, "The Earth is round and orbits the Sun"
        )
        assert factcheck_strategy._is_relevant_to_claim(
            claim, "Planet Earth has a spherical shape"
        )

        # Irrelevant content
        assert not factcheck_strategy._is_relevant_to_claim(
            claim, "The weather is sunny today"
        )
        assert not factcheck_strategy._is_relevant_to_claim(
            claim, "Cats are domestic animals"
        )
    ...
    def test_factcheck_strategy_calculate_verification_confidence(
        self, factcheck_strategy
    ):
        """Test verification confidence calculation."""
        verifications = [{"confidence": 0.8}, {"confidence": 0.9}, {"confidence": 0.7}]

        confidence = factcheck_strategy._calculate_verification_confidence(
            verifications
        )
...
class TestRetrievalStrategy:
    ...
    @pytest.mark.asyncio
    async def test_retrieval_strategy_execution(
        self, retrieval_strategy, valid_task, context
    ):
    ...
    @pytest.mark.asyncio
    async def test_retrieval_strategy_query_expansion_failure(
        self, retrieval_strategy, valid_task, context
    ):
        ...
        with patch.object(retrieval_strategy, "_call_llm") as mock_llm:
            ...
            assert result.success
            assert result.data["expanded_queries"] == [
    ...
    def test_retrieval_strategy_parse_ranked_results(self, retrieval_strategy):
        ...
        ranked_results = retrieval_strategy._parse_ranked_results(
    ...
    def test_retrieval_strategy_calculate_retrieval_confidence(
...
class TestStrategyBasedAgent:
    ...
    async def test_strategy_based_agent_process_task(self):
        ...
        task = {
            "verified_facts": [{"claim": "test", "confidence": 0.8}],
            "query": "test",
        }
