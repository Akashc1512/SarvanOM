...
class AIImprovementsTester:
    ...
    async def test_basic_query(self, query: str) -> Dict[str, Any]:
        ...
        response = await self.session.post(
            ...
            json={"query": query, "max_tokens": 500, "confidence_threshold": 0.7},
        )
    ...
    async def test_task_generation(
        self, answer: str, query: str = None
    ) -> Dict[str, Any]:
        ...
        response = await self.session.post(f"{self.base_url}/tasks", json=payload)
        ...
        if response.status_code == 200:
            ...
            for i, task in enumerate(tasks, 1):
                print(
                    f"   {i}. {task.get('task', '')} [{task.get('priority', 'Medium')}]"
                )
    ...
    async def run_comprehensive_test(self):
        ...
        print(
            f"   ✅ WebSocket collaboration: {'Working' if self.test_collaboration_websocket() else 'Failed'}"
        )
        print(
            f"   ✅ WebSocket query updates: {'Working' if self.test_query_updates_websocket() else 'Failed'}"
        )
