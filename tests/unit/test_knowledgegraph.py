...
class TestArangoDBRelationshipQuery:
    ...
    def mock_arangodb_client(self):
        ...
        with patch('shared.core.agents.knowledge_graph_agent.ArangoClient') as mock_client:
    ...
    @pytest.mark.asyncio
    async def test_arangodb_relationship_query_single_entity(self, agent, mock_arangodb_client):
        """
        ...
        ml_relationship = next((r for r in result.relationships if "machine_learning" in r.target_id), None)
        ...
        entity_docs = [doc for doc in pseudo_docs if doc["source_type"] == "knowledge_graph"]
        ...
        print(f"✅ Single entity query successful - found {len(result.entities)} entities, {len(result.relationships)} relationships")
        print(f"✅ Generated {len(pseudo_docs)} pseudo-documents for downstream processing")
        
        return True
    
    @pytest.mark.asyncio
    async def test_arangodb_relationship_query_two_entities(self, agent, mock_arangodb_client):
        """
        ...
        python_entity = next((e for e in result.entities if e.name == "Python"), None)
        ai_entity = next((e for e in result.entities if e.name == "Artificial Intelligence"), None)
        assert python_entity is not None
        ...
        assert "Python -> Machine Learning -> Artificial Intelligence" in path_doc["title"]
        assert path_doc["score"] == 0.9
        
        print(f"✅ Two entity path finding successful - found path with {len(path)} entities")
        print(f"✅ Path: {' -> '.join([e.name for e in path])}")
        
        return True
    
    @pytest.mark.asyncio
    async def test_arangodb_relationship_query_entities_not_found(self, agent, mock_arangodb_client):
        """
    ...
    @pytest.mark.asyncio
    async def test_arangodb_relationship_query_connection_error(self, agent, mock_arangodb_client):
        """
    ...
    async def test_pseudo_document_formatting(self, agent):
        ...
        print(f"✅ Pseudo-document formatting successful - generated {len(pseudo_docs)} documents")
    ...
    async def test_parameterized_query_safety(self, agent, mock_arangodb_client):
        ...
        if call_args:
            ...
            assert "@entity" in aql_query or "@entity1" in aql_query or "DOCUMENT" in aql_query
