...
class TestDynamicLLMClient:
    ...
    def client(self):
        ...
        with patch.dict(os.environ, {
            ...
            with patch('shared.core.query_classifier.QueryClassifier', side_effect=ImportError("QueryClassifier not available")):
    ...
    async def test_provider_fallback_logic(self, client):
        ...
        with patch.object(client, '_classify_query', return_value=QueryType.SIMPLE_FACTUAL):
    ...
    async def test_dispatch_research_synthesis_query(self, client):
        ...
        # Mock query classification
        with patch.object(client, '_classify_query', return_value=QueryType.RESEARCH_SYNTHESIS):
            response = await client.dispatch(query, context)
    ...
    async def test_dispatch_complex_reasoning_query(self, client):
        ...
        # Mock query classification
        with patch.object(client, '_classify_query', return_value=QueryType.COMPLEX_REASONING):
            response = await client.dispatch(query, context)
    ...
    async def test_dispatch_all_providers_fail(self, client):
        ...
        with patch.object(client, '_classify_query', return_value=QueryType.SIMPLE_FACTUAL):
            ...
            if "RetryError" in error_str:
                ...
                original_exception = exc_info.value.args[0] if exc_info.value.args else None
            ...
            assert "All LLM providers failed" in error_str or "Provider failed" in error_str
    ...
    async def test_convenience_functions(self):
        ...
        from shared.core.llm_client_dynamic import get_dynamic_llm_client, dispatch_query, generate_text
