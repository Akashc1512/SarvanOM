...
class TestEnhancedLLMClientV3:
    ...
    async def test_client_generate_text_fallback(self):
        ...
        config1 = LLMConfig(
            provider=LLMProvider.MOCK, model="mock-model-1", api_key="mock-key-1"
        )
        config2 = LLMConfig(
            provider=LLMProvider.MOCK, model="mock-model-2", api_key="mock-key-2"
        )
        ...
        client.providers[0].generate_text = AsyncMock(
