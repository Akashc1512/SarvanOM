...
class TestFrontendStateService:
    ...
    def test_update_state_new_session(
        self, service, mock_db_session, sample_state_data
    ):
    ...
    def test_update_state_existing_session(
        self, service, mock_db_session, sample_state_data
    ):
        ...
        result = service.update_state(
            "existing_session_123", sample_state_data, "user_456"
        )
    ...
    def test_update_state_database_error(
        self, service, mock_db_session, sample_state_data
    ):
    ...
    def test_get_states_by_user(self, service, mock_db_session):
        ...
        # Setup mock
        mock_states = [Mock(spec=FrontendState), Mock(spec=FrontendState)]
        mock_states[0].to_dict.return_value = {
            "session_id": "session_1",
            "user_id": "user_123",
        }
        mock_states[1].to_dict.return_value = {
    ...
    def test_get_state_value_not_found(self, service, mock_db_session):
        ...
        # Execute
        result = service.get_state_value(
            "nonexistent_session", "sidebar", "default_value"
        )
    ...
    def test_comprehensive_state_workflow(
        self, service, mock_db_session, sample_state_data
    ):
        ...
        updated_state = service.set_state_value(
            "workflow_session", "new_preference", "dark_mode"
        )
        ...
        mock_state.current_view_state = (
            sample_state_data  # Set actual data instead of Mock
        )
