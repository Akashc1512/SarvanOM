...
from shared.vectorstores.vector_store_service import VectorStoreService as HybridRetrievalEngine  # type: ignore
...
class EnhancedRetrievalResult:
    def __init__(self):
        ...
        self.source_types = [RetrievalSource.VECTOR_DB]

class HybridRetrievalResultV2:
    def __init__(self):
        self.query = ""
        self.enhanced_results = [EnhancedRetrievalResult()]
...
class TestHybridRetrievalFusion:
    ...
    def mock_engines(self):
        ...
        with (
            patch(
                "services.search_service.core.meilisearch_engine.MeilisearchEngine"
            ) as mock_meilisearch,
    ...
    async def test_basic_fusion_scenario(self, engine):
        ...
        result = await engine.retrieve(
            query=query, max_results=5, fusion_strategy=FusionStrategy.WEIGHTED_SUM
        )
    ...
    async def test_multi_source_boost(self, engine):
        ...
        multi_source_results = [
            r for r in result.enhanced_results if len(r.source_types) > 1
        ]
    ...
    async def test_engine_failure_handling(self, engine, mock_engines):
        ...
        mock_engines["vector_db"].search_vectors.side_effect = Exception(
    ...
    async def test_document_deduplication(self, engine):
        ...
        result = await engine.retrieve(
            query=query, max_results=10, fusion_strategy=FusionStrategy.WEIGHTED_SUM
        )
    ...
    async def test_fusion_strategies(self, engine):
        ...
        for strategy in strategies:
            result = await engine.retrieve(
                query=query, max_results=5, fusion_strategy=strategy
            )
...
class TestCombinedScoreCalculation:
    ...
    def test_single_source_score(self, engine):
        ...
        score = engine._calculate_combined_score(
            source_scores, FusionStrategy.WEIGHTED_SUM
        )
        assert 0 <= score <= 1.0
    ...
    def test_multi_source_boost(self, engine):
        ...
        single_score = engine._calculate_combined_score(
            single_source, FusionStrategy.WEIGHTED_SUM
        )
        multi_score = engine._calculate_combined_score(
            multi_source, FusionStrategy.WEIGHTED_SUM
        )
