...
class SecurityTester:
    ...
    def test_authorization(self) -> SecurityTestResult:
        ...
        try:
            ...
            response = self.session.post(
                f"{BASE_URL}/query", json={"query": "test query"}, headers=headers
            )
    ...
    def test_input_validation(self) -> SecurityTestResult:
        ...
        for malicious_input in malicious_inputs:
            try:
                ...
                validation_failures.append({"input": malicious_input, "error": str(e)})
    ...
    def test_rate_limiting(self) -> SecurityTestResult:
        ...
        for i in range(10):
            try:
                ...
                rapid_attempts.append({"attempt": i + 1, "error": str(e)})

        # Check if rate limiting was triggered
        rate_limited = any(
            attempt.get("status_code") == 429 for attempt in rapid_attempts
        )
    ...
    def test_audit_logging(self) -> SecurityTestResult:
        ...
        for action_name, method, endpoint, data in actions:
            try:
                ...
                logged_actions.append(
                    {"action": action_name, "endpoint": endpoint, "error": str(e)}
                )

        # In a real implementation, we would check the logs
        # For now, we'll assume logging is working if requests succeed
        successful_actions = [
            action for action in logged_actions if action.get("success", False)
        ]
    ...
    def test_api_security(self) -> SecurityTestResult:
        ...
        try:
            ...
            security_tests.append(
                {"test": "CORS", "passed": False, "details": {"error": str(e)}}
            )
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        owasp_compliance = (
            sum(compliance["OWASP Top 10"].values())
            / len(compliance["OWASP Top 10"])
            * 100
        )
        nist_compliance = (
            sum(compliance["NIST Cybersecurity Framework"].values())
            / len(compliance["NIST Cybersecurity Framework"])
            * 100
        )
