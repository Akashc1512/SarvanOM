...
class TestServicePerformance:
        ...
        with patch(
            ...
            for _ in range(10):
                ...
                response = client.post(
                    ...
                    json={"query": "performance test", "search_engine": "google"},
                )
    ...
    async def test_concurrent_service_usage(self, client, mock_browser_service):
        ...
        def make_request():
            ...
            return client.post(
                "/agents/browser/search",
                json={"query": "concurrent test", "search_engine": "google"},
            )
    ...
    async def test_concurrent_different_services(
        self, client, mock_browser_service, mock_pdf_service
    ):
        ...
        def make_browser_request():
            return client.post(
                "/agents/browser/search",
                json={"query": "browser test", "search_engine": "google"},
            )
    ...
    async def test_response_time_distribution(self, client, mock_browser_service):
        ...
        with patch(
            ...
            for i in range(20):
                ...
                response = client.post(
                    "/agents/browser/search",
                    json={"query": f"test query {i}", "search_engine": "google"},
                )
        ...
        assert (
            median_response_time < 0.5
        ), f"Median response time too high: {median_response_time:.3f}s"
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                futures = [
                    executor.submit(
                        lambda: client.post(
                            "/agents/browser/search",
                            json={"query": "memory test", "search_engine": "google"},
                        )
                    )
                    for _ in range(50)
                ]
    ...
    async def test_service_error_performance(self, client, mock_browser_service):
        ...
        with patch(
            ...
            for _ in range(10):
                ...
                response = client.post(
                    "/agents/browser/search",
                    json={"query": "error test", "search_engine": "google"},
                )
        ...
        assert (
            avg_error_time < 1.0
        ), f"Error response time too high: {avg_error_time:.3f}s"
    ...
    async def test_service_initialization_performance(self):
        ...
        assert (
            avg_init_time < 0.1
        ), f"Service initialization too slow: {avg_init_time:.3f}s"
...
class TestLoadTesting:
    ...
    def mock_services(self):
        ...
        browser_service.search_web = AsyncMock(
            return_value={
                "success": True,
                "data": {
                    "results": [{"title": "Load Test Result", "url": "http://test.com"}]
                },
                "service_used": "BrowserService",
            }
        )
    ...
    async def test_low_load_performance(self, client, mock_services):
        ...
        with (
            ...
            for i in range(5):
                ...
                response = client.post(
                    "/agents/browser/search",
                    json={"query": f"low load test {i}", "search_engine": "google"},
                )
                response_times.append(time.time() - start_time)
    ...
    async def test_medium_load_performance(self, client, mock_services):
        ...
        def make_request():
            return client.post(
                "/agents/browser/search",
                json={"query": "medium load test", "search_engine": "google"},
            )
    ...
    async def test_high_load_performance(self, client, mock_services):
        ...
        def make_request():
            return client.post(
                "/agents/browser/search",
                json={"query": "high load test", "search_engine": "google"},
            )
        ...
        with patch(
            ...
            assert (
                success_rate >= 0.8
            ), f"Success rate too low under high load: {success_rate:.1%}"
...
class TestResourceManagement:
    ...
    async def test_concurrent_resource_usage(self, client, mock_browser_service):
        ...
        def make_request():
            return client.post(
                "/agents/browser/search",
                json={"query": "resource test", "search_engine": "google"},
            )
