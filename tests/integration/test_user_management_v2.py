...
from shared.core.user_management_v2 import (
    # Exceptions
    UserManagementError,
    UserNotFoundError,
...
class TestPasswordStrength:
    ...
    def test_password_strength_scoring(
        self, password: str, expected_score: int
    ) -> None:
...
class TestUserCreateRequest:
    ...
    def test_invalid_email(self, email: str) -> None:
        ...
        with pytest.raises(ValueError):
            UserCreateRequest(
                username="testuser", email=email, password=TestConstants.VALID_PASSWORD
            )
...
class TestUserModel:
    ...
    @pytest.mark.parametrize(
        "status,email_verified,locked,expected",
        [
    ...
    def test_is_active(
        self, status: UserStatus, email_verified: bool, locked: bool, expected: bool
    ) -> None:
...
class TestUserRole:
    ...
    def test_has_permission(
        self, user_role: UserRole, required_role: UserRole, expected: bool
    ) -> None:
...
class TestInMemoryUserRepository:
    ...
    async def test_create_user(
        self, repository: InMemoryUserRepository, regular_user: UserModel
    ) -> None:
...
class TestUserService:
    ...
    async def test_authenticate_success(
        ...
        with patch.object(user_service, "_verify_password", return_value=True):
            result = await user_service.authenticate(
                regular_user.username, TestConstants.VALID_PASSWORD
            )
    ...
    async def test_authenticate_invalid_username(
        self, user_service: UserService, mock_repository: AsyncMock
    ) -> None:
    ...
    async def test_authenticate_wrong_password(
        ...
        with patch.object(user_service, "_verify_password", return_value=False):
            with pytest.raises(InvalidCredentialsError):
                await user_service.authenticate(regular_user.username, "wrong_password")
    ...
    async def test_verify_token_valid(
        self, user_service: UserService, regular_user: UserModel
    ) -> None:
    ...
    async def test_verify_token_invalid(self, user_service: UserService) -> None:
        ...
        with patch.object(user_service, "_verify_password", return_value=True):
            with patch.object(user_service, "_invalidate_user_sessions"):
                result = await user_service.change_password(
                    regular_user.id, "current_password", TestConstants.VALID_PASSWORD
                )
        ...
        with patch.object(user_service, "_verify_password", return_value=False):
            with pytest.raises(InvalidCredentialsError):
                await user_service.change_password(
                    regular_user.id, "wrong_password", TestConstants.VALID_PASSWORD
                )
    ...
    async def test_change_password_weak_new(
        ...
        with patch.object(user_service, "_verify_password", return_value=True):
            with pytest.raises(ValueError, match="password too weak"):
                await user_service.change_password(
                    regular_user.id, "current_password", "weak"
                )
    ...
    async def test_update_user_role(
        ...
        updated = await user_service.update_user_role(
            regular_user.id, UserRole.MODERATOR, admin_user.id
        )
...
class TestUserServiceIntegration:
    ...
    async def test_full_user_lifecycle(
        self, integration_user_service: UserService
    ) -> None:
        ...
        auth_result = await integration_user_service.authenticate(
            "lifecycle_user", TestConstants.VALID_PASSWORD
        )
        ...
        success = await integration_user_service.change_password(
            created_user.id, TestConstants.VALID_PASSWORD, new_password
        )
        assert success is True

        # Verify new password works
        auth_result2 = await integration_user_service.authenticate(
            "lifecycle_user", new_password
        )
        ...
        for i in range(MAX_LOGIN_ATTEMPTS - 1):
            with pytest.raises(InvalidCredentialsError):
                await integration_user_service.authenticate(
                    "rate_limit_user", "wrong_password"
                )
...
class TestUserServicePerformance:
    ...
    async def test_password_hashing_performance(
        self, user_service: UserService, benchmark
    ) -> None:
        """Benchmark password hashing performance."""
        password = TestConstants.VALID_PASSWORD

        # Benchmark hashing
        result = benchmark(user_service._hash_password, password)
    ...
    async def test_token_generation_performance(
        self, user_service: UserService, regular_user: UserModel, benchmark
    ) -> None:
        """Benchmark JWT token generation."""
        result = benchmark(user_service._generate_access_token, regular_user)
    ...
    async def test_user_creation_performance(
        self, integration_user_service: UserService, benchmark
    ) -> None:
