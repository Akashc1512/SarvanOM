...
from services.api_gateway.services.integration_layer import (
...
from shared.core.query_classifier import QueryClassifier as QueryIntelligenceLayer
from shared.core.query_classifier import (
    ProcessedQuery,
    QueryCategory as IntentType,
    QueryComplexity as ComplexityLevel,
)
DomainType = IntentType
from shared.core.agent_orchestrator import AgentOrchestrator as MultiAgentOrchestrator
OrchestrationResult = dict
ModelType = str
RoutingStrategy = str
from shared.vectorstores.vector_store_service import VectorStoreService as HybridRetrievalEngine
HybridRetrievalResult = dict
RetrievalSource = str
FusionStrategy = str
from shared.core.memory_manager import MemoryManager, MemoryType, MemoryPriority
ExpertValidationLayer = object
ConsensusResult = dict
ExpertNetworkType = str
...
class TestIntegrationLayer:
    ...
    async def test_query_analysis_integration(self, integration_layer, sample_request):
        ...
        try:
            ...
            logger.info(
                f"✅ Query analysis test passed - Intent: {query_analysis.intent.value}, Complexity: {query_analysis.complexity.value}"
            )
    ...
    @pytest.mark.asyncio
    async def test_memory_operations_integration(
        self, integration_layer, sample_request
    ):
        ...
        try:
            ...
            memory_operations = await integration_layer._handle_memory_operations(
                sample_request, mock_analysis
            )
    ...
    async def test_hybrid_retrieval_integration(
        ...
        try:
            ...
            retrieval_result = await integration_layer._perform_hybrid_retrieval(
                sample_request, mock_analysis
            )
            ...
            logger.info(
                f"✅ Hybrid retrieval test passed - {len(retrieval_result.results)} results from {retrieval_result.total_sources} sources"
            )
    ...
    @pytest.mark.asyncio
    async def test_expert_validation_integration(
        self, integration_layer, sample_request
    ):
        ...
        try:
            ...
            validation_result = await integration_layer._perform_expert_validation(
                sample_request, mock_retrieval_result
            )
            ...
            if validation_result is not None:
                ...
                logger.info(
                    f"✅ Expert validation test passed - Status: {validation_result.overall_status.value}"
                )
            else:
    ...
    async def test_orchestration_integration(self, integration_layer, sample_request):
        ...
        try:
            ...
            logger.info(
                f"✅ Orchestration test passed - Success: {orchestration_result.success}, Model: {orchestration_result.model_used.value}"
            )
    ...
    async def test_end_to_end_query_processing(self, integration_layer, sample_request):
        ...
        try:
            ...
            logger.info(
                f"✅ End-to-end query processing test passed - Success: {response.success}, Time: {response.processing_time_ms:.2f}ms"
            )
    ...
    async def test_system_health_check(self, integration_layer):
        ...
        try:
            ...
            logger.info(
                f"✅ System health check test passed - Status: {health_status['status']}"
            )
    ...
    async def test_performance_metrics(self, integration_layer, sample_request):
        ...
        try:
            ...
            assert (
                response.processing_time_ms < 30000
            )  # Should complete within 30 seconds

            logger.info(
                f"✅ Performance metrics test passed - Processing time: {response.processing_time_ms:.2f}ms"
            )
...
class TestIntegrationLayerMocked:
    ...
    @pytest.mark.asyncio
    async def test_mocked_query_processing(
        self, mocked_integration_layer, sample_request
    ):
        """Test query processing with mocked components."""
        # Setup mock responses
        mock_response = IntegrationResponse(
            success=True, processing_time_ms=1500.0, metadata={"test": "mocked"}
        )
