...
from shared.core.agents.base_agent import QueryContext, AgentResult, AgentType
...
class TestLeadOrchestratorRefactored:
    ...
    def create_success_result(self, data=None):
        ...
        return AgentResult(
            success=True, data=data or {}, confidence=0.8, execution_time_ms=100
        )

    def create_failure_result(self, error="Test error"):
        """Helper to create a failed AgentResult."""
        return AgentResult(
            success=False, error=error, confidence=0.0, execution_time_ms=0
        )

    @pytest.mark.asyncio
    async def test_execute_retrieval_phase_success(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        with patch.object(orchestrator, "_extract_entities_parallel") as mock_entities:
            ...
            mock_agents["retrieval"].process_task.return_value = (
    ...
    @pytest.mark.asyncio
    async def test_execute_retrieval_phase_timeout(
        self, orchestrator, query_context, mock_agents
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_fact_checking_phase_success(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        mock_agents["fact_check"].process_task.return_value = (
            self.create_success_result(
        ...
        results = await orchestrator._execute_fact_checking_phase(
            query_context, initial_results
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_fact_checking_phase_skip_on_retrieval_failure(
        self, orchestrator, query_context, mock_agents
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_synthesis_phase_success(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        with patch.object(orchestrator, "_prepare_synthesis_input") as mock_prepare:
            ...
            mock_agents["synthesis"].process_task.return_value = (
                self.create_success_result(
                    {
    ...
    @pytest.mark.asyncio
    async def test_execute_citation_phase_success(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        results = await orchestrator._execute_citation_phase(
            query_context, initial_results
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_pipeline_full_success(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        with patch.object(orchestrator, "_extract_entities_parallel") as mock_entities:
            ...
            with patch.object(orchestrator, "_prepare_synthesis_input") as mock_prepare:
                ...
                mock_agents["synthesis"].process_task.return_value = (
                    self.create_success_result({"response": "ML is a subset of AI."})
                )
                mock_agents["citation"].process_task.return_value = (
                    self.create_success_result({"citations": ["[1] Source"]})
                )
    ...
    @pytest.mark.asyncio
    async def test_execute_pipeline_graceful_degradation(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        with patch.object(orchestrator, "_extract_entities_parallel") as mock_entities:
            ...
            with patch.object(orchestrator, "_prepare_synthesis_input") as mock_prepare:
                ...
                mock_agents["fact_check"].process_task.side_effect = Exception(
                    "Fact check failed"
                )
                mock_agents["synthesis"].process_task.return_value = (
                    self.create_success_result({"response": "ML is a subset of AI."})
    ...
    def test_handle_pipeline_failure(self, orchestrator):
        ...
        partial_results = {AgentType.RETRIEVAL: self.create_success_result()}

        results = orchestrator._handle_pipeline_failure(
            partial_results, "Pipeline error"
        )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_timeout_resilience(
        self, orchestrator, query_context, mock_agents
    ):
        ...
        with patch.object(orchestrator, "_extract_entities_parallel") as mock_entities:
            ...
            with patch.object(orchestrator, "_prepare_synthesis_input") as mock_prepare:
                ...
                mock_agents["retrieval"].process_task.side_effect = (
                    asyncio.TimeoutError()
                )
                mock_agents["fact_check"].process_task.return_value = (
                    self.create_success_result()
                )
                mock_agents["synthesis"].process_task.side_effect = (
                    asyncio.TimeoutError()
                )
                mock_agents["citation"].process_task.return_value = (
                    self.create_success_result()
