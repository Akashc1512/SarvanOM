...
class TestAgentIntegration:
    ...
    async def test_full_agent_pipeline(
        self, synthesis_agent, factcheck_agent, retrieval_agent, context
    ):
        """Test complete agent pipeline: retrieval -> fact-checking -> synthesis."""

        # Mock LLM responses for the entire pipeline
        with patch(
            "shared.core.llm_client_v2.EnhancedLLMClient.generate_text"
        ) as mock_llm:
            ...
            mock_llm.side_effect = (
                retrieval_responses + factcheck_responses + [synthesis_response]
            )
            ...
            factcheck_result = await factcheck_agent.process_task(
                factcheck_task, context
            )
    ...
    @pytest.mark.asyncio
    async def test_agent_pipeline_with_fallback(
        self, synthesis_agent, factcheck_agent, retrieval_agent, context
    ):
        ...
        with patch(
            ...
            assert retrieval_result.success
            assert retrieval_result.data["expanded_queries"] == [
                query
    ...
    async def test_agent_pipeline_error_handling(
        ...
        with patch(
            ...
            factcheck_task = {"query": query, "documents": []}  # No documents
            factcheck_result = await factcheck_agent.process_task(
                factcheck_task, context
            )
            ...
            synthesis_task = {"query": query, "verified_facts": []}  # No facts
            synthesis_result = await synthesis_agent.process_task(
                synthesis_task, context
            )
    ...
    @pytest.mark.asyncio
    async def test_agent_pipeline_performance(
        self, synthesis_agent, factcheck_agent, retrieval_agent, context
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_pipeline_complex_query(
        self, synthesis_agent, factcheck_agent, retrieval_agent, context
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_pipeline_health_check(
        self, synthesis_agent, factcheck_agent, retrieval_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_pipeline_concurrent_execution(
        self, synthesis_agent, factcheck_agent, retrieval_agent, context
    ):
    ...
    @pytest.mark.asyncio
    async def test_agent_pipeline_error_recovery(
        self, synthesis_agent, factcheck_agent, retrieval_agent, context
    ):
