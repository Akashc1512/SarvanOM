...
class TestSecurityVulnerabilities:
    ...
    def test_content_type_manipulation(self, client):
        ...
        response = client.post(
            "/query", data="invalid json", headers={"Content-Type": "text/plain"}
        )
    ...
    def test_header_injection(self, client):
        ...
        for header_name, header_value in malicious_headers.items():
            response = client.post(
                "/query", json={"query": "test"}, headers={header_name: header_value}
            )
            # Should not crash
            assert response.status_code in [200, 400, 500]
...
class TestAuthenticationSecurity:
    ...
    def test_invalid_token_handling(self, client):
        ...
        for token in invalid_tokens:
            response = client.post(
                "/query", json={"query": "test"}, headers={"Authorization": token}
            )
...
class TestDataValidationSecurity:
    ...
    def test_null_byte_injection(self, client):
        ...
        null_byte_attempts = ["test\x00query", "\x00test", "test\x00", "\x00\x00\x00"]
    ...
    def test_rate_limit_bypass_attempts(self, client):
        ...
        for headers in bypass_attempts:
            ...
            for i in range(70):  # Over rate limit
                response = client.post(
                    "/query", json={"query": f"bypass test {i}"}, headers=headers
                )
        ...
        for trigger in error_triggers:
            ...
            if response.status_code != 200:
                ...
                for pattern in sensitive_patterns:
                    assert (
                        pattern not in error_message.lower()
    ...
    def test_debug_information_disclosure(self, client):
        ...
        if response.status_code != 200:
            ...
            for pattern in debug_patterns:
                assert (
                    pattern not in error_message.lower()
                ), f"Debug information disclosed: {pattern}"
