...
class TestDataIntegrity:
    ...
    def test_username_uniqueness(self, session):
        ...
        # Create first user
        user1 = User(
            username="testuser", email="test1@example.com", password_hash="hash1"
        )
        session.add(user1)
        session.commit()
    ...
    def test_role_name_uniqueness(self, session):
        ...
        # Create first role
        role1 = Role(name="testrole", description="Test role 1", permissions={})
        session.add(role1)
        session.commit()

        # Try to create second role with same name
        role2 = Role(
            name="testrole", description="Test role 2", permissions={}  # Same name
        )
        session.add(role2)
...
class TestComplexQueries:
    ...
    def test_knowledge_items_by_creator(self, session, sample_data):
        ...
        admin_item = KnowledgeItem(
            title="Admin Item", content="Admin content", created_by=admin_user.id
        )

        user_item = KnowledgeItem(
            title="User Item", content="User content", created_by=regular_user.id
        )
        ...
        admin_items = (
            session.query(KnowledgeItem).filter_by(created_by=admin_user.id).all()
        )
        user_items = (
            session.query(KnowledgeItem).filter_by(created_by=regular_user.id).all()
        )
    ...
    def test_audit_log_by_entity(self, session, sample_data):
        ...
        entity_logs = (
            ...
            .filter_by(entity_type="User", entity_id=admin_user.id)
...
class TestConcurrentAccess:
    ...
    def test_concurrent_user_creation(self, session):
        ...
        def create_user(username, email):
            ...
            SessionLocal = sessionmaker(
                autocommit=False, autoflush=False, bind=session.bind
            )
    ...
    def test_concurrent_session_creation(self, session, sample_data):
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            futures = [executor.submit(create_session, f"token{i}") for i in range(3)]
...
class TestPerformanceScenarios:
    ...
    def test_complex_query_performance(self, session, sample_data):
        ...
        result = (
            session.query(User, KnowledgeItem, Query, AuditLog)
            .join(KnowledgeItem, User.id == KnowledgeItem.created_by)
...
class TestMigrationScenarios:
    ...
    def test_data_migration_simulation(self, session):
        ...
        migrated_users = (
            session.query(User)
            .filter(User.preferences.contains({"new_format": True}))
            .count()
        )
    ...
    def test_schema_evolution_simulation(self, session):
        ...
        upgraded_users = (
            session.query(User)
            .filter(User.metadata_json.contains({"schema_version": "2.0"}))
            .count()
        )
