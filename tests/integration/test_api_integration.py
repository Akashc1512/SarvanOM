...
# Updated security utilities locations
from shared.core.security_middleware import SecurityValidator
from shared.core.secure_auth import AuthConfig, PasswordValidator
...
class TestAPIAuthentication:
    ...
    def test_protected_endpoint_with_auth(self, client: TestClient, user_factory):
        ...
        response = client.post(
            "/api/v2/query", json={"query": "test query"}, headers=headers
        )
...
class TestAPIQueryProcessing:
    ...
    def test_valid_query_processing(self, client: TestClient, query: str, user_factory):
        ...
        response = client.post("/api/v2/query", json={"query": query}, headers=headers)
    ...
    def test_query_validation_errors(self, client: TestClient, user_factory):
        ...
        # Test empty query
        response = client.post("/api/v2/query", json={"query": ""}, headers=headers)
        assert response.status_code == 422

        # Test query too long
        long_query = "a" * 10000
        response = client.post(
            "/api/v2/query", json={"query": long_query}, headers=headers
        )
        assert response.status_code == 422

    @pytest.mark.parametrize("malicious_query", MALICIOUS_QUERIES)
    def test_malicious_query_handling(
        self, client: TestClient, malicious_query: str, user_factory
    ):
        ...
        response = client.post(
            "/api/v2/query", json={"query": malicious_query}, headers=headers
        )
...
class TestAPIRateLimiting:
    ...
    def test_rate_limit_enforcement(self, client: TestClient, user_factory):
        ...
        for i in range(15):  # Exceed rate limit
            response = client.post(
                "/api/v2/query", json={"query": f"test query {i}"}, headers=headers
            )
    ...
    def test_rate_limit_recovery(self, client: TestClient, user_factory):
        ...
        response = client.post(
            "/api/v2/query", json={"query": "recovery test"}, headers=headers
        )
...
class TestAPISecurity:
    ...
    def test_input_sanitization(self):
        ...
        for malicious_input in malicious_inputs:
            ...
            with pytest.raises(ValidationError):
                SecurityValidator.sanitize_input(malicious_input)
    ...
    def test_email_validation(self):
        ...
        for email in valid_emails:
            # Basic email check via simple pattern
            import re
            validated = bool(re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", email))
            assert validated == email.lower().strip()

        for email in invalid_emails:
            with pytest.raises(ValidationError):
                import re
                assert not re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", email)
    ...
    def test_password_validation(self):
        ...
        for password in valid_passwords:
            cfg = AuthConfig(jwt_secret_key="dev" )
            validated, _ = PasswordValidator.validate_password(password, cfg)
            assert validated == password

        for password in invalid_passwords:
            with pytest.raises(ValidationError):
                cfg = AuthConfig(jwt_secret_key="dev" )
                ok, _ = PasswordValidator.validate_password(password, cfg)
                assert ok is False
...
class TestAPIPerformance:
    ...
    def test_query_response_time(
        self, client: TestClient, user_factory, performance_benchmark
    ):
        ...
        # Run benchmark
        result = asyncio.run(
            performance_benchmark.benchmark("query_response_time", benchmark_query)
        )
...
class TestAPIErrorHandling:
    ...
    def test_authentication_errors(self, client: TestClient):
        ...
        response = client.post("/api/v2/query", json={"query": "test"}, headers=headers)
        assert response.status_code == 401
...
class TestAPIHealthAndMonitoring:
    ...
    def test_health_check_with_external_services(
...
class TestAPICaching:
    ...
    def test_query_caching(self, client: TestClient, user_factory):
        ...
        # First request
        response1 = client.post("/api/v2/query", json={"query": query}, headers=headers)

        # Second request (should be cached)
        response2 = client.post("/api/v2/query", json={"query": query}, headers=headers)
...
class TestAPISecurityTests:
    ...
    def test_sql_injection_protection(self, client: TestClient, security_tester):
        ...
        results = asyncio.run(
            security_tester.test_sql_injection("/api/v2/query", client)
        )
        ...
        assert (
            vulnerable_count == 0
        ), f"SQL injection vulnerabilities found: {vulnerable_count}"

    def test_xss_protection(self, client: TestClient, security_tester):
        """Test XSS protection."""
        results = asyncio.run(security_tester.test_xss("/api/v2/query", client))
    ...
    def test_authentication_bypass(self, client: TestClient):
        ...
        for headers in bypass_attempts:
            response = client.post(
                "/api/v2/query", json={"query": "test"}, headers=headers
            )
            assert response.status_code == 401
