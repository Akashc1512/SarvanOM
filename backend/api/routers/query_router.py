...
async def update_query(
    ...
    """Update a query."""
    try:
        data = await orchestrator.update_query_details(
            query_id, update_request.dict(exclude_unset=True)
        )
        return QueryDetailResponse(**data)
    except ValueError as e:
...
async def reprocess_query(
    ...
    """Reprocess a query."""
    try:
        data = await orchestrator.reprocess_query(
            query_id,
            (
                {"agent_preferences": reprocess_request.agent_preferences}
