...
from ...models.requests.database_requests import (
    DatabaseQueryRequest,
    DatabaseAnalysisRequest,
)
from ...models.responses.database_responses import (
    DatabaseResponse,
...
class DatabaseErrorHandler:
    ...
    def handle_database_error(
        operation: str, error: Exception, user_id: str = "anonymous"
    ) -> DatabaseResponse:
...
def create_database_metadata(user_id: str, **additional_metadata) -> Dict[str, Any]:
...
async def database_health_check(
    ...
    try:
        ...
        return DatabaseErrorHandler.handle_database_error(
            operation="health check", error=e, user_id=get_user_id(current_user)
        )
...
async def database_metrics(
    ...
    try:
        ...
        processing_time = tracker.get_processing_time()
        return DatabaseErrorHandler.handle_database_error(
            operation="get metrics", error=e, user_id=get_user_id(current_user)
        )
...
async def execute_database_query(
    ...
    try:
        ...
        DatabaseErrorHandler.validate_request(
            request.dict(), ["database_name", "query"]
        )
...
async def get_database_schema(
    ...
    try:
        ...
        return DatabaseErrorHandler.handle_database_error(
            operation="Schema retrieval", error=e, user_id=get_user_id(current_user)
        )
...
async def analyze_database_data(
    ...
    try:
        # Validate request
        DatabaseErrorHandler.validate_request(request.dict(), ["table_name"])
        ...
        return DatabaseErrorHandler.handle_database_error(
            operation="Data analysis", error=e, user_id=get_user_id(current_user)
        )
...
async def optimize_database_query(
    ...
    try:
        # Validate request
        DatabaseErrorHandler.validate_request(request.dict(), ["query"])

        optimization = await database_service.optimize_query(
            database_name=database_name, query=request.query
        )
        ...
        return DatabaseResponseFormatter.format_success(
            ...
            processing_time=processing_time,
            metadata=create_database_metadata(
                user_id=user_id, database_name=database_name, query=request.query
            ),
            user_id=user_id,
        )
        ...
        return DatabaseErrorHandler.handle_database_error(
            operation="Query optimization", error=e, user_id=get_user_id(current_user)
        )
...
async def list_databases(
    ...
    try:
        ...
        return DatabaseListResponse(
            ...
            processing_time=tracker.get_processing_time(),
            metadata=create_database_metadata(
                get_user_id(current_user), service="database"
            ),
            timestamp=datetime.now(),
        )
...
async def test_database_connection(
    ...
    try:
        ...
        return DatabaseErrorHandler.handle_database_error(
            operation="Connection test", error=e, user_id=get_user_id(current_user)
        )
