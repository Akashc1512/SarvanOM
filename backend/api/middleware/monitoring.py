...
class PerformanceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if len(self.response_times[request_key]) > 100:
                self.response_times[request_key] = self.response_times[request_key][-100:]
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for endpoint, times in self.response_times.items():
            if times:
                endpoint_stats[endpoint] = {
                    ...
                    "error_rate": self.error_rates[endpoint] / max(self.request_counts[endpoint], 1)
                }
        ...
        return {
            ...
            "overall_error_rate": sum(self.error_rates.values()) / max(sum(self.request_counts.values()), 1),
...
class HealthCheckMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            self._update_health_status("api", "healthy" if response.status_code < 500 else "unhealthy")
    ...
    async def _get_health_status(self) -> str:
        ...
        unhealthy_components = [
            comp for comp, data in self.health_checks.items() 
