...
class QueryProcessor:
    ...
    async def _classify_query(self, query: Query) -> Dict[str, Any]:
        ...
        classification_agent = await self.agent_coordinator.get_agent(AgentType.RETRIEVAL)
        ...
        return {
            ...
            "requires_fact_checking": classification_result.get("requires_fact_checking", False),
            "requires_synthesis": classification_result.get("requires_synthesis", True)
        }
    
    async def _retrieve_information(self, query: Query, classification: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _generate_answer(self, query: Query, retrieval_results: Dict[str, Any]) -> str:
    ...
    async def _analyze_query(self, query: Query) -> Dict[str, Any]:
        ...
        return {
            ...
            "required_sources": analysis_result.get("required_sources", ["web", "database"]),
            "verification_needed": analysis_result.get("verification_needed", True),
            "synthesis_approach": analysis_result.get("synthesis_approach", "comprehensive")
        }
    
    async def _multi_source_retrieval(self, query: Query, analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_query_pipeline(self, query: Query, context: QueryContext) -> Dict[str, Any]:
    ...
    async def _execute_fact_checking(self, query: Query, search_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _get_cached_result(self, query: Query, context: QueryContext) -> Optional[Dict[str, Any]]:
        """Get cached result for query."""
        cache_key = f"query:{hash(query.text)}:{context.user_id}"
        return await self.cache_service.get(cache_key)
    
    async def _cache_result(self, query: Query, context: QueryContext, result: Dict[str, Any]):
        """Cache query result."""
        cache_key = f"query:{hash(query.text)}:{context.user_id}"
    ...
    async def _execute_comprehensive_pipeline(
        ...
        try:
            ...
            return await self._format_comprehensive_response(
                query, retrieval_results, verification_results, synthesis_results, quality_results, context
            )
    ...
    async def _execute_multi_source_retrieval(self, query: Query, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        retrieval_result = await retrieval_agent.process({
            ...
            "query": query.text,
            "analysis": analysis,
            "sources": analysis.get("required_sources", ["web", "database", "knowledge_graph"]),
            "max_results_per_source": 5,
            "context": query.context.metadata
        })
    ...
    async def _format_comprehensive_response(
        ...
        return {
            ...
            "synthesis_method": synthesis_results.get("synthesis_method", "comprehensive"),
            "key_points": synthesis_results.get("key_points", []),
            "source_usage": synthesis_results.get("source_usage", {}),
            "metadata": {
                "query_id": context.query_id,
    ...
    async def _track_query(self, query: Query, context: QueryContext, result: Dict[str, Any], cache_hit: bool = False):
    ...
    async def get_query_stats(self) -> Dict[str, Any]:
        ...
        successful_queries = len([q for q in self.query_history if q.get("success", True)])
        avg_processing_time = sum(q.get("processing_time", 0) for q in self.query_history) / total_queries
        ...
        return {
            ...
            "cache_hit_rate": len([q for q in self.query_history if q.get("cache_hit", False)]) / total_queries if total_queries > 0 else 0
        } 