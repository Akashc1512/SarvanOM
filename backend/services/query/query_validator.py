...
class QueryValidator:
    ...
    def _validate_context(self, query: Query):
        ...
        if query.context.confidence_threshold < 0.0 or query.context.confidence_threshold > 1.0:
    ...
    async def _validate_fact_check_query(self, query: Query) -> bool:
        ...
        fact_check_indicators = [
            ...
            'correct', 'right', 'wrong', 'false', 'true'
        ]
        
        query_lower = query.text.lower()
        has_fact_check_indicator = any(indicator in query_lower for indicator in fact_check_indicators)
        
        if not has_fact_check_indicator:
            raise ValueError("Fact check queries should contain factual claims to verify")
    ...
    async def _validate_synthesis_query(self, query: Query) -> bool:
        ...
        synthesis_indicators = [
            'compare', 'analyze', 'synthesize', 'summarize', 'overview',
            'review', 'examine', 'study', 'research'
        ]
        
        query_lower = query.text.lower()
        has_synthesis_indicator = any(indicator in query_lower for indicator in synthesis_indicators)
