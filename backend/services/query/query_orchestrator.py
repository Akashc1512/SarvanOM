...
from ...models.domain.query import (
    Query,
    QueryContext,
    QueryResult,
    QueryStatus,
...
class QueryOrchestrator:
    ...
    async def list_queries(
        ...
        if user_filter:
            queries = await self.query_repository.get_queries_by_user(
                user_filter, limit=10000
            )
        else:
    ...
    async def update_query_details(
        self, query_id: str, updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "confidence_threshold" in updates:
            context_updates["confidence_threshold"] = updates.pop(
        ...
        if context_updates:
            ...
            new_context = QueryContext(
                ...
                confidence_threshold=context_updates.get(
                    "confidence_threshold", q.context.confidence_threshold
                ),
    ...
    async def reprocess_query(
        self, query_id: str, options: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        await self.query_repository.update_query_status(
            query_id, QueryStatus.PROCESSING
        )
        ...
        await self.query_repository.update_query_status(
            query_id, QueryStatus.COMPLETED, result
        )
    ...
    async def process_basic_query(
        self, request: QueryRequest, user_context: Dict[str, Any]
    ) -> QueryResponse:
            ...
            if request.cache_enabled:
                ...
                if cached_result:
                    ...
                    return self._create_basic_response(
                        query, cached_result, cache_hit=True
                    )
    ...
    async def process_comprehensive_query(
        self, request: ComprehensiveQueryRequest, user_context: Dict[str, Any]
    ) -> ComprehensiveQueryResponse:
        ...
        try:
            ...
            result = await self.query_processor.process_comprehensive_query(
            ...
            logger.error(
                f"Error processing comprehensive query {query_id}: {e}", exc_info=True
            )
            if query_id in self.active_queries:
                self.active_queries[query_id].mark_failed(str(e))
    ...
    def _create_basic_response(
        self, query: Query, result: Dict[str, Any], cache_hit: bool
    ) -> QueryResponse:
