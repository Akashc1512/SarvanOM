...
class AgentCoordinator:
    ...
    async def get_agent_status(self, agent_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                "avg_processing_time": (
                    stats.get("total_processing_time", 0.0)
    ...
    async def track_agent_task(
        self, agent_id: str, task_id: str, processing_time: float, success: bool
    ):
        ...
        try:
            if agent_id in self.agent_stats:
                ...
                logger.debug(
                    f"Tracked task for agent {agent_id}: success={success}, time={processing_time}s"
                )
            ...
            if agent_id in self.active_agents:
                ...
                if agent.agent_type in self.agent_pool:
                    self.agent_pool[agent.agent_type] = [
                        a for a in self.agent_pool[agent.agent_type] if a.id != agent_id
                    ]
