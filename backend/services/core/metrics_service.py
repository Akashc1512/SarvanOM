...
class MetricsService:
    ...
    async def track_query_processing(
        self, query_id: str, query_type: str, processing_time: float, cache_hit: bool
    ):
        ...
        try:
            ...
            logger.debug(
                f"Tracked query processing: {query_id}, type: {query_type}, time: {processing_time}s"
            )
    ...
    async def track_query_error(
        self, query_id: str, error_type: str, processing_time: float
    ):
        ...
        try:
            ...
            self._metrics["error_rate"] = (
                total_errors / total_queries if total_queries > 0 else 0
            )

            logger.debug(
                f"Tracked query error: {query_id}, type: {error_type}, time: {processing_time}s"
            )
    ...
    async def track_agent_usage(
        self, agent_type: str, processing_time: float, success: bool
    ):
        ...
        try:
            ...
            self._histograms[f"agent_processing_time_{agent_type}"].append(
                processing_time
            )
            ...
            logger.debug(
                f"Tracked agent usage: {agent_type}, success: {success}, time: {processing_time}s"
            )
    ...
    async def track_cache_operation(
        self, operation: str, key: str, success: bool, duration: float
    ):
        ...
        try:
            ...
            logger.debug(
                f"Tracked cache operation: {operation}, key: {key}, success: {success}, duration: {duration}s"
            )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate averages
            avg_processing_time = self._calculate_average(
                self._timers["query_processing_time"]
            )
            avg_error_time = self._calculate_average(
                self._timers["error_processing_time"]
            )

            # Calculate percentiles
            p95_processing_time = self._calculate_percentile(
                self._timers["query_processing_time"], 95
            )
            p99_processing_time = self._calculate_percentile(
                self._timers["query_processing_time"], 99
            )
    ...
    async def track_performance_metrics(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._timers[f"endpoint_duration_seconds:{endpoint}"].append(
                processing_time
            )
            self._histograms[f"endpoint_duration_seconds:{endpoint}"].append(
                processing_time
            )
    ...
    def get_agent_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for agent_type in agent_types:
                ...
                avg_processing_time = self._calculate_average(
                    self._timers.get(f"agent_processing_time_{agent_type}", [])
                )
        ...
        try:
            ...
            for operation in operations:
                ...
                avg_duration = self._calculate_average(
                    self._timers.get(f"cache_{operation}_duration", [])
                )
    ...
    def export_metrics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "raw_timers": {
                    k: self._calculate_average(v) for k, v in self._timers.items()
                },
