...
class AgentRepository(ABC):
    ...
    @abstractmethod
    async def update_agent_config(self, agent_id: str, config: Dict[str, Any]) -> Optional[Agent]:
    ...
    @abstractmethod
    async def update_agent_performance(self, agent_id: str, performance_data: Dict[str, Any]) -> Optional[Agent]:
...
class AgentRepositoryImpl(AgentRepository, BaseRepository[Agent]):
    ...
    async def list(self, filters: Optional[Dict[str, Any]] = None, offset: int = 0, limit: int = 100) -> List[Agent]:
    ...
    async def update_agent_config(self, agent_id: str, config: Dict[str, Any]) -> Optional[Agent]:
        ...
        try:
            ...
            # Update in storage
            if self.storage_type == "memory":
                updated_agent = await self._memory_store.update(agent_id, {
    ...
    async def update_agent_status(self, agent_id: str, status: AgentStatus) -> Optional[Agent]:
        ...
        try:
            ...
            logger.info(f"Updated agent {agent_id} status from {old_status} to {status}")
        ...
        try:
            if agent_id:
                ...
                return {
                    ...
                    "last_performance_update": getattr(agent, 'last_performance_update', None),
                    "config": agent.config,
                    "timestamp": datetime.now().isoformat()
                }
