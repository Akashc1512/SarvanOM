...
class UserRepositoryImpl(UserRepository, BaseRepository[User]):
    ...
    async def list(self, filters: Optional[Dict[str, Any]] = None, offset: int = 0, limit: int = 100) -> List[User]:
    ...
    async def update_user_status(self, user_id: str, status: UserStatus) -> Optional[User]:
        ...
        try:
            ...
            # Update in storage
            if self.storage_type == "memory":
                updated_user = await self._memory_store.update(user_id, {
                    'status': status,
    ...
    async def authenticate_user(self, username: str, password_hash: str) -> Optional[User]:
        ...
        try:
            ...
            await self.update_user_profile(user.id, {
    ...
    async def get_user_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            new_users_this_week = len([u for u in all_users if u.created_at >= week_start])
