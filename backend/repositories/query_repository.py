...
class QueryRepositoryImpl(QueryRepository, BaseRepository[Query]):
    ...
    async def create_query(self, query: Query) -> Query:
        ...
        try:
            ...
            logger.info(
                f"Created query {created_query.id} for user {created_query.context.user_id}"
            )
    ...
    async def update_query_status(
        ...
        try:
            ...
            logger.info(
                f"Updated query {query_id} status from {old_status} to {status}"
            )
    ...
    async def get_recent_queries(
        ...
        try:
            ...
            # Filter by time
            recent_queries = [
                query for query in all_queries if query.created_at >= cutoff_time
            ]
    ...
    async def get_query_statistics(
        ...
        try:
            ...
            completed_queries = [
                q
                for q in queries
                if q.status == QueryStatus.COMPLETED and hasattr(q, "processing_time")
            ]
            avg_processing_time = 0
            if completed_queries:
                total_time = sum(
                    getattr(q, "processing_time", 0) for q in completed_queries
                )
    ...
    async def _remove_from_indices(self, query: Query):
        ...
        session_id = query.context.session_id
        if (
            session_id in self._session_index
            and query_id in self._session_index[session_id]
        ):
