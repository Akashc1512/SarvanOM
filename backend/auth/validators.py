...
XSS_PATTERN = re.compile(
    r"(<script|javascript:|onerror=|onclick=|<iframe|<object|<embed)", re.IGNORECASE
)
...
class QueryRequestValidator(BaseModel):
    ...
    language: Optional[str] = Field(default="en", description="Query language")
    ...
    user_context: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="User context and preferences"
    )
    ...
    def validate_search_type(cls, v):
        if v and v not in ALLOWED_SEARCH_TYPES:
            raise ValueError(
                f"Invalid search type. Must be one of: {ALLOWED_SEARCH_TYPES}"
            )
        return v
...
class FeedbackRequestValidator(BaseModel):
    ...
    rating: conint(ge=1, le=5) = Field(..., description="Rating from 1-5")
...
class SearchRequestValidator(BaseModel):
    ...
    limit: conint(ge=MIN_RESULTS, le=MAX_RESULTS) = Field(
        default=20, description="Number of results"
    )

    offset: conint(ge=0) = Field(default=0, description="Result offset for pagination")
...
class AnalyticsRequestValidator(BaseModel):
    ...
    metrics: Optional[List[str]] = Field(
        default_factory=list, description="Specific metrics to retrieve"
    )

    group_by: Optional[str] = Field(
        None, pattern=r"^(hour|day|week|month)$", description="Grouping period"
    )
...
class ConfigUpdateValidator(BaseModel):
    ...
    key: Annotated[str, StringConstraints(pattern=r"^[a-zA-Z][a-zA-Z0-9_]*$")] = Field(
        ..., max_length=50, description="Configuration key"
    )

    value: Any = Field(..., description="Configuration value")

    description: Optional[str] = Field(
        None, max_length=200, description="Description of the change"
    )
