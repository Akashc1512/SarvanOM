...
class TestQueryEndpoints:
    ...
    async def test_basic_query_endpoint(self, mock_dependencies):
        ...
        # Setup mock response
        mock_dependencies["orchestrator"].process_basic_query.return_value = (
            QueryResponse(
    ...
    async def test_comprehensive_query_endpoint(self, mock_dependencies):
        ...
        # Setup mock response
        mock_dependencies["orchestrator"].process_comprehensive_query.return_value = (
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.post(
                ...
                json={
                    ...
                    "options": {"include_sources": True, "include_alternatives": True},
                },
            )
    ...
    def test_query_not_found(self, mock_dependencies):
        ...
        mock_dependencies["orchestrator"].get_query_status.side_effect = ValueError(
            "Query not found"
        )
...
class TestErrorHandling:
    ...
    def test_validation_error_handling(self, test_client):
        ...
        response = test_client.post("/query/", json={"invalid_field": "invalid_value"})
    ...
    async def test_internal_server_error_handling(self, mock_dependencies):
        ...
        mock_dependencies["orchestrator"].process_basic_query.side_effect = Exception(
            "Test error"
        )

        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.post(
                "/query/", json={"query": "Test query", "session_id": "test-session"}
            )
