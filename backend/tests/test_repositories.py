...
class TestQueryRepository:
    ...
    async def test_search_queries(self, test_query_repository, sample_query):
        ...
        queries = [
            Query(id="q1", text="What is Python?", context=sample_query.context, query_type=QueryType.BASIC),
            Query(id="q2", text="How to use FastAPI?", context=sample_query.context, query_type=QueryType.BASIC),
            Query(id="q3", text="Python programming tips", context=sample_query.context, query_type=QueryType.BASIC)
        ]
    ...
    async def test_get_query_statistics(self, test_query_repository, sample_query):
        ...
        queries = [
            Query(id="q1", text="Query 1", context=sample_query.context, query_type=QueryType.BASIC),
            Query(id="q2", text="Query 2", context=sample_query.context, query_type=QueryType.COMPREHENSIVE)
        ]
        
        for query in queries:
            await test_query_repository.create_query(query)
            await test_query_repository.update_query_status(query.id, QueryStatus.COMPLETED)
...
class TestUserRepository:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_username_rejection(self, test_user_repository, sample_user):
    ...
    async def test_get_user_by_username(self, test_user_repository, sample_user):
        ...
        retrieved_user = await test_user_repository.get_user_by_username(sample_user.username)
    ...
    async def test_update_user_profile(self, test_user_repository, sample_user):
        ...
        updated_user = await test_user_repository.update_user_profile(sample_user.id, updates)
...
class TestAgentRepository:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_agent_name_rejection(self, test_agent_repository, sample_agent):
    ...
    async def test_get_agents_by_type(self, test_agent_repository):
        ...
        retrieval_agents = await test_agent_repository.get_agents_by_type(AgentType.RETRIEVAL)
        assert len(retrieval_agents) == 1
        assert retrieval_agents[0].agent_type == AgentType.RETRIEVAL
        
        # Get synthesis agents
        synthesis_agents = await test_agent_repository.get_agents_by_type(AgentType.SYNTHESIS)
