...
class DatabaseSchemaResponse(BaseModel):
    ...
    success: bool = Field(..., description="Operation success status")
    database_name: str = Field(..., description="Database name")
    tables: List[Dict[str, Any]] = Field(default_factory=list, description="Database tables")
    views: List[Dict[str, Any]] = Field(default_factory=list, description="Database views")
...
class DatabaseAnalysisResponse(BaseModel):
    ...
    column_stats: Dict[str, Any] = Field(default_factory=dict, description="Column statistics")
    ...
    unique_values: Dict[str, int] = Field(default_factory=dict, description="Unique value counts")
    processing_time: float = Field(..., ge=0.0, description="Processing time in seconds")
...
class DatabaseOptimizationResponse(BaseModel):
    ...
    performance_improvement: float = Field(..., ge=0.0, le=1.0, description="Expected performance improvement")
    database_name: str = Field(..., description="Database name")
    processing_time: float = Field(..., ge=0.0, description="Processing time in seconds")
