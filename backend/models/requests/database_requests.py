...
class DatabaseQueryRequest(BaseModel):
    ...
    timeout: Optional[int] = Field(60, ge=1, le=3600, description="Query timeout in seconds")
...
class DatabaseAnalysisRequest(BaseModel):
    ...
    columns: Optional[List[str]] = Field(default_factory=list, description="Specific columns to analyze")
...
class DatabaseConfigRequest(BaseModel):
    ...
    max_overflow: int = Field(0, ge=0, le=100, description="Maximum overflow connections")
...
class DatabaseOptimizationRequest(BaseModel):
    ...
    analyze_plan: bool = Field(True, description="Whether to analyze query execution plan") 