...
class AgentUpdateRequest(BaseModel):
    ...
    config: Optional[Dict[str, Any]] = Field(
        None, description="Updated agent configuration"
    )
...
class AgentPoolRequest(BaseModel):
    ...
    count: Optional[int] = Field(
        None, ge=1, le=100, description="Number of agents for resize action"
    )
...
class AgentRequest(BaseModel):
    ...
    search_type: Optional[str] = Field(
        "web", description="Search type for browser operations"
    )
    max_results: Optional[int] = Field(10, description="Maximum results to return")
    extraction_type: Optional[str] = Field(
        "full", description="Extraction type for content operations"
    )
    language: Optional[str] = Field(
        "python", description="Programming language for code operations"
    )
    database_type: Optional[str] = Field(
        "postgres", description="Database type for database operations"
    )
    crawl_type: Optional[str] = Field(
        "full", description="Crawl type for crawler operations"
    )
    query_type: Optional[str] = Field(
        "entities", description="Query type for knowledge graph operations"
    )
