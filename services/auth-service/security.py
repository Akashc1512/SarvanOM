...
import secrets
import base64
from typing import Optional, Dict, Any, List, Union, Callable, TypeVar, Protocol, Tuple
...
from shared.core.api.exceptions import ValidationError, SecurityError
from shared.core.api.monitoring import Counter, Histogram
...
security_events = Counter(
    "security_events_total", "Security events", ["event_type", "severity", "source"]
)

security_scan_duration = Histogram(
    "security_scan_duration_seconds", "Security scan duration", ["scan_type"]
)
...
class InputSanitizer:
    ...
    def sanitize_string(
        ...
        if self._detect_sql_injection(value):
            ...
            raise ValidationError(
                "Input contains potentially malicious SQL content", field="input"
            )
        ...
        if self._detect_xss(value):
            ...
            raise ValidationError(
                "Input contains potentially malicious script content", field="input"
            )
        ...
        if self._detect_path_traversal(value):
            ...
            raise ValidationError(
                "Input contains path traversal attempts", field="input"
            )
        ...
        if self._detect_command_injection(value):
            ...
            raise ValidationError(
                "Input contains potentially malicious command content", field="input"
            )
...
class ThreatDetector:
    ...
    def scan_request(
        self, request_data: Dict[str, Any], client_ip: str
        ...
        total_score = sum(self._get_threat_score(t["type"]) for t in threats)
...
class SecurityMiddleware:
    ...
    async def __call__(self, request: Request, call_next: Callable) -> Response:
        ...
        # Scan for threats
        is_safe, threats = self.threat_detector.scan_request(request_data, client_ip)

        if not is_safe:
            # Log security event
            logger.warning(
                "Security threat detected", client_ip=client_ip, threats=threats
            )
...
async def check_security(
    ...
    try:
        ...
        monitored = any(threat.severity == SecurityLevel.MEDIUM for threat in threats)
        ...
        logger.error(
            "Security check failed", error=str(e), client_ip=client_ip, user_id=user_id
        )
