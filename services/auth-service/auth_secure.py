...
from shared.core.api.exceptions import ConfigurationError, AuthenticationError, AuthorizationError
...
if not SECRET_KEY:
    ...
    raise ConfigurationError(
        "authentication", "Missing SECRET_KEY", "Set SECRET_KEY environment variable"
    )
...
class SecureAPIKeyManager:
    ...
    def _load_from_environment(self):
        ...
        if not self.api_keys:
            logger.error(
                "No API keys configured! Set ADMIN_API_KEY, USER_API_KEY, or READONLY_API_KEY environment variables."
            )
            raise ConfigurationError(
                "authentication",
...
def require_permission(permission: Permission):
    ...
    def permission_checker(
        ...
        if not current_user.has_permission(permission):
            logger.warning(
                f"Permission denied: {current_user.user_id} tried to access {permission.value}"
            )
...
def require_role(role: UserRole):
    ...
    def role_checker(
        ...
        if not current_user.has_role(role):
            logger.warning(
                f"Role denied: {current_user.user_id} tried to access {role.value}"
            )
...
async def authenticate_user(username: str, password: str) -> Optional[Dict[str, Any]]:
    ...
    return {"user_id": username, "role": user_data["role"], "email": user_data["email"]}
