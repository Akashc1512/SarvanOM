...
from shared.core.api.exceptions import ConfigurationError
...
def get_user_api_keys(user_id: str) -> List[str]:
    ...
    return [key for key, data in API_KEY_REGISTRY.items() if data["user_id"] == user_id]
...
async def register_user(
    ...
    permissions = (
        ["read", "write"]
        if role == "user"
        else ["read"] if role == "readonly" else ["read", "write", "admin"]
    )
...
class APIKeyAuth:
    ...
    def verify_api_key(api_key: str) -> Optional[Dict[str, Any]]:
        ...
        if api_key in API_KEYS:
            ...
            logger.debug(
                f"User created: {user.user_id}, role: {user.role}"
        ...
        logger.debug(
            f"Failed to create user from API key: {api_key}"
        )  # Proper debug logging
        return None
...
async def get_current_user(
    ...
    if credentials:
        ...
        if user_from_db:
            ...
            return User(
                ...
                permissions=(
                    ["read", "write"]
                    if user_from_db.role in ["admin", "user"]
                    else ["read"]
                ),
            )
    ...
    raise AuthException(
        "Authentication required. Please provide API key or valid token."
    )
