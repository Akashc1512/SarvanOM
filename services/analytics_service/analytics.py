...
class MetricsCollector:
        ...
        async with self._lock:
            try:
                ...
                if len(self.user_activity) > self.max_user_activity_entries:
                    ...
                    sorted_users = sorted(
                        self.user_activity.items(), key=lambda x: x[1], reverse=True
                    )
                    self.user_activity = defaultdict(
                        int, dict(sorted_users[: self.max_user_activity_entries // 2])
                    )
                ...
                logger.debug(
                    f"Cleaned up analytics data: removed {len(keys_to_remove)} hourly stats, {len(agents_to_remove)} agent entries"
                )
    ...
    async def increment_request(
        self, user_id: str, query: str, category: str = "general"
    ):
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            if self.confidence_scores:
                avg_confidence = sum(self.confidence_scores) / len(
