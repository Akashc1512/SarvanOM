...
feedback_stored = Counter(
    "feedback_stored_total", "Total feedback stored", ["feedback_type", "status"]
)

feedback_retrieved = Counter(
    "feedback_retrieved_total", "Total feedback retrieved", ["feedback_type"]
)
...
feedback_storage_size = Gauge(
    "feedback_storage_size_bytes", "Total feedback storage size"
)
...
class FeedbackRequest(BaseModel):
    ...
    priority: FeedbackPriority = Field(
        default=FeedbackPriority.MEDIUM, description="Feedback priority"
    )
    ...
    def validate_details(cls, v: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "comment" in v:
            ...
            if not isinstance(comment, str) or len(comment) > MAX_COMMENT_LENGTH:
                raise ValueError(
                    f"Comment must be a string with max length {MAX_COMMENT_LENGTH}"
                )
...
class FeedbackRepository(ABC):
    ...
    async def update_status(
        self, feedback_id: FeedbackID, status: FeedbackStatus
    ) -> bool:
...
class InMemoryFeedbackRepository(FeedbackRepository):
    ...
    async def get_by_user_id(
        ...
        feedback_list = [
            self.feedback[fid] for fid in feedback_ids if fid in self.feedback
        ]
    ...
    async def get_analytics(
        ...
        if filters:
            if "feedback_type" in filters:
                all_feedback = [
                    ...
                    if f.feedback_type == filters["feedback_type"]
                ]
            if "status" in filters:
                all_feedback = [
                    f for f in all_feedback if f.status == filters["status"]
                ]
            if "user_id" in filters:
                all_feedback = [
                    f for f in all_feedback if f.user_id == filters["user_id"]
                ]
        ...
        for feedback in all_feedback:
            # Count by type
            type_counts[feedback.feedback_type] = (
                type_counts.get(feedback.feedback_type, 0) + 1
            )
            ...
            if (
                ...
                and "rating" in feedback.details
            ):
...
class FeedbackStorage:
    ...
    async def store_feedback(self, request: FeedbackRequest) -> FeedbackModel:
        ...
        try:
            ...
            feedback_processing_duration.labels(operation="store").observe(
                processing_time
            )
            ...
            feedback_stored.labels(
                feedback_type=request.feedback_type.value, status="error"
            ).inc()
            raise
        ...
        try:
            ...
            if self.redis_client:
                await self.redis_client.setex(
                    cache_key, ANALYTICS_CACHE_TTL, json.dumps(analytics)
                )
    ...
    async def _cache_feedback(self, feedback: FeedbackModel):
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, FEEDBACK_CACHE_TTL, feedback.json()
            )
        except Exception as e:
