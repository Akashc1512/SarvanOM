...
async def check_redis() -> Dict[str, Any]:
    ...
    try:
        redis = await aioredis.from_url(
            REDIS_URL, socket_connect_timeout=HEALTH_CHECK_TIMEOUT
        )
...
async def check_openai_api() -> Dict[str, Any]:
    ...
    if not OPENAI_API_KEY:
        return {"healthy": False, "error": "API key not configured", "latency_ms": 0}
...
async def check_all_services() -> Dict[str, Any]:
    ...
    for name, result in zip(service_names, results):
        if isinstance(result, Exception):
            health_status[name] = {"healthy": False, "error": str(result)}
            all_healthy = False
        else:
