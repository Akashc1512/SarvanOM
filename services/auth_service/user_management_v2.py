...
user_creation_counter = Counter(
    "user_creation_total", "Total number of users created", ["status"]
)
...
authentication_duration = Histogram(
    "authentication_duration_seconds", "Time spent authenticating users"
)
...
class AccountLockedException(UserManagementError):
    ...
    def __init__(self, username: str, locked_until: datetime) -> None:
        super().__init__(
            ...
            details={"username": username, "locked_until": locked_until.isoformat()},
        )
...
class UserRole(str, Enum):
    ...
    def has_permission(self, required_role: "UserRole") -> bool:
        ...
        hierarchy = {cls.READONLY: 0, cls.USER: 1, cls.MODERATOR: 2, cls.ADMIN: 3}
...
class UserCreateRequest(BaseModel):
    ...
    def validate_password_strength(cls, v: SecretStr) -> SecretStr:
        ...
        common_passwords = {"password123", "admin123", "qwerty123", "letmein123"}
...
class UserService:
            ...
            try:
                ...
                if not user:
                    authentication_counter.labels(
                        status="failure", method=method.value
                    ).inc()
                ...
                if user.is_locked:
                    authentication_counter.labels(
                        status="locked", method=method.value
                    ).inc()
                    raise AccountLockedException(username, user.locked_until)
                ...
                if not user.can_login():
                    authentication_counter.labels(
                        status="inactive", method=method.value
                    ).inc()
                ...
                # Update metrics
                authentication_counter.labels(
                    status="success", method=method.value
                ).inc()
                active_sessions.inc()
                ...
                authentication_counter.labels(status="error", method=method.value).inc()
    ...
    async def change_password(
        ...
        if strength.score < 3:
            raise ValueError(f"New password too weak. {', '.join(strength.feedback)}")
        ...
        logger.info("Password changed", user_id=user_id, username=user.username)

        return True

    async def update_user_role(
        self, user_id: UserID, new_role: UserRole, updated_by: UserID
    ) -> UserModel:
        """
...
def get_user_service() -> UserService:
    ...
    # Use memory cache instead of Redis to avoid Pydantic validation issues
