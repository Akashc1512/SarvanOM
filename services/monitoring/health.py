    ...
    async def run_check(self, name: str) -> HealthCheckResult:
        ...
        try:
            ...
            self.metrics_collector.record_histogram(
                "health_check_duration", duration, labels={"check": name}
            )
            ...
            self.metrics_collector.increment_counter(
                "health_check_total", labels={"check": name, "status": "unhealthy"}
            )
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            summary = {
                ...
                "healthy_checks": sum(
                    1 for r in self.results.values() if r.status == HealthStatus.HEALTHY
                ),
                "degraded_checks": sum(
                    1
...
def register_custom_health_check(
    name: str, check_func: Callable[[], Awaitable[HealthCheckResult]]
):
    """Register a custom health check."""
    health_checker.register_check(name, check_func)


def register_custom_sync_health_check(
    name: str, check_func: Callable[[], HealthCheckResult]
):
