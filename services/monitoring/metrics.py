...
class MetricsCollector:
    ...
    def increment_counter(self, name: str, value: int = 1, labels: Optional[Dict[str, str]] = None):
        ...
        with self.lock:
            ...
            self._add_metric(name, MetricType.COUNTER, float(self.counters[key]), labels)
    ...
    def _add_metric(self, name: str, metric_type: MetricType, value: float, labels: Optional[Dict[str, str]] = None):
...
class MetricsExporter:
    ...
    def export_health_check_format(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy" if not any(rate > 0.1 for rate in error_rates.values()) else "degraded",
...
def record_api_request(endpoint: str, method: str, status_code: int, duration: float):
    ...
    collector.increment_counter("api_requests_total", labels={"endpoint": endpoint, "method": method, "status": str(status_code)})
    collector.record_histogram("api_request_duration", duration, labels={"endpoint": endpoint, "method": method})


def record_service_call(service_name: str, method: str, duration: float, success: bool):
    """Record service call metrics."""
    collector = get_metrics_collector()
    collector.increment_counter("service_calls_total", labels={"service": service_name, "method": method, "success": str(success)})
    collector.record_histogram("service_call_duration", duration, labels={"service": service_name, "method": method})


def record_database_query(query_type: str, duration: float, success: bool):
    """Record database query metrics."""
    collector = get_metrics_collector()
    collector.increment_counter("database_queries_total", labels={"type": query_type, "success": str(success)})
    collector.record_histogram("database_query_duration", duration, labels={"type": query_type})
...
def record_error(error_type: str, error_message: str):
    ...
    collector.increment_counter("errors_by_message", labels={"message": error_message[:100]})  # Truncate long messages
