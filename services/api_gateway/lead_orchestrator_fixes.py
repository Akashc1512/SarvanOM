...
def create_safe_agent_result(
    ...
    return AgentResult(
        success=success, data=safe_data, error=error, confidence=confidence
    )


def safe_prepare_synthesis_input(
    results: Dict[AgentType, AgentResult], context: QueryContext
) -> Dict[str, Any]:
    """
...
def handle_agent_failure(
    ...
    if agent_type == AgentType.RETRIEVAL:
        return create_safe_agent_result(
            success=False,
            data={"documents": [], "search_performed": False, "fallback": True},
        ...
        if AgentType.FACT_CHECK in previous_results:
            facts = previous_results[AgentType.FACT_CHECK].data.get(
                "verified_facts", []
            )
        ...
        return create_safe_agent_result(
            success=False,
            data={"answer": answer, "response": answer, "fallback": True},
        ...
        answer = ""
        if synthesis_data:
            answer = synthesis_data.data.get(
                "answer", synthesis_data.data.get("response", "")
            )
...
def validate_agent_result(result: Any, agent_type: AgentType) -> AgentResult:
    ...
    if result is None:
        return create_safe_agent_result(
            success=False, error=f"{agent_type.value} returned None", confidence=0.0
        )

    # Handle exceptions
    if isinstance(result, Exception):
        return create_safe_agent_result(
            success=False, error=str(result), confidence=0.0
        )
    ...
    if not isinstance(result, AgentResult):
        ...
        return create_safe_agent_result(
            success=False, error=f"Invalid result type: {type(result)}", confidence=0.0
        )
...
def create_pipeline_error_response(
    context: QueryContext, error: str, partial_results: Dict[AgentType, Any]
) -> Dict[str, Any]:
    """
    ...
    if AgentType.RETRIEVAL in partial_results:
        retrieval_result = partial_results[AgentType.RETRIEVAL]
    ...
    if AgentType.FACT_CHECK in partial_results:
        ...
        if isinstance(fact_result, dict):
            facts = fact_result.get("data", {}).get("verified_facts", [])
            if facts:
    ...
    if AgentType.SYNTHESIS in partial_results:
        synthesis_result = partial_results[AgentType.SYNTHESIS]
        if isinstance(synthesis_result, dict):
            partial_answer = synthesis_result.get("data", {}).get("answer", "")
            if partial_answer:
