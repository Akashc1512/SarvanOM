...
class BaseAgentService(ABC):
    ...
    def increment_error_count(self) -> None:
        ...
        self.error_count += 1
        self.status = (
            ServiceStatus.DEGRADED if self.error_count > 0 else ServiceStatus.HEALTHY
        )
    ...
    def update_status(self, status: ServiceStatus) -> None:
        ...
        self.status = status
        logger.info(
            f"{self.service_type.value} service status updated to {status.value}"
        )
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        logger.info(
            f"Updated {self.service_type.value} service config: {key} = {value}"
        )
    ...
    async def reload_config(self, new_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not await self.validate_config():
                ...
                logger.error(
                    f"Failed to reload {self.service_type.value} service config: validation failed"
                )
                return False

            logger.info(
                f"Successfully reloaded {self.service_type.value} service config"
            )
            return True

        except Exception as e:
            logger.error(
                f"Failed to reload {self.service_type.value} service config: {e}"
            )
            return False
...
class ServiceHealthChecker:
    ...
    def check_service_health(service: BaseAgentService) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Health check failed for {service.service_type.value} service: {e}"
            )
            return {
                "healthy": False,
                "error": str(e),
...
class ServiceMetricsCollector:
    ...
    def collect_service_metrics(service: BaseAgentService) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Metrics collection failed for {service.service_type.value} service: {e}"
            )
            return {
                "error": str(e),
                "service_type": service.service_type.value,
