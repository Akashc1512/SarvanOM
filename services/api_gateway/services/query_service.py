...
class QueryService:
    ...
    async def process_basic_query(
        self, query: str, user_context: Dict[str, Any], cache_enabled: bool = True
    ) -> Dict[str, Any]:
    ...
    async def process_comprehensive_query(
        self, query: str, user_context: Dict[str, Any], options: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def _execute_query_pipeline(
        ...
        try:
            ...
            logger.error(
                f"Pipeline execution failed: {e}", extra={"query_id": context.query_id}
            )
            raise

    async def _execute_comprehensive_pipeline(
        self, query: str, context: QueryContext, options: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Comprehensive pipeline execution failed: {e}",
                extra={"query_id": context.query_id},
            )
            raise
    ...
    async def _execute_fact_checking(
        ...
        try:
            ...
            from services.fact_check.core.expert_validation import (
            ...
            for claim in claims:
                ...
                if claim.get("domain") in ["science", "medical", "technical"]:
                    verification_result = await expert_validation.verify_claim(
                        claim["text"], search_results.get("keyword_results", [])
                    )
                else:
            ...
            avg_confidence = (
                total_confidence / len(verified_claims) if verified_claims else 0.0
            )

            # Determine overall status
            verified_count = sum(
                1 for claim in verified_claims if claim["status"] == "verified"
            )
    ...
    async def _extract_claims(
        ...
        for pattern in claim_patterns:
            ...
            for match in matches:
                claims.append(
                    {"text": match.strip(), "source": "query", "domain": "general"}
                )
        ...
        for result in search_results.get("keyword_results", [])[:5]:
            ...
            if len(content) > 50:  # Only process substantial content
                ...
                factual_statements = re.findall(
                    r"[^.]*(?:is|are|was|were|has|have|had)[^.]*\.", content
                )
                for statement in factual_statements[
                    :3
                ]:  # Limit to 3 statements per result
                    claims.append(
                        {
        ...
        try:
            ...
            return {
                "answer": synthesis_result.get(
                    "answer", f"Based on verified sources: {query}"
                ),
        ...
        try:
            ...
            if analysis.get("domain") != "internal":
                retrieval_tasks.append(
                    self._retrieve_web_results(processed_query, analysis)
                )

            # Database search
            retrieval_tasks.append(
                self._retrieve_database_results(processed_query, analysis)
            )

            # Knowledge graph search
            retrieval_tasks.append(
                self._retrieve_knowledge_graph_results(processed_query, analysis)
            )

            # Document search
            retrieval_tasks.append(
                self._retrieve_document_results(processed_query, analysis)
            )

            # Vector search
            retrieval_tasks.append(
                self._retrieve_vector_results(processed_query, analysis)
            )
    ...
    async def _retrieve_web_results(
        self, query: str, analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _retrieve_database_results(
        self, query: str, analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _retrieve_knowledge_graph_results(
        self, query: str, analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _retrieve_document_results(
        self, query: str, analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _rank_results(
        self, results: List[Dict[str, Any]], query: str, analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _execute_advanced_verification(
        self, query: str, retrieval_results: Dict[str, Any], analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            all_claims = await self._extract_advanced_claims(
                query, retrieval_results, analysis
            )
            ...
            for claim in all_claims:
                ...
                if claim.get("domain") in ["science", "medical", "technical"]:
                    verification_result = await expert_validation.verify_claim_advanced(
                        claim["text"], retrieval_results, analysis
                    )
                else:
            ...
            return {
                ...
                "verification_metadata": {
                    ...
                    "unverified_count": len(verified_claims) - verified_count,
                    "contradiction_rate": (
                        contradiction_count / len(verified_claims)
                        if verified_claims
                        else 0
                    ),
        ...
        try:
            ...
            for source_type, results in retrieval_results.items():
                if isinstance(results, list):
                    for result in results[
                        :15
                    ]:  # More results for comprehensive synthesis
            ...
            return {
                "answer": synthesis_result.get(
                    "answer", f"Comprehensive analysis for: {query}"
                ),
                "confidence": avg_confidence,
                "sources": list(set(all_sources)),  # Remove duplicates
                "citations": citations,
    ...
    async def _assess_synthesis_quality(
        ...
        if verified_count > 0:
            ...
            quality_metrics["verification_coverage"] = verified_count / max(
                len(verified_content), 1
            )
        ...
        unique_sources = len(
            set([content.get("sources", []) for content in verified_content])
        )
        quality_metrics["source_diversity"] = min(
            unique_sources / max(len(verified_content), 1), 1.0
        )
    ...
    async def _cache_result(
        self, query: str, context: QueryContext, result: Dict[str, Any]
    ):
        """Cache query result."""
        cache_key = f"query:{hash(query)}:{context.session_id}"
