...
class PDFService(BaseAgentService):
        ...
        try:
            ...
            if test_result["success"]:
                ...
                return {
                    ...
                    and await self._check_ocr_availability_async(),
            ...
            return {"healthy": False, "pdf_processing": "FAILED", "error": str(e)}
        ...
        try:
            ...
            if self.ocr_enabled:
                ...
                if not ocr_available:
                    logger.warning(
                        "PDF service: OCR enabled but Tesseract not available"
                    )

            # Check temp directory
            if not os.path.exists(self.temp_dir):
                logger.error(
                    f"PDF service: Temp directory does not exist: {self.temp_dir}"
                )
                return False
    ...
    async def process_pdf(
        ...
        try:
            ...
            if not filename or not filename.strip():
                raise ValidationError(
                    field="filename", message="Filename is required", value=filename
                )
    ...
    async def extract_text(
        self, pdf_content: bytes, pages: Optional[List[int]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _extract_images_from_doc_sync(
        self, doc: fitz.Document, pages: Optional[List[int]] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _analyze_pdf_structure_sync(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if text_stats["pages_with_text"] > 0:
                text_stats["average_words_per_page"] = (
                    text_stats["total_words"] / text_stats["pages_with_text"]
                )
            ...
            if image_stats["pages_with_images"] > 0:
                image_stats["average_images_per_page"] = (
                    image_stats["total_images"] / image_stats["pages_with_images"]
                )
    ...
    async def _test_pdf_processing(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                return {"success": True, "message": "PDF processing test successful"}
            else:
    ...
    async def _check_ocr_availability_async(self) -> bool:
        ...
        try:
            ...
            return await loop.run_in_executor(
                self.executor, self._check_ocr_availability_sync
            )
        except Exception as e:
