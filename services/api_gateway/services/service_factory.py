...
class ServiceFactory:
    ...
    def register_service_class(
        self, service_type: ServiceType, service_class: Type[BaseAgentService]
    ) -> None:
        """
    ...
    def create_service(
        ...
        if service_type.value not in self._service_classes:
            raise ValueError(
                f"Service class not registered for type: {service_type.value}"
            )
    ...
    def shutdown_all_services(self) -> None:
        ...
        for service in self._services.values():
            try:
                ...
                logger.error(
                    f"Error shutting down service {service.service_type.value}: {e}"
                )
    ...
    def reload_service_config(
        self, service_type: ServiceType, new_config: Dict[str, Any]
    ) -> bool:
        """
        ...
        if service is None:
            logger.warning(
                f"Cannot reload config for non-existent service: {service_type.value}"
            )
            return False
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully reloaded config for service: {service_type.value}"
                )
            else:
                logger.error(
                    f"Failed to reload config for service: {service_type.value}"
                )
            return success

        except Exception as e:
            logger.error(
                f"Error reloading config for service {service_type.value}: {e}"
            )
            return False
    ...
    def get_service_metrics(self, service_type: ServiceType) -> Dict[str, Any]:
        ...
        service = self.get_service(service_type)
        if service is None:
            return {"service_type": service_type.value, "error": "Service not found"}

        try:
            return service.get_metrics()
        except Exception as e:
            return {"service_type": service_type.value, "error": str(e)}
