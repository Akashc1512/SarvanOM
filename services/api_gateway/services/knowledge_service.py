...
class KnowledgeService(BaseAgentService):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": False, "database_connection": "FAILED", "error": str(e)}
    ...
    async def query_entities(
        ...
        try:
            ...
            query = self._build_entity_query(
                entity_type, properties, limit or self.max_results
            )
    ...
    async def find_paths(
        ...
        try:
            cache_key = (
                f"paths:{source_entity}:{target_entity}:{max_paths}:{max_length}"
            )
            ...
            query = self._build_path_query(
                source_entity, target_entity, max_paths, max_length
            )
            result = await self._execute_query(query)
    ...
    async def analyze_subgraph(
        self, entities: List[str], include_neighbors: bool = True, max_depth: int = 2
    ) -> Dict[str, Any]:
        """
    ...
    async def search_entities(
        ...
        try:
            ...
            query = self._build_search_query(
                search_term, entity_types, limit or self.max_results
            )
            result = await self._execute_query(query)
    ...
    def _build_path_query(
        self, source_entity: str, target_entity: str, max_paths: int, max_length: int
    ) -> str:
    ...
    def _build_subgraph_query(
        self, entities: List[str], include_neighbors: bool, max_depth: int
    ) -> str:
    ...
    def _build_search_query(
        self, search_term: str, entity_types: Optional[List[str]], limit: int
    ) -> str:
        """Build search query."""
        query_parts = ["FOR entity IN entities"]
        ...
        # Add search filter
        query_parts.append(
            f"FILTER CONTAINS(LOWER(entity.name), LOWER('{search_term}'))"
        )
