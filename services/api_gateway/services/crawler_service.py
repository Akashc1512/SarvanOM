...
class CrawlerService(BaseAgentService):
    ...
    async def crawl_website(self, start_url: str, max_depth: Optional[int] = None,
    ...
    async def extract_content(self, url: str, extract_images: Optional[bool] = None,
                            extract_links: Optional[bool] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            extract_links = extract_links if extract_links is not None else self.extract_links
            
            # Extract content
            result = await self._extract_page_content(url, extract_images, extract_links)
            await self.post_request(success=True)
            return result
    ...
    async def discover_links(self, url: str, max_links: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    async def generate_sitemap(self, start_url: str, max_pages: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            crawl_result = await self.crawl_website(start_url, max_depth=2, max_pages=max_pages or self.max_pages)
            
            # Generate sitemap
            sitemap = await self._generate_sitemap_xml(crawl_result["pages_crawled"], start_url)
    ...
    async def crawl_with_filters(self, start_url: str, include_patterns: Optional[List[str]] = None,
    ...
    async def _crawl_page(self, url: str, depth: int) -> Dict[str, Any]:
        ...
        try:
            # Extract content
            content_result = await self._extract_page_content(url, self.extract_images, self.extract_links)
    ...
    async def _extract_page_content(self, url: str, extract_images: bool, extract_links: bool) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Fetch page
            async with self.session.get(url, allow_redirects=self.follow_redirects) as response:
    ...
    async def _discover_page_links(self, url: str, max_links: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            content_result = await self._extract_page_content(url, extract_images=False, extract_links=True)
    ...
    async def _generate_sitemap_xml(self, pages: List[Dict[str, Any]], base_url: str) -> str:
        """
