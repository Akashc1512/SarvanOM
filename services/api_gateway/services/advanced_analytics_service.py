...
class AdvancedAnalyticsService:
    ...
    async def collect_analytics_data(
        ...
        try:
            ...
            logger.debug(f"Collected analytics data: {metric_name}={value} for tenant {tenant_id}")
            return True
    ...
    async def get_usage_analytics(
        ...
        try:
            ...
            filtered_data = [
                d for d in self.analytics_data[tenant_id]
    ...
    async def get_predictive_analytics(
        ...
        try:
            ...
            metric_data = [
                dp for dp in self.analytics_data[tenant_id]
    ...
    async def _generate_bar_chart_data(
        ...
        return {
            ...
            "data": {
                ...
                "y": [
                    metric_data["data_points"][-1]["value"] if metric_data["data_points"] else 0,
                    metric_data["mean"],
                    metric_data["max"]
                ],
                "metric": metric
    ...
    async def _generate_gauge_chart_data(
        ...
        for metric in metrics:
            ...
            if "error" not in analytics_data and metric in analytics_data["metrics"]:
                current_value = analytics_data["metrics"][metric]["data_points"][-1]["value"] if analytics_data["metrics"][metric]["data_points"] else 0
                gauge_data.append({
                    "metric": metric,
                    "value": current_value,
