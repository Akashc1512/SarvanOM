...
class MultiTenantService:
    ...
    async def track_api_call(self, tenant_id: str) -> bool:
        ...
        # Check limits
        if (
            tenant.config.max_api_calls_per_month != -1
            and tenant.usage.api_calls_this_month
            >= tenant.config.max_api_calls_per_month
        ):
    ...
    async def track_storage_usage(self, tenant_id: str, storage_gb: float) -> bool:
        ...
        # Check limits
        if (
            tenant.config.max_storage_gb != -1
            and tenant.usage.storage_used_gb + storage_gb > tenant.config.max_storage_gb
        ):
    ...
    async def track_user_activity(self, tenant_id: str, user_id: str) -> bool:
        ...
        # Check user limits
        if (
            tenant.config.max_users != -1
            and tenant.usage.active_users >= tenant.config.max_users
        ):
    ...
    async def get_tenant_stats(self) -> Dict[str, Any]:
        ...
        total_api_calls = sum(
            t.usage.api_calls_this_month for t in self.tenants.values()
        )
    ...
    async def is_feature_enabled(self, tenant_id: str, feature: str) -> bool:
        ...
        return (
            feature in tenant.config.features_enabled
            or "all" in tenant.config.features_enabled
        )
