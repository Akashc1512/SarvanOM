...
class HealthService:
    ...
    async def get_basic_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            is_healthy = (
                memory.percent < 90  # Memory usage < 90%
                and cpu_percent < 95  # CPU usage < 95%
        ...
        for service_name, check_func in services.items():
            try:
                ...
                health_results[service_name] = ServiceHealth(
                    ...
                    error_count=self.service_health.get(service_name, {}).get(
                        "error_count", 0
                    ),
                    details=details,
                )
        ...
        try:
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    if result != 0:
                        return "unhealthy", {
                            "error": "API Gateway not listening on port 8000"
                        }
                ...
                return "unhealthy", {
                    "error": f"API Gateway health check failed: {str(e)}"
                }
    ...
    async def _check_database(self) -> tuple[str, Dict[str, Any]]:
        """Check database health using DATABASE_URL or environment manager config."""
        try:
            import asyncpg
            import os
            ...
            # Prefer central environment manager / DATABASE_URL
            try:
            ...
            if not database_url:
                return "unhealthy", {"error": "DATABASE_URL not configured"}

            parsed = urlparse(database_url)
            scheme = (parsed.scheme or "").lower()

            if scheme.startswith("sqlite"):
                # Skip asyncpg check for SQLite; treat as healthy in dev
                return "healthy", {"database": "sqlite", "note": "SQLite check skipped"}

            # Extract connection params for postgres-like URLs
            ...
            try:
                ...
                return "unhealthy", {
                    "error": f"Cannot connect to database at {db_host}:{db_port}"
                }
            except Exception as e:
    ...
    async def _check_cache(self) -> tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                return "unhealthy", {
                    "error": f"Cannot connect to Redis at {redis_host}:{redis_port}"
                }
    ...
    async def _check_search_service(self) -> tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(
                        ...
                        if response.status != 200:
                            return "unhealthy", {
                                "error": f"Meilisearch health check failed with status {response.status}"
                            }
                        ...
                        async with session.get(
                            f"{base_url}/indexes", headers=headers, timeout=5
                        ) as index_response:
                            if index_response.status == 200:
                                ...
                                for index in indexes_data.get("results", []):
                                    ...
                                    if index_name:
                                        async with session.get(
                                            f"{base_url}/indexes/{index_name}/stats",
                                            headers=headers,
                                            timeout=5,
                                        ) as stats_response:
                                            if stats_response.status == 200:
                                                stats_data = await stats_response.json()
                                                total_documents += stats_data.get(
                                                    "numberOfDocuments", 0
                                                )
                            else:
                                index_count = 0
                ...
                return "unhealthy", {
                    "error": f"Cannot connect to Meilisearch at {meili_host}:{meili_port}"
                }
            except aiohttp.ClientTimeout:
                return "unhealthy", {"error": "Meilisearch health check timeout"}
            except Exception as e:
                return "unhealthy", {
                    "error": f"Meilisearch health check failed: {str(e)}"
                }
            ...
            return "unhealthy", {
                "error": f"Search service health check error: {str(e)}"
            }
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(
                        ...
                        if response.status != 200:
                            return "unhealthy", {
                                "error": f"Ollama health check failed with status {response.status}"
                            }
                        ...
                        try:
                            async with session.post(
                                f"{base_url}/api/generate", json=test_data, timeout=15
                            ) as test_response:
                ...
                return "unhealthy", {
                    "error": f"Cannot connect to Ollama at {ollama_host}:{ollama_port}"
                }
            except aiohttp.ClientTimeout:
    ...
    def _determine_overall_health(
        ...
        unhealthy_services = [
            s for s in service_health.values() if s.status == "unhealthy"
        ]
        if (
            len(unhealthy_services) > len(service_health) * 0.5
    ...
    def _check_critical_services(self) -> bool:
        ...
        try:
            ...
            for service_name, port in critical_services.items():
                try:
                    ...
                    if result == 0:
                        ...
                        logger.debug(
                            f"Critical service {service_name} is available on port {port}"
                        )
                    else:
                        logger.warning(
                            f"Critical service {service_name} is not available on port {port}"
                        )
            ...
            logger.info(
                f"Critical services availability: {available_services}/{total_services} ({availability_percentage:.1f}%)"
            )
    ...
    async def _get_recent_errors(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            log_patterns = ["ERROR", "CRITICAL", "FATAL", "Exception", "Traceback"]
