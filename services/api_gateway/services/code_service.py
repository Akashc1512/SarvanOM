...
class CodeService(BaseAgentService):
    ...
    def __init__(
        ...
        self.allowed_languages = self.get_config(
        ...
        self.blocked_imports = self.get_config(
            "blocked_imports",
            ["os", "sys", "subprocess", "socket", "urllib", "requests"],
        )
        self.blocked_functions = self.get_config(
            "blocked_functions", ["eval", "exec", "compile", "input", "open"]
        )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"healthy": False, "code_execution": "FAILED", "error": str(e)}
    ...
    async def validate_config(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(self.temp_dir) or not os.access(
                self.temp_dir, os.W_OK
            ):
                logger.error(
                    f"Code service: Temp directory not accessible: {self.temp_dir}"
                )
                return False
    ...
    async def execute_code(
        ...
        try:
            ...
            if not security_result["safe"]:
                raise ValueError(
                    f"Code failed security scan: {security_result['reason']}"
                )
    ...
    async def _execute_python(
        self, code: str, inputs: Optional[Dict[str, Any]], timeout: int
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".py", delete=False, dir=self.temp_dir
            ) as temp_file:
                # Add input variables
                if inputs:
                    input_code = "\n".join(
    ...
    async def _execute_javascript(
        ...
        try:
            # Create temporary file
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".js", delete=False, dir=self.temp_dir
            ) as temp_file:
                # Add input variables
                if inputs:
                    input_code = "\n".join(
                        [f"const {k} = {json.dumps(v)};" for k, v in inputs.items()]
                    )
                    temp_file.write(input_code + "\n\n")
    ...
    async def _execute_bash(
        ...
        try:
            # Create temporary file
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".sh", delete=False, dir=self.temp_dir
            ) as temp_file:
    ...
    async def _security_scan(self, code: str, language: str) -> Dict[str, Any]:
        ...
        try:
            if language == "python":
                ...
                return {
                    "safe": True,
                    "reason": "Language not supported for security scanning",
                }
        ...
        try:
            ...
            for pattern in dangerous_patterns:
                if re.search(pattern, code, re.IGNORECASE):
                    return {
                        "safe": False,
                        "reason": f"Dangerous command found: {pattern}",
                    }
        ...
        try:
            if language == "python":
                ...
                return {
                    "valid": False,
                    "errors": [
                        f"Language '{language}' not supported for syntax validation"
                    ],
                }
