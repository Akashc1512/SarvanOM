...
websocket_connections = Gauge(
    "websocket_connections_total", "Active WebSocket connections", ["connection_type"]
)

websocket_messages = Counter(
    "websocket_messages_total", "WebSocket messages", ["message_type", "direction"]
)
...
class ConnectionManager:
    ...
    async def send_message(self, connection_id: str, message: RealtimeMessage) -> bool:
        ...
        try:
            ...
            websocket_messages.labels(
                message_type=message.message_type.value, direction="outgoing"
            ).inc()
    ...
    async def broadcast_message(
        self, message: RealtimeMessage, user_ids: Optional[List[str]] = None
    ) -> int:
        ...
        while self._running:
            try:
                ...
                stream_processing_time.labels(
                    stream_type=stream_name, operation="process"
...
class CollaborationManager:
    ...
    async def join_session(
        self, session_id: str, user_id: str, connection_id: str
    ) -> None:
        ...
        logger.info(
            "User joined collaboration session", session_id=session_id, user_id=user_id
        )
    ...
    async def update_document(
        self, session_id: str, user_id: str, changes: List[Dict[str, Any]]
    ) -> None:
        ...
        for change in changes:
            if change["type"] == "insert":
                ...
                session["document"] = (
                    session["document"][:start] + session["document"][end:]
                )
    ...
    async def update_cursor(self, session_id: str, user_id: str, position: int) -> None:
...
class NotificationManager:
    ...
    def __init__(self, connection_manager: ConnectionManager):
        ...
        self.notification_preferences: Dict[str, Dict[str, bool]] = defaultdict(
            lambda: {"email": True, "push": True, "in_app": True}
        )
    ...
    async def send_notification(
        ...
        realtime_message = RealtimeMessage(
            message_type=MessageType.NOTIFICATION, payload=notification, user_id=user_id
        )
    ...
    async def mark_as_read(self, user_id: str, notification_id: str) -> None:
        ...
        message = RealtimeMessage(
            ...
            payload={"action": "mark_read", "notification_id": notification_id},
            user_id=user_id,
        )
