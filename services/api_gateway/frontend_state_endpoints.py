...
def get_frontend_state_service(
...
async def get_state(
    session_id: str, service: FrontendStateService = Depends(get_frontend_state_service)
) -> Dict[str, Any]:
    """
...
async def get_session_info(
    ...
    try:
        ...
        logger.info(
            f"API: Successfully retrieved session info for session: {session_id}"
        )
        return {
            "success": True,
            "data": session_info,
...
async def get_states_by_user(
    user_id: str, service: FrontendStateService = Depends(get_frontend_state_service)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"API: Successfully retrieved {len(states)} states for user: {user_id}"
        )
        return {
            "success": True,
            "data": states,
...
async def get_state_value(
    ...
    try:
        ...
        logger.info(
            f"API: Successfully retrieved state value for session {session_id}, key {key}"
        )
        return {
            "success": True,
            "data": {"session_id": session_id, "key": key, "value": value},
            "message": "State value retrieved successfully",
        }

    except Exception as e:
        logger.error(
            f"API: Error getting state value for session {session_id}, key {key}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Internal server error: {str(e)}",
...
async def set_state_value(
    ...
    try:
        ...
        logger.info(
            f"API: Successfully set state value for session {session_id}, key {key}"
        )
        return {
            "success": True,
            "data": updated_state,
        ...
        logger.error(
            f"API: Error setting state value for session {session_id}, key {key}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Internal server error: {str(e)}",
