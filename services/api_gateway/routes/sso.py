...
async def get_oauth_url(provider: str, redirect_uri: str):
    ...
    try:
        ...
        return OAuthUrlResponse(
            provider=provider, authorization_url=oauth_url, redirect_uri=redirect_uri
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def change_password(
    request: PasswordChangeRequest, current_user=Depends(get_current_user)
):
    """
    ...
    try:
        ...
        if not sso_service.verify_password(
        ...
        current_user.hashed_password = sso_service.get_password_hash(
