...
async def extract_content(
    ...
    try:
        ...
        result = await crawler_service.extract_content(
            url=url, extract_images=extract_images, extract_links=extract_links
        )

        processing_time = tracker.get_processing_time()
        metadata = create_agent_metadata(
            user_id, url=url, extract_images=extract_images, extract_links=extract_links
        )
...
async def discover_links(
    ...
    try:
        ...
        result = await crawler_service.discover_links(url=url, max_links=max_links)

        processing_time = tracker.get_processing_time()
        metadata = create_agent_metadata(user_id, url=url, max_links=max_links)
...
async def generate_sitemap(
    ...
    try:
        ...
        processing_time = tracker.get_processing_time()
        metadata = create_agent_metadata(user_id, url=url, max_pages=max_pages)
...
async def crawler_health(
    ...
    try:
        ...
        return AgentResponseFormatter.format_success(
            ...
            processing_time=0.0,
            metadata=create_agent_metadata(
                get_user_id(current_user), health_check=True
            ),
            user_id=get_user_id(current_user),
        )
...
async def crawler_status(
    ...
    try:
        ...
        return AgentResponseFormatter.format_success(
            ...
            processing_time=0.0,
            metadata=create_agent_metadata(
                get_user_id(current_user), status_check=True
            ),
            user_id=get_user_id(current_user),
        )
