...
# Ensure .env is loaded so integration checks (Meilisearch, etc.) see env vars
load_dotenv()
...
async def health_check():
    ...
    try:
        ...
        response_data = {
            ...
            "active_connections": health_data.get("system_metrics", {}).get(
...
async def get_metrics(
    admin: bool = False, format: str = "json", current_user=Depends(get_current_user)
):
    ...
    try:
        ...
        response_data = {
            ...
            "resources": {
                "memory_usage_percent": metrics_data.get("current_metrics", {})
                .get("memory", {})
                .get("percent", 0),
                "cpu_usage_percent": metrics_data.get("current_metrics", {})
                .get("cpu", {})
                .get("usage_percent", 0),
                "disk_usage_percent": metrics_data.get("current_metrics", {})
                .get("disk", {})
                .get("percent", 0),
                "active_processes": metrics_data.get("current_metrics", {})
                .get("network", {})
                .get("connections", 0),
            },
        }
    ...
    try:
        ...
        try:
            ...
            total_queries = await conn.fetchval(
                "SELECT COUNT(*) FROM queries WHERE created_at >= $1", start_time
            )
        ...
        analytics_data = {
            ...
            "user_activity": {"active_users": 12, "new_users": 3, "returning_users": 9},
        }
    ...
    try:
        ...
        try:
            ...
            if suspicious_activity > 0:
                ...
                threats_detected += 1
                vulnerabilities.append(
                    f"Suspicious user activity detected: {suspicious_activity} users"
                )
            ...
            if cpu_usage > 90:
                ...
                recommendations.append(
                    "Monitor system resources and investigate high CPU usage"
                )
            ...
            raise HTTPException(
                status_code=500, detail="Failed to retrieve security data"
            )
    ...
    try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        ...
                        if response.status in [
                            200,
                            401,
                        ]:  # 401 means auth works but no message
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    f"{meili_url}/health", headers=headers, timeout=5
                ) as response:
                    if response.status == 200:
        ...
        connected_services = sum(
            1
            for service in integration_data["integrations"].values()
        ...
        integration_data["health_checks"] = {
            ...
            "database_connectivity": integration_data["integrations"]
            .get("postgresql", {})
            .get("status", "unknown"),
            "search_service": integration_data["integrations"]
            .get("meilisearch", {})
            .get("status", "unknown"),
            "llm_services": (
                "healthy"
                if any(
                    ...
                    for service in [
                        integration_data["integrations"].get("openai", {}),
                        integration_data["integrations"].get("anthropic", {}),
                    ]
                )
                else "unhealthy"
            ),
            "cache_service": integration_data["integrations"]
            .get("redis", {})
            .get("status", "unknown"),
            "graph_database": integration_data["integrations"]
            .get("arangodb", {})
            .get("status", "unknown"),
        }
