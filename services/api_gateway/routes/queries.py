...
from shared.core.utilities.timing_utilities import (
    time_operation,
    start_timer,
    calculate_execution_time,
)
from shared.core.utilities.response_utilities import (
...
async def process_query(
    ...
    try:
        ...
        user_id = (
            getattr(current_user, "user_id", "anonymous")
            if current_user
            else "anonymous"
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                # Retrieval
                try:
                    retrieval_url = f"{str(cfg.search_service_url).rstrip('/')}/search"
                    retrieval_payload = RetrievalSearchRequest(
                        query=query, max_results=10, context={"user_id": user_id}
                    ).dict()
                    retrieval_resp = await client.post(
                        str(retrieval_url), json=retrieval_payload
                    )
                    ...
                    logger.error(
                        f"Retrieval call timeout: {timeout_error}", exc_info=True
                    )
                ...
                try:
                    synthesis_url = (
                        f"{str(cfg.synthesis_service_url).rstrip('/')}/synthesize"
                    )
                    ...
                    synth_resp = await client.post(
                        str(synthesis_url), json=synthesis_payload
                    )
                    synth_resp.raise_for_status()
                    result = synth_resp.json()
                    # Short-circuit: if we have a valid synthesis payload, return success now
                    if (
                        ...
                        response = create_success_response(
                            data={
                                ...
                                "validation_status": result.get("verification", {}).get(
                                    "overall_status", "Unverified"
                                ),
                                "llm_provider": result.get("metadata", {}).get(
                                    "llm_provider", "Unknown"
                                ),
                                "cache_status": result.get("cache_status", "Miss"),
                                "agent_results": {
                                    "retrieval": search_data,
                                    "synthesis": {
                                        "answer": result.get("answer", ""),
        ...
        response_data = {
            ...
            "agent_results": {
                "retrieval": {
                    "vector_results": result.get("metadata", {}).get(
                        "vector_results", []
                    ),
                    "keyword_results": result.get("metadata", {}).get(
                        "keyword_results", []
                    ),
                    "knowledge_graph_results": result.get("metadata", {}).get(
                ...
                "citation": {"sources": result.get("sources", [])},
            },
...
async def process_comprehensive_query(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Comprehensive query service error: {service_error}", exc_info=True
            )
            execution_time = calculate_execution_time(start_time)
            return create_error_response(
            ...
            api_response.update(
                {
                    ...
                    "llm_model": result.get("metadata", {}).get(
                        "model_used", "Unknown"
                    ),
                    "model_used": result.get("metadata", {}).get(
                        "model_used", "Unknown"
                    ),
...
async def get_query(
    ...
    try:
        ...
        try:
            ...
            try:
                row = await asyncio.wait_for(
                    conn.fetchrow(
                        query, query_id, current_user.get("user_id", "anonymous")
                    ),
                    timeout=15.0,  # 15 second timeout for query execution
                )
            except asyncio.TimeoutError:
            ...
            query_detail = {
                ...
                "metadata": metadata,
                "processing_time": (
                    float(row["processing_time"]) if row["processing_time"] else None
                ),
        ...
        mock_query = {
            ...
            "status": "completed",
            "result": {"answer": "Sample answer", "sources": ["source1", "source2"]},
            "metadata": {"processing_time": 1.5, "model_used": "gpt-4o-mini"},
            "processing_time": 1.5,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get query {query_id}")
    ...
    try:
        ...
        try:
            ...
            existing_query = await conn.fetchrow(
                check_query, query_id, current_user.get("user_id", "anonymous")
            )
            ...
            if updated_row["result"]:
                try:
                    result_data = (
                        updated_row["result"]
                        if isinstance(updated_row["result"], dict)
                        else {}
                    )
                except:
                    result_data = {}
            ...
            if updated_row["metadata"]:
                try:
                    metadata = (
                        ...
                        if isinstance(updated_row["metadata"], dict)
                        else {}
                    )
                except:
                    metadata = {}
            ...
            if updated_row["tags"]:
                try:
                    tags = (
                        updated_row["tags"]
                        if isinstance(updated_row["tags"], list)
                        else []
                    )
                except:
                    tags = []
            ...
            if updated_row["sources"]:
                try:
                    sources = (
                        updated_row["sources"]
                        if isinstance(updated_row["sources"], list)
                        else []
                    )
                except:
                    sources = []
            ...
            updated_query = {
                ...
                "metadata": metadata,
                "processing_time": (
                    float(updated_row["processing_time"])
                    if updated_row["processing_time"]
                    else None
                ),
                ...
                "confidence_score": (
                    float(updated_row["confidence_score"])
                    if updated_row["confidence_score"]
                    else None
                ),
                "sources": sources,
            }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update query {query_id}"
        )
    ...
    try:
        ...
        try:
            ...
            deleted_query = await conn.fetchrow(
                delete_query, query_id, current_user.get("user_id", "anonymous")
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete query {query_id}"
        )
    ...
    try:
        ...
        try:
            ...
            row = await conn.fetchrow(
                status_query, query_id, current_user.get("user_id", "anonymous")
            )
            ...
            if row["status"] == "completed":
                ...
                if row["processing_time"]:
                    estimated_completion = (
                        row["created_at"]
                        + timedelta(seconds=row["processing_time"] * 2)
                    ).isoformat()
                else:
                    estimated_completion = (
            ...
            raise HTTPException(
                status_code=500, detail="Failed to retrieve query status"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get status for query {query_id}"
        )
...
async def reprocess_query(
    ...
    try:
        ...
        try:
            ...
            row = await conn.fetchrow(
                query_details, query_id, current_user.get("user_id", "anonymous")
            )
            ...
            if not updated_row:
                raise HTTPException(
                    status_code=500, detail="Failed to update query status"
                )
            ...
            try:
                ...
                status = (
                    "completed" if reprocess_result.get("success", False) else "failed"
                )
                ...
                updated_metadata = {
                    **original_metadata,
                    ...
                    "original_processing_time": original_metadata.get(
                        "processing_time", 0
                    ),
                }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to reprocess query {query_id}"
        )
