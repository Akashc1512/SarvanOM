...
class ExpertReviewRequest(BaseModel):
    ...
    verdict: str = Field(
        ..., description="Review verdict: supported, contradicted, or unclear"
    )
...
class KnowledgeGraphRequest(AgentRequest):
    ...
    query_type: str = Field(
        default="entity_relationship", description="Type of graph query"
    )
...
class TenantConfigRequest(BaseModel):
    ...
    storage_limit_gb: float = Field(
        default=1.0, ge=0.1, description="Storage limit in GB"
    )
...
class DashboardConfigRequest(BaseModel):
    ...
    refresh_interval: int = Field(
        default=300, ge=30, description="Refresh interval in seconds"
    )
...
class WidgetConfigRequest(BaseModel):
    ...
    position: Dict[str, int] = Field(
        default_factory=dict, description="Widget position"
    )
