...
class TestOrchestratorReviewer:
    ...
    async def test_orchestrator_with_reviewer_enabled(
        self, orchestrator, mock_synthesis_result, mock_reviewer_response
    ):
        """Test that orchestrator calls reviewer when enabled."""
        with patch(
            "shared.core.config.central_config.get_central_config"
        ) as mock_config:
            mock_config.return_value.enable_reviewer_agent = True

            with patch.object(
                orchestrator.agents["REVIEWER"], "process_task"
            ) as mock_reviewer:
    ...
    async def test_orchestrator_with_reviewer_disabled(
        self, orchestrator, mock_synthesis_result
    ):
    ...
    async def test_orchestrator_reviewer_timeout(
        self, orchestrator, mock_synthesis_result
    ):
    ...
    async def test_orchestrator_reviewer_with_sources(
        self, orchestrator, mock_synthesis_result, mock_reviewer_response
    ):
        ...
        with patch(
            ...
            with patch.object(
                ...
                retrieval_result = AgentResult(
                    success=True,
                    data={
                        "documents": [{"title": "Test", "content": "Paris is capital"}]
                    },
                    confidence=0.8,
                )
                results = {
    ...
    async def test_orchestrator_reviewer_improves_answer(
        self, orchestrator, mock_synthesis_result
    ):
        ...
        with patch(
            ...
            with patch.object(
                ...
                synth_result = updated_results[AgentType.SYNTHESIS]
                assert (
                    "Paris, located in northern France" in synth_result.data["answer"]
                )
