...
from shared.core.database.models import UserSession
from shared.core.security import (
    create_csrf_token,
    verify_csrf_token,
    generate_device_fingerprint,
)
from shared.core.cache import cache_response, invalidate_cache
from shared.core.api.monitoring import track_endpoint_usage
from shared.core.rate_limiter import RateLimitConfig, rate_limit
...
AUTH_RATE_LIMIT = RateLimitConfig(
    requests_per_minute=10, burst_size=5, lockout_duration=300  # 5 minutes
)

PASSWORD_RESET_RATE_LIMIT = RateLimitConfig(
    requests_per_minute=3, burst_size=1, lockout_duration=3600  # 1 hour
)
...
async def login(
    ...
    **Response includes**:
    ...
    try:
        ...
        # Create session
        session_duration = (
            timedelta(days=30) if login_data.remember_me else timedelta(minutes=30)
        )
        ...
        logger.warning(
            "Login failed", username=login_data.username, ip=request.client.host
        )

        raise AuthenticationError("Invalid username or password")

    except AccountLockedException as e:
        logger.warning("Login attempt on locked account", username=login_data.username)
...
async def register(
    ...
    **Rate limit**: 10 requests per minute
    ...
    try:
        ...
        background_tasks.add_task(user_service.send_verification_email, user)

        # Send welcome email
        if register_data.newsletter:
            background_tasks.add_task(user_service.send_welcome_email, user)
        ...
        logger.info(
            "User registered", user_id=user.id, username=user.username, email=user.email
        )
...
async def refresh_token(
    ...
    **Security**: Refresh tokens are single-use and rotated on each refresh
    ...
    try:
        # Refresh tokens
        result = await user_service.refresh_access_token(refresh_request.refresh_token)
    ...
    **Backup codes**: 10 single-use codes for account recovery
    ...
    try:
        ...
        totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=current_user.email, issuer_name="Universal Knowledge Platform"
        )
        ...
        await user_service.store_2fa_setup(current_user.id, secret, backup_codes)

        logger.info("2FA setup initiated", user_id=current_user.id)

        return TwoFactorSetupResponse(
            secret=secret, qr_code=qr_code_url, backup_codes=backup_codes
        )
    ...
    try:
        ...
        if verify_request.trust_device:
            await user_service.trust_device(
                current_user.id, device_fingerprint=generate_device_fingerprint(request)
            )

        logger.info("2FA verified", user_id=current_user.id, setup=setup)
    ...
    **Security**: Always returns success to prevent email enumeration
    ...
    try:
        ...
        if user:
            ...
            # Send reset email
            background_tasks.add_task(
                user_service.send_password_reset_email, user, token
            )
            ...
            logger.warning(
                "Password reset for non-existent email", email=reset_request.email
            )
...
async def confirm_password_reset(
    ...
    **Post-reset**:
    ...
    try:
        ...
        background_tasks.add_task(user_service.send_password_reset_confirmation, user)
