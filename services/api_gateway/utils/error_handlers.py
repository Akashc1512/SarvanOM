        ...
        **kwargs
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"LLM call error on attempt {attempt + 1}/{max_retries}: {e}")
                if attempt == max_retries - 1:
        ...
        **kwargs
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"Database {operation_name} timeout on attempt {attempt + 1}/{max_retries}")
                if attempt == max_retries - 1:
                ...
                logger.error(f"Database {operation_name} error on attempt {attempt + 1}/{max_retries}: {e}")
                if attempt == max_retries - 1:
                    logger.error(f"Database {operation_name} failed after all retries", exc_info=True)
                    raise HTTPException(
                        status_code=500,
        ...
        **kwargs
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(f"External API call timeout on attempt {attempt + 1}/{max_retries}")
                if attempt == max_retries - 1:
                ...
                logger.error(f"External API call error on attempt {attempt + 1}/{max_retries}: {e}")
                if attempt == max_retries - 1:
                    logger.error("External API call failed after all retries", exc_info=True)
                    if fallback_data:
                        return fallback_data
                    raise HTTPException(
                        status_code=503,
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        async def wrapper(*args, **kwargs) -> T:
            try:
                ...
                logger.error(f"{operation_type} operation error in {func.__name__}: {e}", exc_info=True)
                if fallback_data:
                    return fallback_data
                raise HTTPException(
                    status_code=503,
