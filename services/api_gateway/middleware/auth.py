...
class SecurityMiddleware:
    ...
    async def check_security(
        self, query: str, client_ip: str, user_id: str, initial_confidence: float = 0.0
    ) -> Dict[str, Any]:
        """
        ...
        if rate_limit_key in self.rate_limits:
            ...
            if (
                current_time - last_request_time < 1.0
            ):  # 1 second minimum between requests
                threats.append({"type": "rate_limit", "severity": "low"})
                confidence -= 0.1
    ...
    async def _validate_token(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                # Get JWT secret from environment (prefer JWT_SECRET_KEY)
                jwt_secret = os.getenv("JWT_SECRET_KEY") or os.getenv(
                ...
                user = {
                    "user_id": payload.get(
                        "user_id", payload.get("sub", f"user_{hash(token) % 10000}")
                    ),
                    "username": payload.get(
                        "username", payload.get("name", f"user_{hash(token) % 10000}")
                    ),
                    ...
                    async with aiohttp.ClientSession() as session:
                        ...
                        async with session.get(
                            ...
                            if response.status == 200:
                                ...
                                user = {
                                    "user_id": str(
                                        user_info.get("id", hash(token) % 10000)
                                    ),
                                    "username": user_info.get(
                                        "login", f"user_{hash(token) % 10000}"
                                    ),
                                    "role": "user",
                                    "permissions": ["read", "write"],
...
async def security_check(request: Request, call_next):
    ...
    if any(
        ...
        for path in security_middleware.security_config["bypass_paths"]
    ):
