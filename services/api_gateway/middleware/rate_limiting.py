...
class RateLimitingMiddleware:
    ...
    async def check_rate_limit(
        self, request: Request, user_id: Optional[str] = None, user_type: str = "free"
    ) -> Dict[str, Any]:
        """
        ...
        if not ip_allowed:
            logger.warning(
                ...
                extra={"ip": request.client.host, "path": path, "user_id": user_id},
            )
            raise HTTPException(
                status_code=429,
        ...
        if user_id and user_type in self.user_limiters:
            ...
            if not user_allowed:
                logger.warning(
                    ...
                    extra={"user_id": user_id, "user_type": user_type, "path": path},
                )
                raise HTTPException(
                    status_code=429,
        ...
        if endpoint_limit:
            ...
            if endpoint_key not in self.endpoint_limiters:
                self.endpoint_limiters[endpoint_key] = RateLimiter(
                    window_size=60, max_requests=endpoint_limit
                )

            endpoint_allowed, endpoint_info = self.endpoint_limiters[
                endpoint_key
            ].is_allowed(endpoint_key)

            if not endpoint_allowed:
                logger.warning(
                    f"Endpoint rate limit exceeded: {path}",
                    extra={"path": path, "user_id": user_id, "ip": request.client.host},
                )
                raise HTTPException(
                    status_code=429,
...
async def rate_limit_check(request: Request, call_next):
    ...
    try:
        ...
        rate_limit_info = await rate_limiting_middleware.check_rate_limit(
            request=request, user_id=user_id, user_type=user_type
        )
        ...
        if not rate_limit_info["exempt"]:
            ...
            if rate_limit_info["user_limit"]:
                ...
                response.headers["X-UserRateLimit-Remaining"] = str(
                    user_info["remaining"]
                )
                response.headers["X-UserRateLimit-Reset"] = str(
                    int(user_info["reset_time"])
                )
