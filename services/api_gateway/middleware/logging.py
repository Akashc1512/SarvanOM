    ...
    async def log_performance(
        ...
        if response_time > 1.0:
            ...
            if len(self.performance_metrics["slow_requests"]) > 100:
                self.performance_metrics["slow_requests"] = self.performance_metrics[
                    "slow_requests"
                ][-100:]
        ...
        if response.status_code >= 400:
            if endpoint not in self.performance_metrics["error_rates"]:
                self.performance_metrics["error_rates"][endpoint] = {
                    "errors": 0,
                    "total": 0,
                }
        ...
        if len(self.performance_metrics["throughput"]) > 1000:
            self.performance_metrics["throughput"] = self.performance_metrics[
                "throughput"
            ][-1000:]
