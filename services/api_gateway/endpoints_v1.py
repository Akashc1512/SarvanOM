...
async def process_query_v1(
    ...
    return await process_query(request, http_request, current_user)


@router.post("/feedback", response_model=FeedbackResponse)
async def submit_feedback_v1(
    request: FeedbackRequestValidator, current_user=Depends(get_current_user)
):
    ...
    # Call the main handler
    return await submit_feedback(request, current_user)


@router.get("/search")
@version_deprecated(sunset_date="2025-12-31")
async def search_v1(q: str, limit: int = 10, current_user=Depends(get_current_user)):
    """
    ...
    query_request = QueryRequestValidator(
        query=q, max_tokens=1000, confidence_threshold=0.7
    )
...
async def get_status_v1():
    ...
    return {"status": "operational", "version": "1.0.0", "api_version": "v1"}
