...
from shared.core.api.exceptions import ValidationError, VersionError
...
class VersionManager:
    ...
    def register_version(self, version: APIVersion) -> None:
        ...
        logger.info(
            "API version registered", version=version.version, status=version.status
        )
    ...
    def register_migration_handler(
        self, from_version: str, to_version: str, handler: Callable
    ) -> None:
    ...
    def migrate_data(
        ...
        if not handler:
            raise VersionError(
                f"No migration handler for {from_version} to {to_version}"
            )
...
class VersionDetector:
    ...
    def detect_version(self, request: Request) -> str:
        ...
        # Check Accept header
        accept_version = self._extract_version_from_header(
...
class VersionMiddleware:
    ...
    def __init__(
        self, version_manager: VersionManager, version_detector: VersionDetector
    ):
...
class VersionedRoute(APIRoute):
    ...
    def __init__(self, path: str, endpoint: Callable, version: str, **kwargs: Any):
...
class MigrationManager:
    ...
    def register_migration(
        ...
        self.version_manager.register_migration_handler(
            from_version, to_version, migration_func
        )
...
def initialize_migrations() -> None:
    ...
    migration_manager.register_migration(
        "v1", "v2", migrate_v1_to_v2, "Migrate from v1 to v2 API format"
    )

    # Register v2 to v1 migration
    migration_manager.register_migration(
        "v2", "v1", migrate_v2_to_v1, "Migrate from v2 to v1 API format"
    )
