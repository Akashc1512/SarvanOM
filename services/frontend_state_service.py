...
class FrontendStateService:
    ...
    def get_state(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            state_record = self.db_session.query(FrontendState).filter(
                FrontendState.session_id == session_id
            ).first()
            ...
            logger.debug(f"Retrieved state for session {session_id}: {len(state_data.get('current_view_state', {}))} keys")
            
            return state_data
            
        except SQLAlchemyError as e:
            logger.error(f"Database error while fetching state for session {session_id}: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error while fetching state for session {session_id}: {e}")
            raise
    
    def update_state(self, session_id: str, state_data: Dict[str, Any], 
        ...
        try:
            ...
            logger.debug(f"Successfully updated state for session {session_id}: {len(updated_state.get('current_view_state', {}))} keys")
            
            return updated_state
            
        except SQLAlchemyError as e:
            logger.error(f"Database error while updating state for session {session_id}: {e}")
            self.db_session.rollback()
            raise
        except Exception as e:
            logger.error(f"Unexpected error while updating state for session {session_id}: {e}")
            self.db_session.rollback()
            raise
    ...
    def clear_state(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Database error while clearing state for session {session_id}: {e}")
            self.db_session.rollback()
            raise
        except Exception as e:
            logger.error(f"Unexpected error while clearing state for session {session_id}: {e}")
            self.db_session.rollback()
            raise
    ...
    def delete_state(self, session_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Database error while deleting state for session {session_id}: {e}")
            self.db_session.rollback()
            raise
        except Exception as e:
            logger.error(f"Unexpected error while deleting state for session {session_id}: {e}")
            self.db_session.rollback()
            raise
    ...
    def get_states_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            state_records = self.db_session.query(FrontendState).filter(
            ...
            logger.error(f"Database error while fetching states for user {user_id}: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error while fetching states for user {user_id}: {e}")
            raise
    ...
    def get_state_value(self, session_id: str, key: str, default: Any = None) -> Any:
        ...
        try:
            ...
            logger.error(f"Error getting state value for session {session_id}, key {key}: {e}")
            return default
    
    def set_state_value(self, session_id: str, key: str, value: Any, 
                       user_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if state_record:
                ...
                print(f"DEBUG: Before set_state_value - current_view_state: {state_record.current_view_state}")
                
                # Use the model's set_state_value method
                state_record.set_state_value(key, value)
                
                # Debug: Check state after update
                print(f"DEBUG: After set_state_value - current_view_state: {state_record.current_view_state}")
            ...
            print(f"DEBUG: After commit - current_view_state: {state_record.current_view_state}")
            
            # Return updated state
            return state_record.to_dict()
            
        except Exception as e:
            logger.error(f"Error setting state value for session {session_id}, key {key}: {e}")
            raise
    
    def merge_state(self, session_id: str, new_state: Dict[str, Any], 
                   user_id: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def get_session_info(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'last_updated': state_record.last_updated.isoformat() if state_record.last_updated else None,
                'state_keys_count': len(state_record.current_view_state) if state_record.current_view_state else 0
            }
            
        except SQLAlchemyError as e:
            logger.error(f"Database error while getting session info for {session_id}: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error while getting session info for {session_id}: {e}")
            raise 