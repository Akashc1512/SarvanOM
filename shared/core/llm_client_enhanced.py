...
class ProviderConfig:
    ...
    token_limits: ProviderTokenLimits = field(
        default_factory=lambda: ProviderTokenLimits(4000, 4000)
    )
...
class OllamaProvider(LLMProviderInterface):
    ...
    async def generate_text(self, request: LLMRequest) -> LLMResponse:
        ...
        if not self.can_handle_request(request):
            raise ValueError(
                f"Request exceeds Ollama token limits: {self.token_limits.max_total_tokens}"
            )
...
class HuggingFaceProvider(LLMProviderInterface):
    ...
    async def generate_text(self, request: LLMRequest) -> LLMResponse:
        ...
        if not self.can_handle_request(request):
            raise ValueError(
                f"Request exceeds HuggingFace token limits: {self.token_limits.max_total_tokens}"
            )
...
class OpenAIProvider(LLMProviderInterface):
    ...
    async def generate_text(self, request: LLMRequest) -> LLMResponse:
        ...
        if not self.can_handle_request(request):
            raise ValueError(
                f"Request exceeds OpenAI token limits: {self.token_limits.max_total_tokens}"
            )
...
class EnhancedLLMClient:
    ...
    def _setup_providers(self):
        ...
        if (
            getattr(settings.ollama_enabled, "value", "true")
            if hasattr(settings.ollama_enabled, "value")
            else settings.ollama_enabled.lower() == "true"
        ):
    ...
    async def select_llm_provider(
        self, query: str, context_size: int = 0
    ) -> LLMProvider:
        """
        ...
        total_tokens_needed = (
            estimated_query_tokens + context_size + 1000
        )  # Buffer for response
        ...
        if query_type == QueryType.SIMPLE_FACTUAL:
            ...
            if LLMProvider.OLLAMA in self.providers:
                ...
                if provider.can_handle_request(
                    LLMRequest(query=query, max_tokens=1000)
                ):
                    logger.info(
                        f"🎯 Selected LLM Provider: Ollama for simple factual query"
                    )
            ...
            if LLMProvider.HUGGINGFACE in self.providers:
                ...
                if provider.can_handle_request(
                    ...
                    logger.info(
                        f"🎯 Selected LLM Provider: HuggingFace for simple factual query"
                    )
            ...
            if LLMProvider.HUGGINGFACE in self.providers:
                ...
                if provider.can_handle_request(
                    ...
                    logger.info(
                        f"🎯 Selected LLM Provider: HuggingFace for complex synthesis"
                    )
                    return LLMProvider.HUGGINGFACE
            ...
            if LLMProvider.OPENAI in self.providers:
                ...
                if provider.can_handle_request(
                    ...
                    logger.info(
                        f"🎯 Selected LLM Provider: OpenAI for complex synthesis"
                    )
        ...
        if LLMProvider.OPENAI in self.providers:
            ...
            if provider.can_handle_request(LLMRequest(query=query, max_tokens=1000)):
                logger.info(
                    f"🎯 Selected LLM Provider: OpenAI for large context/fallback"
                )
                return LLMProvider.OPENAI
        ...
        for provider_name in [
            ...
            if provider_name in self.providers:
                logger.info(
                    f"🎯 Selected LLM Provider: {provider_name.value} (fallback)"
                )
    ...
    async def dispatch(
        ...
        logger.info(
            f"Selected LLM Provider: {selected_provider.value} for query: {query_id}"
        )
            ...
            for fallback_provider in fallback_providers:
                try:
                    ...
                    if await provider.health_check():
                        ...
                        self.provider_stats[fallback_provider][
                            "last_used"
                        ] = time.time()

                        logger.info(
                            f"✅ Fallback successful using {fallback_provider.value}"
                        )
                        logger.info(
                            f"Selected LLM Provider: {fallback_provider.value} for query: {query_id}"
                        )
    ...
    def get_provider_stats(self) -> Dict[str, Any]:
        ...
        for provider, provider_stats in self.provider_stats.items():
            stats[provider.value] = {
                ...
                "success_rate": provider_stats["success"]
                / max(provider_stats["calls"], 1),
