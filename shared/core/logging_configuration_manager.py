...
class ConfigurationValidator:
    ...
    def validate_profile(profile: LoggingConfigProfile) -> List[str]:
        ...
        if (
            profile.cache_hit_rate_threshold < 0
            or profile.cache_hit_rate_threshold > 100
        ):
    ...
    def validate_environment_consistency(profile: LoggingConfigProfile) -> List[str]:
        ...
        if profile.environment == Environment.PRODUCTION:
            ...
            if (
                profile.log_format == LogFormat.JSON
                and profile.log_level != LogLevel.DEBUG
            ):
...
class LoggingConfigurationManager:
    ...
    def save_profile(self, profile: LoggingConfigProfile, filename: str):
        ...
        try:
            ...
            self.logger.error(
                "Failed to save configuration profile",
                config_file=filename,
                error=str(e),
                component="config_manager",
            )
        ...
        with self.lock:
            ...
            if profile.environment == Environment.PRODUCTION and profile.enable_metrics:
                setup_production_logging(
                    service_name=profile.service_name, enable_collection=True
                )
            else:
                setup_logging(
    ...
    def load_and_apply_environment_config(self, environment: Environment):
        ...
        try:
            ...
            self.logger.warning(
                "Environment config not found, creating default",
    ...
    def rollback_configuration(self) -> bool:
        ...
        with self.lock:
            if not self.profile_history:
                self.logger.warning(
                    "No previous configuration to rollback to",
                    component="config_manager",
                )
                return False
    ...
    def start_monitoring(self, check_interval: int = 30):
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_config_files, args=(check_interval,), daemon=True
        )
    ...
    def stop_monitoring(self):
        ...
        self.logger.info("Configuration monitoring stopped", component="config_manager")
    ...
    def _monitor_config_files(self, check_interval: int):
        ...
        while self._monitoring_enabled:
            try:
                    ...
                    if self._has_file_changed(str(config_file)):
                        ...
                        try:
                            ...
                            if (
                                ...
                                == self.current_profile.environment
                            ):
                                self.apply_profile(profile)
                        except Exception as e:
                            self.logger.error(
                                "Failed to reload changed configuration",
                                config_file=config_file.name,
                                error=str(e),
                                component="config_manager",
                            )
                ...
                self.logger.error(
                    "Error in configuration monitoring",
                    error=str(e),
                    component="config_manager",
                )
                time.sleep(check_interval)
...
def initialize_logging_for_environment(
    environment: str, service_name: str = "sarvanom", enable_monitoring: bool = True
):
    """
    ...
    try:
        ...
        logger.warning(
            "Unknown environment, defaulting to development",
