    ...
    async def _log_error(self, error_info: ErrorInfo):
        ...
        if error_info.severity == ErrorSeverity.CRITICAL:
            logger.error(
                ...
                log_data,
                exc_info=error_info.original_exception,
            )
        elif error_info.severity == ErrorSeverity.HIGH:
            logger.error(
                "High severity API error: %s",
                log_data,
                exc_info=error_info.original_exception,
            )
        else:
            logger.warning("API error: %s", log_data)
...
class LLMErrorHandler(ErrorHandler):
    ...
    async def _log_error(self, error_info: ErrorInfo):
        ...
        if error_info.severity == ErrorSeverity.CRITICAL:
            logger.error(
                "Critical LLM error: %s",
                log_data,
                exc_info=error_info.original_exception,
            )
        elif error_info.severity == ErrorSeverity.HIGH:
            logger.error(
                "High severity LLM error: %s",
                log_data,
                exc_info=error_info.original_exception,
            )
        else:
            logger.warning("LLM error: %s", log_data)
...
class DatabaseErrorHandler(ErrorHandler):
    ...
    async def _log_error(self, error_info: ErrorInfo):
        ...
        if error_info.severity == ErrorSeverity.CRITICAL:
            logger.error(
                "Critical database error: %s",
                log_data,
                exc_info=error_info.original_exception,
            )
        elif error_info.severity == ErrorSeverity.HIGH:
            logger.error(
                "High severity database error: %s",
                log_data,
                exc_info=error_info.original_exception,
            )
        else:
            logger.warning("Database error: %s", log_data)
...
def handle_critical_operation(
    operation_type: str = "api", max_retries: int = 3, timeout: float = 30.0
):
    """
