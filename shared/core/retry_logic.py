            ...
            for attempt in range(config.max_attempts):
                try:
                    ...
                    if config.circuit_breaker:
                        return await config.circuit_breaker.async_call(
                            func, *args, **kwargs
                        )
                    else:
...
FAST_RETRY = RetryConfig(max_attempts=3, initial_delay=0.1, max_delay=1.0, jitter=True)

STANDARD_RETRY = RetryConfig(
    max_attempts=3, initial_delay=1.0, max_delay=10.0, jitter=True
)

AGGRESSIVE_RETRY = RetryConfig(
    max_attempts=5, initial_delay=0.5, max_delay=30.0, jitter=True
)
