...
class LoggingDecorator(BaseDecorator):
    ...
    async def _handle_error(self, error: Exception, *args, **kwargs) -> Any:
        """Log errors."""
        logger.error(f"Error in {self.component_name}: {error}", exc_info=True)
        raise error
...
class TimingDecorator(BaseDecorator):
    ...
    async def _after_call(self, result: Any, *args, **kwargs) -> Any:
        ...
        logger.debug(f"{self.component_name} executed in {execution_time:.2f}ms")
...
class RetryDecorator(BaseDecorator):
    ...
    async def _handle_error(self, error: Exception, *args, **kwargs) -> Any:
        ...
        if retry_count < self.max_retries and isinstance(error, self.retry_exceptions):
        ...
        kwargs.pop("_retry_count", None)
        logger.error(f"Failed after {retry_count} retries: {self.component_name}")
        raise error
...
class RateLimitDecorator(BaseDecorator):
    ...
    async def _before_call(self, *args, **kwargs) -> tuple:
        ...
        async with self._lock:
            ...
            self._call_times[identifier] = [
                t for t in self._call_times[identifier] if t > cutoff_time
            ]
...
def create_logged_component(
    component: Any, log_level: str = "INFO", **kwargs
) -> LoggingDecorator:
...
def create_timed_component(
    component: Any, threshold_ms: Optional[float] = None, **kwargs
) -> TimingDecorator:
...
def create_cached_component(
    component: Any, ttl_seconds: int = 3600, **kwargs
) -> CachingDecorator:
...
def create_retryable_component(
    component: Any, max_retries: int = 3, **kwargs
) -> RetryDecorator:
...
def rate_limited(max_calls: int = 100, time_window: int = 60, **kwargs):
    ...
    def decorator(func):
        return RateLimitDecorator(
            func, max_calls=max_calls, time_window=time_window, **kwargs
        )
