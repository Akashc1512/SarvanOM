...
class MetricsCollector:
    ...
    def get_metrics_dict(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_response_time = (
                statistics.mean(self._response_times) if self._response_times else 0.0
            )
            avg_retrieval_time = (
                statistics.mean(self._retrieval_times) if self._retrieval_times else 0.0
            )
            avg_llm_time = statistics.mean(self._llm_times) if self._llm_times else 0.0
            avg_synthesis_time = (
                statistics.mean(self._synthesis_times) if self._synthesis_times else 0.0
            )
            ...
            if total_provider_usage > 0:
                for provider, count in self._provider_counters.items():
                    provider_percentages[provider.value] = (
                        count / total_provider_usage
                    ) * 100
            else:
            ...
            return {
                ...
                "queries": {
                    ...
                    "success_rate": (
                        (self._query_counter - self._error_counter)
                        / max(self._query_counter, 1)
                    )
                    * 100,
                },
                "response_times": {
                    ...
                    "breakdown": {
                        "retrieval_percent": (
                            avg_retrieval_time / max(avg_response_time, 1)
                        )
                        * 100,
                        "llm_percent": (avg_llm_time / max(avg_response_time, 1)) * 100,
                        "synthesis_percent": (
                            avg_synthesis_time / max(avg_response_time, 1)
                        )
                        * 100,
                    },
                },
    ...
    def get_prometheus_metrics(self) -> str:
        ...
        try:
            ...
            metrics_lines.append(
                f"# HELP queries_total Total number of queries processed"
            )
            ...
            if self._response_times:
                ...
                metrics_lines.append(
                    f"# HELP response_time_seconds Average response time"
                )
            ...
            for cache_name, metrics in self._cache_metrics.items():
                ...
                metrics_lines.append(
                    f"# HELP {cache_name}_misses_total Total cache misses"
                )
            ...
            for provider, count in self._provider_counters.items():
                metrics_lines.append(
                    f"# HELP llm_provider_usage_total Usage count by provider"
                )
                metrics_lines.append(f"# TYPE llm_provider_usage_total counter")
                metrics_lines.append(
                    f'llm_provider_usage_total{{provider="{provider.value}"}} {count}'
                )
            ...
            for component, result in self._health_results.items():
                status_value = {
                    ...
                    "unhealthy": 0,
                    "unknown": -1,
                }.get(result.status.value, -1)
                metrics_lines.append(f"# HELP component_health Component health status")
                metrics_lines.append(f"# TYPE component_health gauge")
                metrics_lines.append(
                    f'component_health{{component="{component}"}} {status_value}'
                )

                metrics_lines.append(
                    f"# HELP component_response_time_ms Component response time"
                )
                metrics_lines.append(f"# TYPE component_response_time_ms gauge")
                metrics_lines.append(
                    f'component_response_time_ms{{component="{component}"}} {result.response_time_ms}'
                )
...
async def record_query_metrics(
    ...
    try:
        ...
        logger.debug(
            f"📊 Recorded query metrics: provider={provider.value}, total_time={response_time_breakdown.total_time_ms:.2f}ms"
        )
...
async def record_health_check(
    ...
    try:
        ...
        logger.debug(
            f"📊 Recorded health check: {component}={status.value}, {response_time_ms:.2f}ms"
        )
