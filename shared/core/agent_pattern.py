...
class ModelSelectionStrategy:
    ...
    async def select_model_for_query(
        self, query: str, context: str = "", **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(
            f"Model selection - Prioritize free: {prioritize_free}, Dynamic selection: {use_dynamic_selection}"
        )
        ...
        if prioritize_free:
            return await self._select_with_free_priority(
                query, context, complexity, **kwargs
            )
        else:
            return await self._select_with_quality_priority(
                query, context, complexity, **kwargs
            )
    ...
    async def _select_with_free_priority(
        self, query: str, context: str, complexity: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        too_complex_for_local = (
            complexity == "complex" and len(query) + len(context) > 4000
        )
        ...
        if ollama_available and not too_complex_for_local:
            ...
            self.logger.info(
                f"🎯 Selected Ollama as primary provider for {complexity} query"
            )
        ...
        # Strategy 2: If Ollama not available or query too complex, try OpenAI with fallback
            ...
            selection_strategy["fallback_providers"] = (
                ["anthropic", "ollama"] if ollama_available else ["anthropic"]
            )
            selection_strategy["reason"] = (
                "OpenAI selected for complex query or Ollama unavailable"
            )

            self.logger.info(
                f"🎯 Selected OpenAI as primary provider for {complexity} query"
            )
            return selection_strategy
        ...
        # Strategy 3: Fallback to available providers
            ...
            self.logger.info(
                f"🎯 Selected Anthropic as primary provider for {complexity} query"
            )
            return selection_strategy
        ...
        # Strategy 4: Last resort - Ollama if available
            ...
            self.logger.info(
                f"🎯 Selected Ollama as last resort for {complexity} query"
            )
            return selection_strategy
    ...
    async def _select_with_quality_priority(
        self, query: str, context: str, complexity: str, **kwargs
    ) -> Dict[str, Any]:
        ...
        if complexity == "complex":
            if self.settings.openai_api_key:
                ...
                selection_strategy["primary_provider"] = "anthropic"
                selection_strategy["fallback_providers"] = (
                    ["ollama"] if self.settings.ollama_enabled else []
                )
        ...
        self.logger.info(
            f"🎯 Selected {selection_strategy['primary_provider']} as primary provider for {complexity} query"
        )
        return selection_strategy
...
class EnhancedLLMClientWithFallback:
    ...
    async def generate_text_with_fallback(
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    f"✅ Success with primary provider: {primary_provider}"
                )
                return response

            except Exception as e:
                self.logger.warning(
                    f"❌ Primary provider {primary_provider} failed: {str(e)}"
                )
                ...
                for fallback_provider in model_selection["fallback_providers"]:
                    try:
                        ...
                        self.logger.info(
                            f"✅ Success with fallback provider: {fallback_provider}"
                        )
                        return response
    ...
    async def _call_provider(
        self, provider: str, prompt: str, system_message: str = None, **kwargs
    ) -> LLMResponse:
    ...
    async def _call_llm(
        ...
        try:
            ...
            response = await self.llm_client.generate_text_with_fallback(
                prompt=prompt, system_message=system_message, context=context, **kwargs
            )
