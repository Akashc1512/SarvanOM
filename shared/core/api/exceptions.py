...
class InvalidAPIKeyError(AuthenticationError):
    ...
    def __init__(self, api_key_hint: Optional[str] = None):
        internal_msg = (
            f"Invalid API key: {api_key_hint}" if api_key_hint else "Invalid API key"
        )
...
class AgentError(UKPException):
    ...
    def __init__(
        self, agent_type: str, message: str, details: Optional[Dict[str, Any]] = None
    ):
...
class QueryProcessingError(UKPHTTPException):
    ...
    def __init__(self, query_id: str, internal_error: str, recoverable: bool = True):
        detail = (
            "Query processing failed"
            if recoverable
            else "Query processing failed permanently"
        )
        super().__init__(
            status_code=(
...
class ExternalServiceError(UKPException):
    ...
    def __init__(
        self, service: str, operation: str, error: str, retryable: bool = True
    ):
        super().__init__(
