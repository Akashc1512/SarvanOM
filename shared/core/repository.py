...
class BaseRepository(Repository[TEntity], Generic[TEntity]):
    ...
    def __init__(self, entity_type: Type[TEntity]):
        ...
        self.metrics = {"finds": 0, "saves": 0, "deletes": 0, "errors": 0}
...
class CachedRepository(CacheableRepository[TEntity], Generic[TEntity]):
    ...
    def __init__(self, base_repository: Repository[TEntity], cache_ttl: int = 3600):
...
class QueryableRepository(
    QueryRepository[TEntity, Query], BaseRepository[TEntity], Generic[TEntity]
):
    """
    ...
    def _apply_filters(
        self, entities: List[TEntity], filters: Dict[str, Any]
    ) -> List[TEntity]:
    ...
    def _apply_sorting(
        self, entities: List[TEntity], sort_by: str, desc: bool = False
    ) -> List[TEntity]:
        """Apply sorting to entities."""
        try:
            return sorted(entities, key=lambda e: getattr(e, sort_by, ""), reverse=desc)
        except Exception as e:
...
class RepositoryFactory:
    ...
    def create_file_repository(
        entity_type: Type[TEntity], file_path: Path
    ) -> Repository[TEntity]:
