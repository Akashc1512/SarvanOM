...
from shared.core.query_classifier import QueryClassifier, QueryCategory, QueryComplexity, QueryClassification
...
class DynamicModelSelector:
    ...
    async def select_model(
        ...
        try:
            ...
            if force_tier:
                ...
                optimal_tier = self._determine_optimal_tier(classification, estimated_tokens)
                reasoning = self._generate_selection_reasoning(classification, estimated_tokens, optimal_tier)
            ...
            estimated_cost = self._calculate_estimated_cost(model_config, estimated_tokens)
    ...
    def _calculate_model_score(
        ...
        max_cost = self.selection_thresholds["complexity_thresholds"]["complex"]["max_cost_per_query"]
        if config.cost_per_1k_tokens > 0:
            cost_efficiency = 1.0 - (config.cost_per_1k_tokens * estimated_tokens / 1000) / max_cost
    ...
    def _get_fallback_models(self, model_config: ModelConfig, tier: ModelTier) -> List[str]:
    ...
    def _calculate_estimated_cost(self, config: ModelConfig, estimated_tokens: int) -> float:
    ...
    def _generate_selection_reasoning(
        ...
        if classification.complexity == QueryComplexity.SIMPLE:
            ...
            reasoning_parts.append("Complex query - using powerful model for better reasoning")
        
        if classification.category == QueryCategory.CODE:
            reasoning_parts.append("Code-related query - prioritizing models with code capabilities")
        elif classification.category == QueryCategory.ANALYTICAL:
            reasoning_parts.append("Analytical query - using model with strong reasoning capabilities")
    ...
    def _log_model_selection(
        ...
        logger.info(f"Model selection: {result.selected_model} ({result.model_tier.value}) "
    ...
    def get_selection_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_confidence": sum(entry["confidence"] for entry in self.selection_history) / total_selections
        }
