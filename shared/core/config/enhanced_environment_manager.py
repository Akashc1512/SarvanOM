...
class SecretManager:
    ...
    def get_secret(self, key: str, environment: str, default: Optional[str] = None) -> Optional[str]:
        ...
        if self.config.vault_type == SecretVaultType.ENVIRONMENT:
            ...
            logger.warning(f"Vault type {self.config.vault_type} not implemented, falling back to environment")
            return self._get_from_environment(key, default)
    ...
    def validate_secret_format(self, key: str, value: str) -> List[str]:
        ...
        if "database_url" in key.lower():
            if not any(value.startswith(scheme) for scheme in ["postgresql://", "mysql://", "sqlite:///"]):
...
class EnhancedEnvironmentManager(EnvironmentManager):
    ...
    def __init__(self, app_env: Optional[str] = None, secret_config: Optional[SecretConfig] = None):
        """
    ...
    def _validate_configuration_security(self):
        ...
        for rule in self.validation_rules:
            if rule.environment_specific and self.environment not in rule.environment_specific:
                continue
            ...
            if rule.required and field_value is None:
                if rule.security_level == "critical":
                    errors.append(f"CRITICAL: {rule.field_name} is required for {self.environment.value}")
                else:
                    errors.append(f"{rule.field_name} is required for {self.environment.value}")
            ...
            if rule.security_level in ["high", "critical"]:
                secret_errors = self.secret_manager.validate_secret_format(rule.field_name, field_str)
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if self.config.mock_ai_responses:
                errors.append("CRITICAL: Mock AI responses must be disabled in production")
            if self.config.skip_authentication:
                errors.append("CRITICAL: Authentication cannot be skipped in production")
            if self.config.enable_debug_endpoints:
                errors.append("CRITICAL: Debug endpoints must be disabled in production")
        
        # Log warnings
        for warning in warnings_list:
            logger.warning(warning, environment=self.environment.value, component="config_security")
        
        # Fail on errors
        if errors:
            error_msg = f"Security validation failed for {self.environment.value} environment:\n" + "\n".join(errors)
            logger.error(error_msg, environment=self.environment.value, component="config_security")
    ...
    def _validate_required_secrets(self):
        ...
        if self.environment in [Environment.PRODUCTION, Environment.STAGING]:
            ...
            for secret_key in required_secrets:
                value = self.secret_manager.get_secret(secret_key, self.environment.value)
                if not value:
                    missing_secrets.append(secret_key)
            
            # At least one AI provider API key is required
            openai_key = self.secret_manager.get_secret("OPENAI_API_KEY", self.environment.value)
            anthropic_key = self.secret_manager.get_secret("ANTHROPIC_API_KEY", self.environment.value)
        ...
        if missing_secrets:
            ...
            logger.error(error_msg, environment=self.environment.value, component="secret_validation")
            raise ValueError(error_msg)
    ...
    def _log_configuration_summary(self):
        ...
        # Environment info
        logger.info(f"🌍 Environment: {self.environment.value.upper()}", component="config_summary")
        ...
        logger.info(f"📝 Log Level: {self.config.log_level}", component="config_summary")
        ...
        logger.info(f"✅ Enabled Features ({len(enabled_features)}): {', '.join(enabled_features)}", component="config_summary")
        logger.info(f"❌ Disabled Features ({len(disabled_features)}): {', '.join(disabled_features)}", component="config_summary")
        ...
        logger.info(f"   Rate Limit: {self.config.rate_limit_per_minute}/min", component="config_summary")
        logger.info(f"   Workers: {self.config.worker_processes} processes, {self.config.worker_threads} threads", component="config_summary")
        logger.info(f"   DB Pool: {self.config.db_pool_size} connections", component="config_summary")
        logger.info(f"   Cache TTL: {self.config.cache_ttl_seconds}s", component="config_summary")
    ...
    def reload_configuration(self):
        ...
        logger.info("Reloading configuration...", environment=self.environment.value, component="config_reload")
        ...
        if old_config_hash != new_config_hash:
            logger.info("Configuration changed, validating...", environment=self.environment.value, component="config_reload")
            self._validate_configuration_security()
            self._validate_required_secrets()
            logger.info("Configuration reloaded successfully", environment=self.environment.value, component="config_reload")
        else:
            logger.info("Configuration unchanged", environment=self.environment.value, component="config_reload")
    
    def _hash_config(self) -> str:
        """Create hash of current configuration for change detection."""
        config_str = json.dumps(self.config.to_dict(include_secrets=False), sort_keys=True)
    ...
    def validate_startup_requirements(self) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.error(f"Security validation failed: {e}", component="startup_validation")
        ...
        try:
            ...
            logger.error(f"Secret validation failed: {e}", component="startup_validation")
            checks["secret_validation"] = False
        
        # Database connectivity (placeholder)
        checks["database_connection"] = True  # Would implement actual connectivity check
        ...
        if all_passed:
            ...
            logger.error(f"❌ Startup validation failed: {failed_checks}", 
                        checks=checks, 
                        component="startup_validation")
...
def get_enhanced_environment_manager(app_env: Optional[str] = None, 
                                   secret_config: Optional[SecretConfig] = None) -> EnhancedEnvironmentManager:
...
def validate_environment_on_startup() -> bool:
    ...
    try:
        ...
        logger.error(f"Environment validation failed on startup: {e}", component="startup")
        return False
