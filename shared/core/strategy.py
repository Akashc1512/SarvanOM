...
class BaseSearchStrategy(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.metrics = {"searches": 0, "total_time": 0.0, "errors": 0}
...
class HybridSearchStrategy(BaseSearchStrategy):
    ...
    async def _execute_search(self, query: SearchQuery, **kwargs) -> List[SearchResult]:
        ...
        tasks = [strategy.search(query, **kwargs) for strategy in self.strategies]
...
class PersonalizedRankingStrategy:
    ...
    async def rank(
        ...
        for result in results:
            ...
            if any(
                pref in result.content.lower()
                for pref in user_preferences.get("topics", [])
            ):
                result.score *= 1.2
...
class MetadataFilterStrategy:
    ...
    async def filter(
        self, results: List[SearchResult], criteria: Dict[str, Any]
    ) -> List[SearchResult]:
        """Filter results based on metadata criteria."""
        filtered = []
...
class SearchStrategyContext:
    ...
    async def search(
        self, query: SearchQuery, strategy_name: Optional[str] = None, **kwargs
    ) -> List[SearchResult]:
        """
        ...
        # Apply ranking
        context = {"query": query, "user_context": query.user_context}
