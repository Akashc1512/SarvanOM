...
class CitationResult(AgentDataModel):
    ...
    bibliography: list[str] = Field(
        default_factory=list, description="Full bibliography"
    )
...
class AgentTaskModel(AgentDataModel):
    ...
    timeout_ms: int = Field(
        default=5000, ge=0, description="Task timeout in milliseconds"
    )
...
class LegacyCitationResult(AgentDataModel):
    ...
    citations: list[str] = Field(
        ...
        description="Legacy citations field (same as bibliography)",
    )
    citation_style: str = Field(default="APA", description="Citation style used")
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata"
            ...
            verified_facts.append(
                VerifiedFactModel(
                    ...
                    contradicting_evidence=getattr(
                        fact_data, "contradicting_evidence", []
                    ),
