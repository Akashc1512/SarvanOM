...
from ..cache.cached_agents import (
    CachedSynthesisAgent,
    CachedFactCheckAgent,
    CachedKnowledgeGraphAgent,
)
...
class CachedLeadOrchestrator(RefinedLeadOrchestrator):
    ...
    def _replace_agents_with_cached_versions(self):
        ...
        try:
            ...
            if ExtendedAgentType.KNOWLEDGE_GRAPH in self.agent_registry:
                ...
                logger.info(
                    "✅ Replaced KnowledgeGraphAgent with CachedKnowledgeGraphAgent"
                )
    ...
    async def process_query(self, query: str, user_context: Dict[str, Any] = None):
        ...
        self.metrics_collector.record_cache_operation(
            "query_processing", ExtendedAgentType.RETRIEVAL, 0, True
        )
        ...
        for agent_type, agent in self.agent_registry.items():
            if hasattr(agent, "get_cache_stats"):
                try:
                    ...
                    logger.warning(
                        f"Could not get cache stats for {agent_type.value}: {e}"
                    )
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if isinstance(config, dict):
            ...
            if "enable_parallel_retrieval" in config:
                pipeline_config.enable_parallel_retrieval = config[
                    "enable_parallel_retrieval"
                ]
            if "enable_enrichment_stage" in config:
                pipeline_config.enable_enrichment_stage = config[
                    "enable_enrichment_stage"
                ]
        else:
...
# Export main classes
__all__ = ["CachedLeadOrchestrator", "LeadOrchestrator"]
