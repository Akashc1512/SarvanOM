...
class BaseAgent(ABC):
    ...
    async def _handle_control_message(self, message: AgentMessage) -> AgentMessage:
        ...
        if control_action == "pause":
            ...
            return self._create_control_response(
                message, "paused", "Agent paused successfully"
            )
            ...
            return self._create_control_response(
                message, "resumed", "Agent resumed successfully"
            )
            ...
            return self._create_control_response(
                message, "shutdown", "Agent shutdown initiated"
            )

        else:
            logger.warning(f"Unknown control action: {control_action}")
            return self._create_error_message(
                message, f"Unknown control action: {control_action}"
            )
    ...
    def _create_result_message(
        self, original: AgentMessage, result: dict[str, Any]
    ) -> AgentMessage:
        """Create a result message from a dictionary result."""
        return AgentMessage(
            header={
                **original.header,
                "message_type": MessageType.RESULT,
        ...
        while self.is_running:
            try:
                message = await asyncio.wait_for(
                    self.message_queue.get(timeout=1.0), timeout=1.0
                )
