...
class KnowledgeGraphAgent(BaseAgent):
    ...
    async def _process_entity_relationship_query(
    ...
    async def _process_path_finding_query(
        self, query: str, entities: List[str]
    ) -> KnowledgeGraphResult:
    ...
    async def _process_entity_search_query(
        self, query: str, entities: List[str]
    ) -> KnowledgeGraphResult:
    ...
    async def _query_arangodb_entity_relationships(
        ...
        try:
            if len(entities) < 2:
                ...
                LIMIT 20
                """
                parameters = {"entity1": entities[0], "entity2": entities[1]}
    ...
    async def query_relationships(
        self, entity1: str, entity2: str = None
    ) -> KnowledgeGraphResult:
        """
        ...
        try:
                ...
                for record in result:
                    ...
                    if "relationships" in record:
                        for rel_data in record["relationships"]:
                            ...
                            found_entities.append(
                                EntityNode(
                                    id=related_entity.get(
                                        "id", related_entity.get("_key", "")
                                    ),
            ...
            pseudo_documents = self._format_as_pseudo_documents(
                unique_entities, unique_relationships, found_paths
            )
    ...
    def _format_as_pseudo_documents(
        self,
        entities: List[EntityNode],
        relationships: List[Relationship],
        paths: List[List[EntityNode]],
    ...
    async def _query_mock_relationships(
        self, entity1: str, entity2: str = None
    ) -> KnowledgeGraphResult:
        ...
        try:
            ...
            for entity_id, entity_data in self.mock_knowledge_graph["entities"].items():
                if (
                    entity1.lower() in entity_data["name"].lower()
                    or entity1.lower() in entity_data["id"].lower()
                ):
                    found_entities.append(
                        EntityNode(
                            id=entity_data["id"],
                ...
                if entity2 and (
                    entity2.lower() in entity_data["name"].lower()
                    or entity2.lower() in entity_data["id"].lower()
                ):
                    found_entities.append(
                        EntityNode(
                            id=entity_data["id"],
            ...
            pseudo_documents = self._format_as_pseudo_documents(
                found_entities, found_relationships, []
            )
