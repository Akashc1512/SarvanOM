...
class RefinedLeadOrchestrator:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None):
        ...
        logger.info(
            "🚀 Initializing Refined LeadOrchestrator",
        ...
        logger.info(
            "✅ Refined LeadOrchestrator initialized successfully",
    ...
    def _initialize_agents(self):
        ...
        try:
            ...
            logger.info(
                "✅ Core agents initialized successfully", component="orchestrator"
            )
        except Exception as e:
            logger.warning(
                f"Failed to initialize core agents: {e}",
                error=str(e),
                component="orchestrator",
            )
    ...
    def register_agent(self, agent_type: ExtendedAgentType, agent: BaseAgent):
        ...
        logger.info(
            f"Registered new agent: {agent_type.value}",
            agent_type=agent_type.value,
            component="orchestrator",
        )
    ...
    async def _execute_enrichment_stage(
        ...
        return await self._execute_agents_in_parallel(
            parallel_agents, context, "enrichment"
        )
    ...
    async def _execute_single_agent_stage(
        ...
        try:
            ...
            logger.error(
                f"Agent {agent_type.value} execution failed: {e}",
                agent_type=agent_type.value,
                error=str(e),
            )
            return AgentResult(
        ...
        try:
            ...
            for i, (result, (agent_type, _)) in enumerate(zip(results, agents)):
                if isinstance(result, Exception):
                    logger.error(
                        f"Parallel agent {agent_type.value} failed: {result}",
                        agent_type=agent_type.value,
                        stage=stage_name,
                        error=str(result),
                    )
            ...
            if result.success and result.data:
                ...
                if isinstance(result.data, dict):
                    ...
                    if "documents" in result.data:
                        ...
                        context.metadata["all_documents"].extend(
    ...
    def _build_pipeline_result(
        ...
        if synthesis_results and synthesis_results.success:
            ...
            if isinstance(synthesis_data, dict):
                final_answer = synthesis_data.get("answer") or synthesis_data.get(
                    "response"
                )
        ...
        return PipelineResult(
            ...
            stage_results={
                stage.value: result for stage, result in stage_results.items()
            },
