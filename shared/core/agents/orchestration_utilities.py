...
class AgentExecutionWorkflow:
    ...
    async def _execute_with_retry(
        self, agent, context: Any, timeout: float = 30.0, max_retries: int = 3
    ) -> Any:
...
class PipelineOrchestrator:
    ...
    async def execute_pipeline(
        self, agents: List[Any], context: Any, pipeline_config: Dict[str, Any]
    ) -> OrchestrationResult:
        """
        ...
        try:
            for i, agent in enumerate(agents):
                ...
                if not result.success and not pipeline_config.get(
...
class WorkflowManager:
        ...
        try:
            for i, step in enumerate(workflow_steps):
                ...
                step_result = await self._execute_workflow_step(
                    step=step, input_data=current_data, context=context
                )
    ...
    async def _execute_workflow_step(
        self, step: Dict[str, Any], input_data: Dict[str, Any], context: Any
    ) -> OrchestrationResult:
