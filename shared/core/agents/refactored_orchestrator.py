...
class PipelineConfig:
    ...
    enabled_agents: Set[ExtendedAgentType] = field(
...
class RefactoredOrchestrator:
    ...
    def __init__(self, config: Optional[PipelineConfig] = None):
        ...
        logger.info(
            "🚀 Refactored Orchestrator initialized",
    ...
    def _initialize_agents(self):
        ...
        for agent_type in self.config.enabled_agents:
            try:
                ...
                logger.info(
                    f"✅ Initialized agent: {agent_type.value}",
    ...
    async def process_query(
        ...
        aggregated_results = await self._aggregate_pipeline_results(
            stage_results, context
        )

        # Step 5: Format final response
        final_result = await self._format_final_response(
            aggregated_results, context, start_time
        )
    ...
    async def _create_query_context(
        self, parsed_input: Dict[str, Any], start_time: float
    ) -> QueryContext:
        """
    ...
    async def _execute_pipeline_stages(
        ...
        try:
            ...
            if self.config.enable_parallel_retrieval:
                retrieval_results = await self._execute_parallel_retrieval_stage(
                    context
                )
            ...
            logger.error(
                f"Pipeline stage execution failed: {e}",
    ...
    async def _execute_parallel_retrieval_stage(
        self, context: QueryContext
    ) -> List[AgentResult]:
        """
        ...
        if ExtendedAgentType.RETRIEVAL in self.agent_registry:
            parallel_agents.append(
                (
                    ExtendedAgentType.RETRIEVAL,
                    self.agent_registry[ExtendedAgentType.RETRIEVAL],
                )
            )
        ...
        if ExtendedAgentType.KNOWLEDGE_GRAPH in self.agent_registry:
            parallel_agents.append(
                (
                    ExtendedAgentType.KNOWLEDGE_GRAPH,
                    self.agent_registry[ExtendedAgentType.KNOWLEDGE_GRAPH],
                )
            )

        if not parallel_agents:
            logger.warning(
                "No retrieval agents available",
                query_id=context.trace_id,
                component="orchestrator",
            )
            return []
    ...
    async def _execute_enrichment_stage(
        ...
        if ExtendedAgentType.BROWSER in self.agent_registry:
            enrichment_agents.append(
                (
                    ...
                    self.agent_registry[ExtendedAgentType.BROWSER],
                )
            )
        ...
        if ExtendedAgentType.DATABASE in self.agent_registry:
            enrichment_agents.append(
                (
                    ...
                    self.agent_registry[ExtendedAgentType.DATABASE],
                )
            )
        ...
        if ExtendedAgentType.PDF in self.agent_registry:
            enrichment_agents.append(
                (ExtendedAgentType.PDF, self.agent_registry[ExtendedAgentType.PDF])
            )
    ...
    async def _execute_single_agent_stage(
        self, agent_type: ExtendedAgentType, context: QueryContext
    ) -> AgentResult:
        """
        ...
        if agent_type not in self.agent_registry:
            logger.error(
                f"Agent {agent_type.value} not found in registry",
                agent_type=agent_type.value,
                query_id=context.trace_id,
                component="orchestrator",
            )
    ...
    async def _execute_agents_in_parallel(
        ...
        logger.info(
            f"Executing {len(agents)} agents in parallel for stage: {stage_name}",
            query_id=context.trace_id,
            stage=stage_name,
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                logger.error(
                    f"Agent {agent_type.value} failed: {result}",
                    agent_type=agent_type.value,
                    query_id=context.trace_id,
                    component="orchestrator",
                )
                processed_results.append(
                    AgentResult(success=False, agent_type=agent_type, error=str(result))
                )
            else:
    ...
    async def _execute_agent_with_timeout(
        self, agent: BaseAgent, context: QueryContext, agent_type: ExtendedAgentType
    ) -> AgentResult:
        """
        ...
        try:
            ...
            result = await asyncio.wait_for(
                agent.execute(context), timeout=self.config.agent_timeout_seconds
            )
            ...
            logger.error(
                f"Agent {agent_type.value} timed out",
                agent_type=agent_type.value,
                query_id=context.trace_id,
            ...
            logger.error(
                f"Agent {agent_type.value} failed: {e}",
                agent_type=agent_type.value,
                query_id=context.trace_id,
                component="orchestrator",
            )
            return AgentResult(success=False, agent_type=agent_type, error=str(e))

    async def _update_context_with_results(
        self, context: QueryContext, results: List[AgentResult]
    ):
        """
    ...
    async def _aggregate_pipeline_results(
        self, stage_results: Dict[PipelineStage, Any], context: QueryContext
    ) -> Dict[str, Any]:
        """
            ...
            if isinstance(results, list):
                ...
                if stage == PipelineStage.SYNTHESIS and results:
                    ...
                    if synthesis_result.success:
                        aggregated["final_answer"] = getattr(
                            synthesis_result, "content", ""
                        )
                        aggregated["confidence"] = getattr(
    ...
    async def _format_final_response(
        ...
        success = (
            aggregated_results["successful_agents"] > 0
            and aggregated_results["final_answer"] is not None
            and aggregated_results["confidence"]
            >= self.config.minimum_confidence_threshold
        )
    ...
    def _create_error_result(
        ...
        return PipelineResult(
            success=False, errors=errors, total_execution_time_ms=execution_time_ms
        )
    ...
    def register_agent(self, agent_type: ExtendedAgentType, agent: BaseAgent):
        ...
        logger.info(
            f"Registered agent: {agent_type.value}",
            agent_type=agent_type.value,
            component="orchestrator",
        )
    ...
    def unregister_agent(self, agent_type: ExtendedAgentType):
        ...
        if agent_type in self.agent_registry:
            ...
            logger.info(
                f"Unregistered agent: {agent_type.value}",
                agent_type=agent_type.value,
                component="orchestrator",
            )
