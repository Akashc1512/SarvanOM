...
class ResultProcessor:
    ...
    def deduplicate_documents(
        documents: List[Document], similarity_threshold: float = 0.8
    ) -> List[Document]:
    ...
    def format_documents_for_llm(
        documents: List[Document], max_tokens: int = 4000
    ) -> str:
...
class SearchFusion:
    ...
    async def fuse_results(
        results: List[SearchResult], fusion_strategy: str = "weighted"
    ) -> SearchResult:
        """Fuse multiple search results into a single result."""
        if not results:
            return SearchResult(
                documents=[], search_type="fused", query_time_ms=0, total_hits=0
            )
    ...
    def _simple_fusion(results: List[SearchResult]) -> SearchResult:
        ...
        return SearchResult(
            documents=[
                doc.to_dict() if hasattr(doc, "to_dict") else doc for doc in unique_docs
            ],
...
async def execute_search_with_fallback(
    primary_search: callable, fallback_search: callable, query: str, **kwargs
) -> SearchResult:
    """Execute search with automatic fallback."""
    return await FallbackManager.execute_with_fallback(
        primary_search, fallback_search, query, **kwargs
    )
