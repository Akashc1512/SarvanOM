...
class KnowledgeGraphService:
    ...
    async def extract_entities_enhanced(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [{"text": entity, "type": "CONCEPT", "confidence": 0.8} for entity in basic_entities]
            
        except Exception as e:
            logger.error(f"Enhanced entity extraction failed: {e}")
            return []
    
    async def query_entity_relationships(self, query: str, entities: List[str]) -> KnowledgeGraphResult:
    ...
    async def query_path_finding(self, query: str, entities: List[str]) -> KnowledgeGraphResult:
    ...
    async def query_entity_search(self, query: str, entities: List[str]) -> KnowledgeGraphResult:
    ...
    async def query_general(self, query: str, entities: List[str]) -> KnowledgeGraphResult:
    ...
    async def _query_arangodb_entity_relationships(self, query: str, entities: List[str]) -> KnowledgeGraphResult:
        ...
        try:
            if len(entities) < 2:
                # Single entity query
                nodes = await self.graph_client.find_nodes_by_name(entities[0] if entities else "")
                found_entities = [self._convert_graph_node_to_entity(node) for node in nodes]
                found_relationships = []
            else:
                # Multiple entities - find relationships between them
                edges = await self.graph_client.find_relationships_between_nodes(entities[0], entities[1])
                found_entities = []
                found_relationships = [self._convert_graph_edge_to_relationship(edge) for edge in edges]
                ...
                for entity_id in entity_ids:
                    ...
                    if nodes:
                        found_entities.append(self._convert_graph_node_to_entity(nodes[0]))
    ...
    async def _query_arangodb_path_finding(self, query: str, entities: List[str]) -> KnowledgeGraphResult:
        ...
        try:
            ...
            paths = await self.graph_client.find_paths_between_nodes(entities[0], entities[1])
            ...
            for path in paths:
                path_entities = [self._convert_graph_node_to_entity(node) for node in path]
                converted_paths.append(path_entities)
                all_entities.extend(path_entities)
            
            # Remove duplicates
            unique_entities = list({entity.id: entity for entity in all_entities}.values())
    ...
    def _convert_graph_edge_to_relationship(self, graph_edge: GraphEdge) -> Relationship:
    ...
    async def _create_document_relationships(self, entities: List[Dict], document_id: str, 
        ...
        try:
            ...
            for i, entity1 in enumerate(entities):
                for j, entity2 in enumerate(entities[i+1:], i+1):
                    ...
                    relationship_type = self._determine_entity_relationship_type(entity1, entity2)
