...
Refactored to use modular components: GraphDBClient and KnowledgeGraphService.
"""
...
from shared.core.agents.graph_db_client import GraphDBClient
from shared.core.agents.knowledge_graph_service import KnowledgeGraphService, KnowledgeGraphResult
...
class ArangoDBKnowledgeGraphAgent(BaseAgent):
    ...
    async def upsert_document_entities(self, document_id: str, content: str, metadata: Dict[str, Any] = None) -> bool:
        ...
        """
        return await self.knowledge_service.upsert_document_entities(document_id, content, metadata)
    ...
    async def upsert_document_entities_enhanced(self, document_id: str, content: str, metadata: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            for entity in entities:
                ...
                success = await self.graph_client.create_node(node_id, entity['type'], properties)
            ...
            for record in result:
                if 'entity' in record and record['entity']:
                    ...
                    if entity['_id'] not in entity_ids:
                        found_entities.append(self.knowledge_service._convert_graph_node_to_entity(
                ...
                if 'relationship' in record and record['relationship']:
                    ...
                    found_relationships.append(self.knowledge_service._convert_graph_edge_to_relationship(
    ...
    async def _query_mock_related_entities(self, query_entities: List[str]) -> KnowledgeGraphResult:
        ...
        for entity_name in query_entities:
            for entity_id, entity_data in self.knowledge_service.mock_knowledge_graph["entities"].items():
                if entity_name.lower() in entity_data["name"].lower():
                    found_entities.append(self.knowledge_service.EntityNode(
                        id=entity_id,
        ...
        for rel in self.knowledge_service.mock_knowledge_graph["relationships"]:
            ...
            if source_entity and target_entity:
                found_relationships.append(self.knowledge_service.Relationship(
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "entities_count": len(self.knowledge_service.mock_knowledge_graph["entities"]) if hasattr(self.knowledge_service, 'mock_knowledge_graph') else 0,
            "relationships_count": len(self.knowledge_service.mock_knowledge_graph["relationships"]) if hasattr(self.knowledge_service, 'mock_knowledge_graph') else 0,
