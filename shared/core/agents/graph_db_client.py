...
class GraphDBClient:
    ...
    async def create_edge(
        ...
        try:
            ...
            logger.debug(
                f"✅ Created graph edge: {from_node_id} --[{relationship_type}]--> {to_node_id}"
            )
            return True
        except Exception as e:
    ...
    async def execute_query(
        self, aql_query: str, parameters: Dict[str, Any] = None
    ) -> List[Dict[str, Any]]:
    ...
    async def find_relationships_between_nodes(
        self, node1_name: str, node2_name: str
    ) -> List[GraphEdge]:
    ...
    async def find_paths_between_nodes(
        self, start_node: str, end_node: str, max_depth: int = 3
    ) -> List[List[GraphNode]]:
        ...
        try:
            ...
            parameters = {"start": start_node, "end": end_node, "max_depth": max_depth}
    ...
    async def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get entity count
            entity_count = self.db.aql.execute(
                "RETURN LENGTH(FOR doc IN entities RETURN doc)"
            )[0]

            # Get relationship count
            relationship_count = self.db.aql.execute(
                "RETURN LENGTH(FOR doc IN relationships RETURN doc)"
            )[0]
