...
class QueryClassifier:
    ...
    async def classify_query(self, query: str) -> QueryClassification:
        ...
        kg_score = self._calculate_pattern_score(query_lower, self.knowledge_graph_patterns)
        if kg_score > 0:
            category_scores[QueryCategory.KNOWLEDGE_GRAPH] = kg_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.knowledge_graph_patterns))
        
        # Code classification
        code_score = self._calculate_pattern_score(query_lower, self.code_patterns)
        if code_score > 0:
            category_scores[QueryCategory.CODE] = code_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.code_patterns))
        
        # Analytical classification
        analytical_score = self._calculate_pattern_score(query_lower, self.analytical_patterns)
        if analytical_score > 0:
            category_scores[QueryCategory.ANALYTICAL] = analytical_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.analytical_patterns))
        
        # Comparative classification
        comparative_score = self._calculate_pattern_score(query_lower, self.comparative_patterns)
        if comparative_score > 0:
            category_scores[QueryCategory.COMPARATIVE] = comparative_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.comparative_patterns))
        
        # Procedural classification
        procedural_score = self._calculate_pattern_score(query_lower, self.procedural_patterns)
        if procedural_score > 0:
            category_scores[QueryCategory.PROCEDURAL] = procedural_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.procedural_patterns))
        
        # Creative classification
        creative_score = self._calculate_pattern_score(query_lower, self.creative_patterns)
        if creative_score > 0:
            category_scores[QueryCategory.CREATIVE] = creative_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.creative_patterns))
        
        # Opinion classification
        opinion_score = self._calculate_pattern_score(query_lower, self.opinion_patterns)
        if opinion_score > 0:
            category_scores[QueryCategory.OPINION] = opinion_score
            detected_patterns.extend(self._get_matched_patterns(query_lower, self.opinion_patterns))
        ...
        routing_hints = self._generate_routing_hints(category, complexity, detected_patterns)
        ...
        logger.info(f"Query classified as {category.value} (confidence: {confidence:.2f}) in {processing_time:.2f}ms")
    ...
    def _get_matched_patterns(self, query: str, patterns: List[str]) -> List[str]:
        ...
        for pattern in patterns:
            if re.search(pattern, query, re.IGNORECASE):
                ...
                pattern_desc = pattern.replace(r'\b', '').replace('|', ' or ').replace('\\', '')
    ...
    def _generate_routing_hints(self, category: QueryCategory, complexity: QueryComplexity, patterns: List[str]) -> Dict[str, Any]:
        """Generate routing hints based on classification."""
        hints = {
            "primary_category": category.value,
            "complexity_level": complexity.value,
            "detected_patterns": patterns,
            "execution_strategy": self._suggest_execution_strategy(category, complexity),
    ...
    def _suggest_agents(self, category: QueryCategory, complexity: QueryComplexity) -> List[str]:
        ...
        if category == QueryCategory.KNOWLEDGE_GRAPH:
            agents.extend(["retrieval", "synthesis"])  # Knowledge graph queries need good retrieval
        elif category == QueryCategory.CODE:
            agents.extend(["retrieval", "synthesis"])  # Code queries need examples and explanations
        elif category == QueryCategory.ANALYTICAL:
            agents.extend(["fact_check", "citation"])  # Analytical queries need verification
        elif category == QueryCategory.COMPARATIVE:
            agents.extend(["retrieval", "synthesis"])  # Comparative queries need multiple sources
        elif category == QueryCategory.PROCEDURAL:
            agents.extend(["retrieval", "synthesis"])  # Procedural queries need step-by-step guidance
            ...
            agents.extend(["fact_check", "citation"])  # Opinion queries need fact checking
        
        # Complexity-based adjustments
        if complexity == QueryComplexity.COMPLEX:
            agents.extend(["fact_check", "citation"])  # Complex queries need verification
        
        return list(set(agents))  # Remove duplicates
    
    def _suggest_execution_strategy(self, category: QueryCategory, complexity: QueryComplexity) -> str:
    ...
    def _determine_priority(self, category: QueryCategory, complexity: QueryComplexity) -> str:
    ...
    def get_classification_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "complexity_indicators": {
                complexity: len(patterns) for complexity, patterns in self.complexity_indicators.items()
            }
        } 