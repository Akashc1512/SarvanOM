...
class ShortTermMemory:
    ...
    async def retrieve(self, key: str) -> Optional[Any]:
        ...
        try:
            with self.db_service.get_session() as session:
                session_memory = (
                    session.query(self.SessionMemory)
                    .filter(
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            with self.db_service.get_session() as session:
                result = (
                    session.query(self.SessionMemory)
                    .filter(self.SessionMemory.session_id == key)
                    .delete()
                )
                session.commit()
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self.db_service.get_session() as session:
                ...
                active_items = (
                    session.query(self.SessionMemory)
                    .filter(self.SessionMemory.expires_at > datetime.now())
                    .count()
                )
...
class MediumTermMemory:
    ...
    async def cleanup_expired(self) -> int:
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {cleaned_count} expired items from medium-term memory"
            )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"items": len(files), "size_bytes": total_size}
        except Exception as e:
...
class LongTermMemory:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"items": len(self.cache), "size_bytes": total_size}
        except Exception as e:
...
class MemoryManager:
    ...
    async def get_memory_stats(self) -> MemoryStats:
        ...
        try:
            ...
            total_items = (
                short_stats["items"] + medium_stats["items"] + long_stats["items"]
            )
            total_size = (
                short_stats["size_bytes"]
