...
rate_limit_hits = Counter(
    "rate_limit_hits_total", "Rate limit hits", ["limit_type", "identifier_type"]
)
...
active_rate_limits = Gauge("active_rate_limits", "Active rate limits", ["limit_type"])
...
class RedisRateLimiter:
    ...
    async def connect(self) -> None:
        ...
        if not self._pool:
            self._pool = aioredis.ConnectionPool.from_url(
                self.redis_url, max_connections=self.pool_size
            )
    ...
    async def _leaky_bucket(
        ...
        result = await self._client.eval(
            lua_script,
            keys=[key],
            args=[config.requests_per_second, config.burst_size, now],
        )
    ...
    async def get_usage(self, identifier: str) -> Dict[str, Any]:
        ...
        usage = {"identifier": identifier, "algorithms": {}}
...
class RateLimiter:
    ...
    async def initialize(self) -> None:
        ...
        if not self.backend:
            ...
            # Use InMemoryRateLimiter by default to avoid Redis issues
    ...
    async def check_rate_limit(
        ...
        # Check rate limit
        allowed, info = await self.backend.check_rate_limit(identifier, config)
...
async def rate_limit_middleware(request: Request, call_next: Callable) -> Response:
    ...
    config = rate_limiter._endpoint_configs.get(
        request.url.path, rate_limiter.default_config
    )
    identifier = rate_limiter._get_identifier(request, config)

    # Check rate limit
    allowed, info = await rate_limiter.check_rate_limit(
        identifier, request.url.path, request.method
    )
    ...
    response.headers["X-RateLimit-Reset"] = str(int(time.time()) + 60)  # Next window

    return response


# Decorator
def rate_limit(config: Optional[RateLimitConfig] = None) -> Callable:
    """
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if request:
                ...
                if endpoint not in rate_limiter._endpoint_configs:
                    rate_limiter.configure_endpoint(endpoint, func._rate_limit_config)

                # Get identifier
                identifier = rate_limiter._get_identifier(
                    request, func._rate_limit_config
                )

                # Check limit
                allowed, info = await rate_limiter.check_rate_limit(
                    identifier, endpoint, request.method
                )
