...
class HuggingFaceProvider(LLMProviderInterface):
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            async with session.get("https://api-inference.huggingface.co/models/microsoft/DialoGPT-medium") as response:
                return response.status in [200, 404]  # 404 means model not loaded but API is working
        except Exception as e:
...
class DynamicLLMClient:
    ...
    async def _classify_query(self, query: str) -> QueryType:
        ...
        if not self.query_classifier:
            ...
            if any(word in query_lower for word in ["what is", "who is", "when", "where", "how many", "define"]):
                return QueryType.SIMPLE_FACTUAL
            elif any(word in query_lower for word in ["analyze", "compare", "research", "study", "investigate"]):
        ...
        try:
            ...
            if classification.category.value in ["general_factual", "procedural"]:
                return QueryType.SIMPLE_FACTUAL
            elif classification.category.value in ["analytical", "comparative", "research_synthesis"]:
                return QueryType.RESEARCH_SYNTHESIS
            else:
                return QueryType.COMPLEX_REASONING
    ...
    async def dispatch(self, query: str, context: Optional[str] = None, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            fallback_providers = [p for p in self.providers.keys() if p != selected_provider]
            ...
            for fallback_provider in fallback_providers:
                try:
                    ...
                    # Check if provider is healthy
                    if not await provider.health_check():
                        logger.warning(f"⚠️ Fallback provider {fallback_provider.value} is not healthy")
                        continue
                    ...
                    # Log success
                    logger.info(f"✅ Generated response using fallback {fallback_provider.value} in {response.response_time_ms:.2f}ms")
    ...
    def get_provider_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_success": sum(stats["success"] for stats in self.provider_stats.values())
        }
