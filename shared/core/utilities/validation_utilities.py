...
class ValidationUtils:
    ...
    def validate_string(
        ...
        return ValidationResult(
            is_valid=True, sanitized_data={field_name: value.strip()}
        )
    ...
    def validate_non_empty(value: Any, field_name: str) -> ValidationResult:
        ...
        if not value or (isinstance(value, str) and len(value.strip()) == 0):
            return ValidationResult(
                is_valid=False, errors=[f"{field_name} cannot be empty"]
            )

        return ValidationResult(is_valid=True, sanitized_data={field_name: value})

    @staticmethod
    def validate_length(
        value: Any, field_name: str, max_length: int, min_length: Optional[int] = None
    ) -> ValidationResult:
        """
    ...
    def validate_required_fields(
        data: Dict[str, Any], required_fields: List[str], allow_empty: bool = False
    ) -> ValidationResult:
        """
        ...
        return ValidationResult(is_valid=True, sanitized_data=sanitized_data)
    ...
    def validate_query_input(
        ...
        query_validation = ValidationUtils.validate_string(
            value=query, field_name="query", max_length=10000, allow_empty=False
        )
    ...
    def validate_documents_input(
        ...
        if not isinstance(documents, list):
            return ValidationResult(is_valid=False, errors=["Documents must be a list"])

        # Validate each document
        for i, doc in enumerate(documents):
            if not isinstance(doc, dict):
                return ValidationResult(
                    is_valid=False, errors=[f"Document {i} must be a dictionary"]
                )

            if "content" not in doc:
                return ValidationResult(
                    is_valid=False, errors=[f"Document {i} missing 'content' field"]
                )

        return ValidationResult(is_valid=True, sanitized_data={"documents": documents})
    ...
    def validate_sources_input(
        ...
        if not isinstance(sources, list):
            return ValidationResult(is_valid=False, errors=["Sources must be a list"])

        # Validate each source
        for i, source in enumerate(sources):
            if not isinstance(source, dict):
                return ValidationResult(
                    is_valid=False, errors=[f"Source {i} must be a dictionary"]
                )

        return ValidationResult(is_valid=True, sanitized_data={"sources": sources})
    ...
    def validate_agent_task(
        ...
        required_validation = ValidationUtils.validate_required_fields(
            data=task, required_fields=required_fields
        )
        ...
        return ValidationResult(
            is_valid=True, sanitized_data=required_validation.sanitized_data
        )
...
def validate_length(
    ...
    return ValidationUtils.validate_length(
        value=value, field_name=field_name, max_length=max_length, min_length=min_length
    )
...
def validate_required_fields(
    ...
    return ValidationUtils.validate_required_fields(
        data=data, required_fields=required_fields, allow_empty=allow_empty
    )


def validate_query_input(task: Dict[str, Any], context: Any = None) -> ValidationResult:
    """
    Validate query input for agents.
