...
class BaseModel:
    ...
    def update_from_dict(
        self, data: Dict[str, Any], allowed_fields: Optional[List[str]] = None
    ) -> None:
        """
...
class Role(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("length(name) >= 3", name="ck_roles_name_length"),
    ...
    is_system = Column(
        Boolean, nullable=False, default=False, comment="System role flag"
    )
...
class User(Base):
    ...
    username = Column(
        String(50), nullable=False, unique=True, comment="Unique username"
    )

    email = Column(EmailType, nullable=False, unique=True, comment="Email address")
    ...
    avatar_url = Column(URLType, nullable=True, comment="Avatar URL")
    ...
    two_factor_enabled = Column(
        Boolean, nullable=False, default=False, comment="2FA enabled flag"
    )
    ...
    last_login_ip = Column(
        IPAddressType, nullable=True, comment="Last login IP address"
    )
...
class UserSession(Base):
    ...
    token = Column(String(255), nullable=False, unique=True, comment="Session token")

    refresh_token = Column(
        String(255), nullable=True, unique=True, comment="Refresh token"
    )

    expires_at = Column(
        DateTime(timezone=True), nullable=False, comment="Session expiration"
    )

    refresh_expires_at = Column(
        DateTime(timezone=True), nullable=True, comment="Refresh token expiration"
    )

    ip_address = Column(IPAddressType, nullable=True, comment="Client IP address")
...
class APIKey(Base):
    ...
    name = Column(String(100), nullable=False, comment="API key name")

    key_hash = Column(String(255), nullable=False, unique=True, comment="API key hash")
    ...
    scopes = Column(
        ARRAY(String), nullable=False, default=list, comment="API key scopes"
    )
    ...
    rate_limit = Column(Integer, nullable=True, comment="Rate limit per minute")

    allowed_ips = Column(ARRAY(IPAddressType), nullable=True, comment="IP whitelist")
...
class KnowledgeItem(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "confidence >= 0 AND confidence <= 1", name="ck_knowledge_confidence"
        ),
        {"comment": "Knowledge base items"},
    )

    # Fields
    title = Column(String(255), nullable=False, comment="Item title")

    content = Column(Text, nullable=False, comment="Item content")

    summary = Column(Text, nullable=True, comment="Content summary")

    type = Column(String(50), nullable=False, default="document", comment="Item type")

    source = Column(String(255), nullable=True, comment="Content source")

    source_url = Column(URLType, nullable=True, comment="Source URL")

    confidence = Column(Float, nullable=False, default=1.0, comment="Confidence score")

    tags = Column(ARRAY(String), nullable=False, default=list, comment="Item tags")

    embedding = Column(ARRAY(Float), nullable=True, comment="Vector embedding")
    ...
    queries = relationship("Query", secondary="query_results", back_populates="results")
...
query_results = Table(
    ...
    Base.metadata,
    Column(
        "query_id", UUID(as_uuid=True), ForeignKey("queries.id", ondelete="CASCADE")
    ),
    Column(
        "knowledge_item_id",
        UUID(as_uuid=True),
...
class Query(Base):
    ...
    query_embedding = Column(ARRAY(Float), nullable=True, comment="Query embedding")

    response_text = Column(Text, nullable=True, comment="Response text")

    response_time_ms = Column(
        Integer, nullable=True, comment="Response time in milliseconds"
    )

    tokens_used = Column(Integer, nullable=False, default=0, comment="Tokens consumed")

    cost = Column(DECIMAL(10, 4), nullable=False, default=0, comment="Query cost")

    feedback_rating = Column(Integer, nullable=True, comment="User feedback rating")

    feedback_text = Column(Text, nullable=True, comment="User feedback text")
    ...
    results = relationship(
        "KnowledgeItem", secondary=query_results, back_populates="queries"
    )
...
class AuditLog(Base):
    ...
    action = Column(SQLEnum(AuditAction), nullable=False, comment="Action performed")
    ...
    old_values = Column(
        MutableDict.as_mutable(JSON), nullable=True, comment="Previous values"
    )

    new_values = Column(
        MutableDict.as_mutable(JSON), nullable=True, comment="New values"
    )
...
# Database functions - commented out for SQLite compatibility
...
# Triggers for each table - commented out for SQLite compatibility
