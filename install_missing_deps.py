...
def main():
    ...
    logger.info(f"❌ Failed installations: {len(missing_packages) - len(installed_packages)} packages")
    ...
    if missing_critical:
        logger.error(f"❌ Critical packages still missing: {', '.join(missing_critical)}")
