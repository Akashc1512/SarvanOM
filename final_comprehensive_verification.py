...
def main():
    ...
    total_code = (
        factcheck_size + synthesis_size + agent_utils_size + retrieval_utils_size
    )
    reuse_ratio = (
        ((agent_utils_size + retrieval_utils_size) / total_code * 100)
        if total_code > 0
        else 0
    )
    ...
    factcheck_integration = check_integration(
        "shared/core/agents/factcheck_agent.py", factcheck_components
    )
    ...
    synthesis_integration = check_integration(
        "shared/core/agents/synthesis_agent.py", synthesis_components
    )
    ...
    agent_utils_completeness = check_integration(
        "shared/core/agents/agent_utilities.py", agent_utility_components
    )
    ...
    retrieval_utils_completeness = check_integration(
        "shared/core/agents/retrieval_utilities.py", retrieval_utility_components
    )
    ...
    file_existence_score = (
        sum(1 for file in files_to_check if check_file_exists(file))
        / len(files_to_check)
        * 100
    )

    # Integration checks
    factcheck_integration_score = (
        sum(factcheck_integration.values()) / len(factcheck_integration) * 100
    )
    synthesis_integration_score = (
        sum(synthesis_integration.values()) / len(synthesis_integration) * 100
    )

    # Utility completeness checks
    agent_utils_completeness_score = (
        sum(agent_utils_completeness.values()) / len(agent_utils_completeness) * 100
    )
    retrieval_utils_completeness_score = (
        sum(retrieval_utils_completeness.values())
        / len(retrieval_utils_completeness)
        * 100
    )
    ...
    print(
        f"✅ Retrieval Utilities Completeness: {retrieval_utils_completeness_score:.1f}%"
    )
    print(f"✅ Code Reuse Ratio: {reuse_ratio:.1f}%")
