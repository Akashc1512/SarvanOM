...
class ZeroBudgetLLMManager:
    ...
    def _display_dashboard(self):
        ...
        print(
            f"   Total Errors: {self.metrics['ollama']['errors'] + self.metrics['huggingface']['errors']}"
        )
    ...
    async def _update_metrics(self):
        ...
        total_requests = (
            self.metrics["ollama"]["requests"] + self.metrics["huggingface"]["requests"]
        )
        ...
        if await self._check_ollama_health():
            try:
                ...
                response = await client.generate_text(
                    prompt="Hello, this is a test.", max_tokens=20, temperature=0.1
                )
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            f"❌ Hugging Face test failed: {response.status_code}"
                        )
            except Exception as e:
    ...
    def show_model_details(self):
        ...
        for model in ollama_models:
            ...
            print(
                f"     Status: {'Available' if self._check_ollama_health() else 'Unavailable'}"
            )
        ...
        for model in hf_models:
            ...
            print(
                f"     Status: {'Available' if self._check_hf_health() else 'Unavailable'}"
            )
...
async def main():
    ...
    parser.add_argument(
        "--dashboard", action="store_true", help="Run monitoring dashboard"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--restart-ollama", action="store_true", help="Restart Ollama service"
    )
