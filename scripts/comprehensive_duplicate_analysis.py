...
class DuplicateAnalyzer:
    ...
    def _find_timing_patterns(self, content: str, file_path: str):
        ...
        patterns = [
            ...
            (
                r"execution_time\s*=\s*\(time\.time\(\)\s*-\s*start_time\)\s*\*\s*1000",
                "Timing Calculation",
            ),
            (
                r"processing_time\s*=\s*int\(\(time\.time\(\)\s*-\s*start_time\)\s*\*\s*1000\)",
                "Processing Time",
            ),
        ]
    ...
    def _find_error_handling_patterns(self, content: str, file_path: str):
        ...
        patterns = [
            ...
            (
                r'return\s*\{\s*["\']success["\']\s*:\s*False\s*,\s*["\']error["\']\s*:\s*str\(e\)\s*\}',
                "Error Response",
            ),
            (
                r'logger\.error\(f["\'][^"\']*["\']\s*,\s*error_type=type\(e\)\.__name__',
                "Error Logging",
            ),
        ]
    ...
    def _find_validation_patterns(self, content: str, file_path: str):
        ...
        patterns = [
            (
                r"if\s+not\s+\w+\s+or\s+not\s+isinstance\(\w+,\s*str\):",
                "String Validation",
            ),
    ...
    def _find_agent_workflow_patterns(self, content: str, file_path: str):
        ...
        patterns = [
            (
                r"async\s+def\s+process_task\(self,\s*task:\s*Dict\[str,\s*Any\],\s*context:",
                "Agent Process Task",
            ),
            (
                r"result\s*=\s*await\s+self\.task_processor\.process_task_with_workflow",
                "Task Processor Usage",
            ),
            (
                r"return\s+ResponseFormatter\.format_agent_response",
                "Response Formatting",
            ),
        ]
    ...
    def _find_response_formatting_patterns(self, content: str, file_path: str):
        ...
        for pattern, pattern_name in patterns:
            ...
            if len(list(matches)) > 1:
                self._add_pattern(
                    file_path, pattern_name, "Response Formatting", pattern
                )

    def _add_pattern(
        self, file_path: str, pattern_name: str, pattern_type: str, regex_pattern: str
    ):
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"- **Duplicate Patterns Found**: {len(self.results.duplicate_patterns)}"
        )
        report.append(
            f"- **Pattern Types**: {len(set(p.pattern_type for p in self.results.duplicate_patterns))}"
        )
        report.append("")
        ...
        for pattern_name in priority_patterns:
            for pattern in self.results.duplicate_patterns:
                if pattern.pattern_name == pattern_name:
                    report.append(
                        f"- **{pattern_name}**: Found in {len(pattern.files)} files"
                    )
                    break
