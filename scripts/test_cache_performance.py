...
from shared.core.cache.cache_config import (
    CacheConfig,
    CacheLevel,
    CacheBackend,
    get_cache_config,
...
from shared.core.cache.cache_invalidation import (
...
setup_logging(
    service_name="cache-performance-test", log_level="INFO", log_format="text"
)
...
class CachePerformanceTester:
    ...
    async def test_basic_cache_functionality(self) -> Dict[str, Any]:
        ...
        for level in CacheLevel:
            ...
            print(
                f"  {status} {level.value}: Set={set_time:.1f}ms, Get={get_time:.1f}ms"
            )
    ...
    async def test_query_caching_performance(self) -> Dict[str, Any]:
        ...
        if results["cache_misses"] and results["cache_hits"]:
            miss_times = [
                r["execution_time_ms"] for r in results["cache_misses"] if r["success"]
            ]
            hit_times = [
                r["execution_time_ms"] for r in results["cache_hits"] if r["success"]
            ]
            ...
            if miss_times and hit_times:
                ...
                if avg_miss_time > 0:
                    ...
                    time_saved_percent = (
                        (avg_miss_time - avg_hit_time) / avg_miss_time
                    ) * 100
                else:
        ...
        if "agent_stats" in cache_stats:
            results["api_calls_saved"] = cache_stats["agent_stats"].get(
    ...
    async def test_memory_management(self) -> Dict[str, Any]:
        ...
        for i in range(100):
            data = {
                ...
                "results": [
                    {"doc": f"document {j}", "content": "x" * 1000} for j in range(10)
                ],
                "metadata": {"test_id": i},
            }
        ...
        print(
            f"  Successfully cached: {successful_sets}/{len(large_data_sets)} entries"
        )
    ...
    async def test_cache_invalidation(self) -> Dict[str, Any]:
        ...
        await invalidation_manager.invalidate_by_trigger(
            InvalidationTrigger.MANUAL_FLUSH, {"test": "invalidation_test"}
        )
    ...
    async def test_concurrent_access(self) -> Dict[str, Any]:
        ...
        print(
            f"Running {num_workers} concurrent workers, {iterations_per_worker} operations each..."
        )
        ...
        avg_access_time = sum(r["avg_access_time_ms"] for r in worker_results) / len(
            worker_results
        )
    ...
    async def test_cache_with_agents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"  Speed improvement: {results['synthesis']['speed_improvement']:.1f}x"
            )
        ...
        try:
            ...
            print(f"  Second call: {time2:.1f}ms")
            print(
                f"  Speed improvement: {results['factcheck']['speed_improvement']:.1f}x"
            )
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            all_results["basic_functionality"] = (
                await self.test_basic_cache_functionality()
            )

            # Query caching performance test
            all_results["query_performance"] = (
                await self.test_query_caching_performance()
            )
    ...
    def generate_test_report(self, results: Dict[str, Any]) -> str:
        ...
        if "basic_functionality" in results:
            ...
            functional_count = sum(
                1 for r in basic.values() if r.get("functional", False)
            )
            report.append(
                f"✅ BASIC FUNCTIONALITY: {functional_count}/{len(basic)} cache levels working"
            )
        ...
        if "query_performance" in results:
            ...
            if "performance_improvement" in perf:
                ...
                report.append(
                    f"🚀 QUERY PERFORMANCE: {improvement['improvement_ratio']:.1f}x speed improvement"
                )
                report.append(
                    f"   Time saved: {improvement['time_saved_percent']:.1f}%"
                )
        ...
        if "memory_management" in results:
            ...
            report.append(
                f"💾 MEMORY MANAGEMENT: {'✅' if eviction_working else '❌'} Eviction working properly"
            )
        ...
        if "cache_invalidation" in results:
            ...
            report.append(
                f"🗑️ CACHE INVALIDATION: {'✅' if invalidation_working else '❌'} Invalidation working"
            )
        ...
        if "concurrent_access" in results:
            ...
            report.append(
                f"⚡ CONCURRENT ACCESS: {throughput:.1f} ops/sec, {hit_rate:.1%} hit rate"
            )
        ...
        if "agent_caching" in results:
            ...
            report.append(
                f"🤖 AGENT CACHING: {working_agents} agents tested successfully"
            )
...
async def main():
    ...
    try:
        ...
        if (
            "query_performance" in results
            and "performance_improvement" in results["query_performance"]
        ):
        ...
        if "memory_management" in results:
            ...
            print(f"\nMemory Management:")
            print(
                f"  • Entries cached: {memory['successful_sets']}/{memory['total_attempts']}"
            )
            print(
                f"  • Eviction policy: {'✅ Working' if memory.get('early_entry_evicted') else '❌ Not working'}"
            )
            print(
                f"  • Recent entries preserved: {'✅' if memory.get('recent_entry_exists') else '❌'}"
            )
