...
class OllamaHuggingFaceSetup:
    ...
    def __init__(self):
        ...
        self.optional_ollama_models = ["llama3.2:70b", "mixtral:8x7b"]
    ...
    async def _setup_huggingface(self):
        ...
        api_key = input(
            "📝 Enter your Hugging Face API key (or press Enter to skip): "
        ).strip()
    ...
    def _update_environment_config(self):
        ...
        if self.setup_status["huggingface"]["configured"]:
            env_settings.extend(["HUGGINGFACE_MODEL=microsoft/DialoGPT-medium"])
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"❌ Hugging Face model test failed: {response.status_code}"
                    )
                    return False
        except Exception as e:
    ...
    def _print_setup_summary(self):
        ...
        print(f"\n🔧 Ollama Status:")
        print(
            f"   Installed: {'✅' if self.setup_status['ollama']['installed'] else '❌'}"
        )
        ...
        print(f"\n🔧 Hugging Face Status:")
        print(
            f"   Configured: {'✅' if self.setup_status['huggingface']['configured'] else '❌'}"
        )
        print(
            f"   API Key: {'✅' if self.setup_status['huggingface']['api_key'] else '❌'}"
        )
        print(
            f"   Health: {'✅' if self.setup_status['huggingface']['health'] else '❌'}"
        )
        ...
        if total_models > 0 or self.setup_status["huggingface"]["configured"]:
            print(f"\n💰 Cost Savings:")
            print(
                f"   Free models available: {total_models + (1 if self.setup_status['huggingface']['configured'] else 0)}"
            )
        ...
        if not self.setup_status["huggingface"]["configured"]:
            print(
                f"   - Get Hugging Face API key: https://huggingface.co/settings/tokens"
            )
