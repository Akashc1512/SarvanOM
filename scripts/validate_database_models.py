...
class DatabaseModelValidator:
    ...
    def validate_all_models(self) -> None:
        ...
        self.console.print("[bold blue]Database Model Validation - MAANG Standards[/bold blue]")
    ...
    def _validate_indexes(self, model_class) -> List[ValidationResult]:
        ...
        for field in important_indexes:
            if hasattr(model_class, field):
                field_indexes = [idx for idx in indexes if field in [col.name for col in idx.columns]]
        ...
        if hasattr(model_class, '__table_args__'):
            ...
            if isinstance(table_args, tuple):
                for arg in table_args:
                    if hasattr(arg, '__class__') and 'Constraint' in arg.__class__.__name__:
    ...
    def _validate_security_features(self, model_class) -> List[ValidationResult]:
        ...
        encrypted_fields = []
        for column in model_class.__table__.columns:
            if hasattr(column.type, '__class__') and 'Encrypted' in column.type.__class__.__name__:
    ...
    def _validate_performance_features(self, model_class) -> List[ValidationResult]:
        ...
        for attr_name in dir(model_class):
            attr = getattr(model_class, attr_name)
            if hasattr(attr, 'fget') and hasattr(attr.fget, '__class__') and 'hybrid_property' in str(attr.fget.__class__):
    ...
    def generate_validation_report(self) -> None:
        ...
        if avg_score >= 90:
            self.console.print("[green]Excellent - Models meet MAANG-level standards[/green]")
        elif avg_score >= 80:
            self.console.print("[yellow]Good - Models mostly meet standards with minor issues[/yellow]")
        elif avg_score >= 70:
            self.console.print("[orange]Fair - Models need improvements to meet standards[/orange]")
        else:
