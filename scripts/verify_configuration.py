...
class ConfigurationVerifier:
    ...
    def _test_environment_manager(self) -> None:
        ...
        try:
            ...
            if env_manager.environment.value != self.environment.lower():
                self.errors.append(f"Environment mismatch: expected {self.environment}, got {env_manager.environment.value}")
            else:
                print(f"✅ Environment manager initialized for {env_manager.environment.value}")
            
            # Verify configuration is loaded
            config = env_manager.get_config()
            if config.name != self.environment.lower():
                self.errors.append(f"Configuration name mismatch: expected {self.environment}, got {config.name}")
            else:
    ...
    def _test_configuration_loading(self) -> None:
        ...
        try:
            ...
            required_fields = [
                'name', 'debug', 'testing', 'log_level', 'database_url', 
                'redis_url', 'rate_limit_per_minute', 'max_request_size_mb'
            ]
        ...
        try:
            ...
            if database_url:
                ...
                if self.environment in ["production", "staging"]:
                    self.errors.append("DATABASE_URL is required for production/staging environments")
                else:
            ...
            if redis_url:
                ...
                if self.environment in ["production", "staging"]:
                    self.errors.append("REDIS_URL is required for production/staging environments")
                else:
            ...
            if openai_key or anthropic_key:
                ...
                if self.environment in ["production", "staging"]:
                    self.errors.append("At least one AI provider API key is required for production/staging")
                else:
                    self.warnings.append("No AI provider API keys set, using mock responses")
    ...
    def _test_configuration_files(self) -> None:
        ...
        try:
            ...
            if found_config:
                ...
                self.warnings.append(f"No configuration file found for environment: {self.environment}")
        ...
        try:
            ...
            if self.environment == "production":
                ...
                if config.mock_ai_responses:
                    self.errors.append("MOCK_AI_RESPONSES should be False in production")
                if config.skip_authentication:
                    self.errors.append("SKIP_AUTHENTICATION should be False in production")
                if config.enable_debug_endpoints:
                    self.errors.append("ENABLE_DEBUG_ENDPOINTS should be False in production")
                ...
                if config.enable_debug_endpoints:
                    self.errors.append("ENABLE_DEBUG_ENDPOINTS should be False in staging")
                if not config.database_url:
    ...
    def _test_security_checks(self) -> None:
        ...
        try:
            ...
            if self.environment == "development":
                if config.jwt_secret_key and config.jwt_secret_key != "dev-secret-key-change-in-production":
                    self.warnings.append("JWT_SECRET_KEY should use default dev value in development")
            
            # Check for secure defaults
            if self.environment in ["production", "staging"]:
                if not config.security_headers_enabled:
                    self.warnings.append("SECURITY_HEADERS_ENABLED should be True in production/staging")
                if not config.audit_log_enabled:
                    self.warnings.append("AUDIT_LOG_ENABLED should be True in production/staging")
                if not config.backup_enabled:
                    self.warnings.append("BACKUP_ENABLED should be True in production/staging")
    ...
    def _test_performance_settings(self) -> None:
        ...
        try:
            ...
            if config.rate_limit_per_minute < 1:
                ...
                self.warnings.append("RATE_LIMIT_PER_MINUTE is very high, consider reducing")
            ...
            if config.max_request_size_mb < 1:
                ...
                self.warnings.append("MAX_REQUEST_SIZE_MB is very high, consider reducing")
            ...
            if config.agent_timeout_seconds < 1:
                ...
                self.warnings.append("AGENT_TIMEOUT_SECONDS is very high, consider reducing")
    ...
    def _test_feature_flags(self) -> None:
        ...
        try:
            ...
            disabled_features = [name for name, enabled in config.features.items() if not enabled]
