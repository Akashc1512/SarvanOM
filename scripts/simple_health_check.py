...
class SimpleHealthChecker:
    ...
    async def test_endpoint(self, endpoint: str, method: str = "GET", 
                          ...
                          headers: Optional[Dict] = None) -> HealthTestResult:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        basic_endpoints = [r for r in self.results if r.endpoint in ["/", "/health", "/docs", "/openapi.json"]]
        ...
        report = {
            ...
            "failed_tests": [self._result_to_dict(r) for r in self.results if not r.success],
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        slow_responses = [r for r in self.results if r.response_time > 0.3]
        if slow_responses:
            recommendations.append(f"Optimize {len(slow_responses)} slow endpoints (>300ms)")
        
        # Check if API Gateway is not running
        connection_errors = [r for r in self.results if "refused" in str(r.error).lower() or "connect" in str(r.error).lower()]
...
def print_report(report: Dict[str, Any]):
    ...
    if failed_tests:
        ...
        for test in failed_tests:
            print(f"   {test['method']} {test['endpoint']} - {test['status_code']} ({test['response_time']}s)")
            if test['error']:
    ...
    # Successful tests
    successful_tests = [r for r in report["endpoint_results"]["basic_endpoints"] + 
                       report["endpoint_results"]["auth_endpoints"] + 
                       report["endpoint_results"]["query_endpoints"] if r["success"]]
