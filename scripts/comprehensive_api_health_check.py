...
class APIHealthChecker:
    ...
    async def test_endpoint_with_headers(self, method: str, endpoint: str, 
        ...
        try:
            ...
            if expected_status:
                ...
                success = response.status_code in [200, 201] or (response.status_code == 401 and not self.auth_token)
            else:
            ...
            result = TestResult(
                ...
                auth_success=response.status_code in [200, 201] if auth_required else None
            )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        public_endpoints = [r for r in self.results if not r.auth_required and r.method != "WS"]
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if failed_endpoints:
            recommendations.append(f"⚠️  {len(failed_endpoints)} endpoints failed - review server logs")
        
        # Check response times
        slow_endpoints = [r for r in self.results if r.response_time > 1.0]
        if slow_endpoints:
            recommendations.append(f"🐌 {len(slow_endpoints)} endpoints are slow (>1s) - optimize performance")
        ...
        if len(auth_tests) > 0 and len(auth_success) == 0:
            recommendations.append("🔐 Authentication flow is not working - check auth service")
        
        # Check WebSocket
        ws_tests = [r for r in self.results if r.method == "WS"]
        if ws_tests and not any(r.success for r in ws_tests):
            recommendations.append("🔌 WebSocket connectivity failed - check WebSocket service")
        
        if not recommendations:
            recommendations.append("✅ All systems operational - no immediate issues detected")
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        for result in report["endpoint_results"]["public_endpoints"]:
            ...
            self.log(f"   {status} {result['method']} {result['endpoint']} - {result['status_code']} ({result['response_time']:.3f}s)")
        ...
        for result in report["endpoint_results"]["auth_endpoints"]:
            ...
            self.log(f"   {status} {auth_status} {result['method']} {result['endpoint']} - {result['status_code']} ({result['response_time']:.3f}s)")
            if result["error_message"]:
                self.log(f"      Error: {result['error_message']}")
...
async def main():
    ...
    try:
        ...
        print("   uvicorn services.api-gateway.main:app --reload --host 0.0.0.0 --port 8000")
        return
