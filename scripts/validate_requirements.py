...
def main():
    ...
    for import_name, package_name in import_to_package.items():
        if (
            import_name in all_imports
            and package_name.replace("-", "_").lower() in installed_packages
        ):
    ...
    for import_name in sorted(all_imports):
        ...
        if (
            ...
            and package_name.replace("-", "_").lower() not in installed_packages
        ):
    ...
    optional_packages = {
        ...
        "okhttp3",
        "core",
        "prompts",  # Local modules referenced in tests but not present
    }
