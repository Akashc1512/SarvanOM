...
class MAANGIncidentResponse:
        ...
        for incident in self.incidents:
            if incident.status == IncidentStatus.DETECTED:
                ...
                if incident.acknowledged_at:
                    time_since_acknowledgment = (
                        datetime.now(timezone.utc) - incident.acknowledged_at
                    ).total_seconds() / 60
                    sla_time = (
                        self.sla_targets[incident.severity] * 2
                    )  # Double the SLA time
    ...
    async def _auto_investigate_incident(self, incident: Incident) -> None:
        ...
        logger.info("Auto-investigation started", incident_id=incident.id)
    ...
    async def _investigate_security_incident(self, incident: Incident) -> None:
        ...
        if security_manager:
            ...
            if threat_stats.get("blocked_ips", 0) > 10:
                incident.root_cause = (
                    "Multiple IP addresses blocked due to suspicious activity"
                )
    ...
    def get_incident_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "incidents_by_severity": {
                severity.value: len(
                    [inc for inc in active_incidents if inc.severity == severity]
                )
                for severity in IncidentSeverity
            },
            "incidents_by_type": {
                incident_type.value: len(
                    [inc for inc in active_incidents if inc.type == incident_type]
                )
...
async def main():
    ...
    parser.add_argument(
        "--summary", action="store_true", help="Show incident summary and exit"
    )
