...
class VectorBackendChecker:
    ...
    async def _get_meilisearch_client(self, url: str, api_key: str = None):
        ...
        try:
            ...
            raise ImportError(
                "Meilisearch Python client not installed. Run: pip install meilisearch"
            )
    ...
    async def _check_meilisearch(self):
        ...
        status = BackendStatus(
            name="Meilisearch", available=False, configured=False, reachable=False
        )
        ...
        try:
            ...
            async def test_connection():
                async with self._get_meilisearch_client(
                    meili_url, meili_api_key or meili_master_key
                ) as client:
    ...
    async def _check_qdrant(self):
        ...
        start_time = time.time()
        status = BackendStatus(
            name="Qdrant", available=False, configured=False, reachable=False
        )
        ...
        try:
            ...
            try:
                ...
                status.error = (
                    "Qdrant Python client not installed. Run: pip install qdrant-client"
                )
                self.results["qdrant"] = status
                return
    ...
    async def _check_pinecone(self):
        ...
        start_time = time.time()
        status = BackendStatus(
            name="Pinecone", available=False, configured=False, reachable=False
        )
            ...
            async def test_connection():
                ...
                try:
                    ...
                    query_result = index.query(
                        vector=test_vector, top_k=1, include_metadata=False
                    )
                    return {
                        "index_exists": index_exists,
    ...
    async def _check_knowledge_graph(self):
        ...
        start_time = time.time()
        status = BackendStatus(
            name="Knowledge Graph", available=False, configured=False, reachable=False
        )
        ...
        try:
            ...
            async def test_connection():
                ...
                # Test basic connectivity
                db = client.db(
                    arango_database, username=arango_user, password=arango_password
                )
                ...
                try:
                    ...
                    if test_value == 1:
                        ...
                        try:
                            ...
                            if not relationships_collection:
                                logger.info(
                                    "📝 Creating ArangoDB relationships collection"
                                )
                                db.create_collection("relationships", edge=True)

                            # Test a simple graph query
                            result = db.aql.execute(
                                "FOR doc IN entities RETURN COUNT(doc)"
                            )
                            ...
                            return {
                                ...
                                "entities_collection_exists": entities_collection
                                is not None,
                                "relationships_collection_exists": relationships_collection
                                is not None,
                                "test_query_successful": True,
                            }
                        except Exception as e:
                    ...
                    return {"connection_successful": False, "test_error": str(e)}
