...
def fix_env_file():
    ...
    content = re.sub(r'ELASTICSEARCH_URL="([^"]+)"', r"ELASTICSEARCH_URL=\1", content)

    # Fix Pinecone API key (remove quotes if present)
    content = re.sub(r'PINECONE_API_KEY="([^"]+)"', r"PINECONE_API_KEY=\1", content)

    # Fix Anthropic API key (remove quotes if present)
    content = re.sub(r'ANTHROPIC_API_KEY="([^"]+)"', r"ANTHROPIC_API_KEY=\1", content)

    # Fix OpenAI API key (remove quotes if present)
    content = re.sub(r'OPENAI_API_KEY="([^"]+)"', r"OPENAI_API_KEY=\1", content)
