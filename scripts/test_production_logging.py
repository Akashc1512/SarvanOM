...
def test_metrics_collection():
    ...
    logger.warning(
        "Rate limit approaching", current_usage=85, limit=100, user_id="user456"
    )
...
def test_health_check_logging():
    ...
    health_logger.log_health_check(
        "database",
        "healthy",
        {"connection_time_ms": 25, "active_connections": 5, "max_connections": 100},
    )

    health_logger.log_health_check(
        "cache", "healthy", {"hit_rate": "85%", "memory_usage": "60%"}
    )
    ...
    health_logger.log_health_check(
        "database",
        "unhealthy",
        {"error": "Connection timeout", "last_success": "2024-12-28T10:00:00Z"},
    )
...
def test_performance_monitoring():
    ...
    response_times = [
        ...
        200,
        180,
    ]  # Some high times to trigger alerts
