#!/usr/bin/env python3
"""
Comprehensive Health Check and Smoke Test for Universal Knowledge Hub API Gateway
Tests the actual production API Gateway using real endpoints and configurations.

This script:
- Starts the backend using the main entrypoint (services/api-gateway/main.py)
...
class ComprehensiveHealthCheck:
    """Comprehensive health check and smoke test for the API Gateway."""
    ...
    def setup_environment(self):
        ...
        os.environ.setdefault("SERVICE_NAME", "sarvanom-api")
        os.environ.setdefault("VERSION", "1.0.0")
        os.environ.setdefault("RATE_LIMIT_REQUESTS", "60")
        os.environ.setdefault("RATE_LIMIT_TOKENS", "10000")
        ...
        logger.info("✅ Environment variables configured for testing")
    
    def start_backend(self):
        """Start the backend using the main entrypoint."""
        try:
            logger.info("🚀 Starting API Gateway backend...")
    ...
    def generate_report(self):
        ...
        report_file = f"comprehensive_health_check_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def main():
    ...
    try:
        # Setup environment
        health_check.setup_environment()
        
        # Start backend
        if not health_check.start_backend():
            logger.error("❌ Failed to start backend. Exiting.")
            return 1
        
        # Run HTTP tests
        health_check.run_comprehensive_tests()
        
        # Run WebSocket tests
        await health_check.run_websocket_tests()
