...
class ConfigurationTester:
    ...
    def _test_configuration_precedence(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if config.debug is not False:
                ...
                test_result["issues"].append("DEBUG environment variable did not override config default")
            else:
            ...
            if config.log_level != "ERROR":
                precedence_test["success"] = False
                test_result["issues"].append("LOG_LEVEL environment variable did not override config default")
            else:
    ...
    def _test_configuration_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(missing) == 0:
                test_result["issues"].append("Production environment should require critical configuration")
        ...
        try:
            ...
            test_result["issues"].append(f"Development validation test failed: {str(e)}")
        finally:
            self._restore_environment()
    ...
    def _test_critical_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(missing) > 0:
                test_result["issues"].append(f"Should have no missing config, but found: {missing}")
    ...
    def _set_test_environment(self, env_vars: Dict[str, str]):
        ...
        for key in list(os.environ.keys()):
            if key.startswith(('APP_', 'DATABASE_', 'REDIS_', 'JWT_', 'MEILISEARCH_', 'OPENAI_', 'DEBUG', 'LOG_')):
    ...
    def _generate_test_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for env_name, env_result in results["environment_tests"].items():
            ...
            if env_result["success"]:
                ...
                summary["failures"].extend([f"{env_name}: {issue}" for issue in env_result["issues"]])
        
        # Count other tests
        for test_category in ["precedence_tests", "validation_tests", "critical_config_tests"]:
...
def main():
    ...
    if args.missing_config:
        ...
        if missing:
            ...
            print(f"✅ All critical configuration is present for {manager.environment.value}")
        return
