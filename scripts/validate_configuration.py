...
def check_environment_variables() -> Tuple[bool, List[str], List[str]]:
    ...
    if app_env not in REQUIRED_VARS:
        logger.warning(
            f"Unknown environment '{app_env}', using development requirements"
        )
        app_env = "development"
    ...
    for var in recommended_vars:
        if not os.getenv(var):
            warnings.append(
                f"Recommended variable '{var}' is not set for {app_env} environment"
            )
...
def check_security_configuration() -> Tuple[bool, List[str]]:
    ...
    if app_env == "production" and "*" in cors_origins:
        warnings.append(
            "CORS_ORIGINS contains wildcard (*) in production - security risk"
        )
