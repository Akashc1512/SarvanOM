    ...
    >
      <div className="flex">
        ...
        <div className="ml-3 flex-1">
          <h3
            className={`text-sm font-medium ${isError ? "text-red-800" : "text-yellow-800"}`}
          >
            {isError ? "Error processing your request" : "Warning"}
          </h3>
          <div
            className={`mt-2 text-sm ${isError ? "text-red-700" : "text-yellow-700"}`}
          >
            <p>{error}</p>
            {requestId && (
              <p className="mt-1 text-xs opacity-75">Request ID: {requestId}</p>
            )}
          </div>
          {onRetry && (
            <div className="mt-4">
