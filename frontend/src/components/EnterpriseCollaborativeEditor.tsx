...
export function EnterpriseCollaborativeEditor({
  ...
  useEffect(() => {
    ...
    const connectSocket = async () => {
      try {
        await socketIOManager.connect(userId);
        setIsConnected(true);
  ...
  const setupEventListeners = useCallback(() => {
    ...
    // Room events
    socketIOManager.on("room_joined", (data) => {
    ...
    socketIOManager.on("user_joined", (data) => {
      setActiveUsers((prev) => {
    ...
    // Cursor events
    socketIOManager.on("cursor_updated", (data) => {
    ...
    // Typing events
    socketIOManager.on("typing_started", (data) => {
      if (data.user_id !== userId) {
        setTypingUsers((prev) => {
    ...
    socketIOManager.on("typing_stopped", (data) => {
  ...
  const handleCursorMove = useCallback(() => {
    ...
    if (cursorPosition !== lastCursorPosition.current) {
      socketIOManager.updateCursor(cursorPosition);
      lastCursorPosition.current = cursorPosition;
    }
  }, []);

  const handleTyping = useCallback(() => {
    socketIOManager.startTyping();
  }, []);
