...
const mockResponses = {
  ...
  "/query": {
    answer:
      "This is a mock response from the Universal Knowledge Platform. The system is currently in prototype mode.",
    confidence: 0.85,
    citations: [
      {
        id: "1",
...
const server = http.createServer((req, res) => {
  ...
  if (path === "/health") {
    ...
    req.on("end", () => {
      ...
      res.writeHead(200);
      res.end(
        JSON.stringify({ status: "success", message: "Feedback received" }),
      );
    });
  } else {
    res.writeHead(404);
